...
//
// C3（Task 4）起的职责边界：新解释的六维展示归 匹配分析块（模型组件）；本组件只保留
// 「解释合法缺失但有旧正向依据」时的有限依据展示职责，二者不并存（页面接线 Task 5/6
// 落位：同一分析区只渲染其一，已解码解释在场时不再渲染本组件的重复分析）。渲染行为
// 本任务不变 —— 现有消费者（在线简历正文 / 匿名在线简历）由 Task 5/6 迁移。
