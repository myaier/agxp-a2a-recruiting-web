...
/** 真人会话顶部操作排「电话」——听筒轮廓。
 *  意向确认后双盲已解除，号码本来就互相可见，所以是「看」不是「申请换」，
 *  图标取最普通的听筒，不带申请 / 交换语义的箭头。 */
export function 电话图标({ 尺寸 = 20, 色 = '#6f7268', 线宽 = 1.8 }: 图标属性) {
  return (
    <svg
      width={尺寸}
      height={尺寸}
      ...
      aria-hidden
    >
      <path d="M9.4 3.6 5.6 3.6a2.2 2.2 0 0 0-2.2 2.4c.4 7.6 6.6 13.8 14.2 14.2a2.2 2.2 0 0 0 2.4-2.2v-3.4l-4.4-1.6-1.8 2.2a14.4 14.4 0 0 1-5.2-5.2l2.2-1.8Z" />
    </svg>
  );
}

/** 真人会话顶部操作排「微信」——大小两个气泡 + 各两颗点。
 *  点是必需的：本文件已有一个无点双气泡（气泡图标 = 底部导航「消息」），
 *  去掉点这两个图标就分不出来了。 */
export function 微信图标({ 尺寸 = 20, 色 = '#6f7268', 线宽 = 1.8 }: 图标属性) {
  return (
    <svg
      width={尺寸}
      height={尺寸}
      ...
      aria-hidden
    >
      <path d="M8.8 2.9c-3.6 0-6.5 2.5-6.5 5.6 0 1.8 1 3.4 2.5 4.4l-.8 2.3 2.7-1.3c.7.2 1.4.3 2.1.3 3.6 0 6.5-2.5 6.5-5.7S12.4 2.9 8.8 2.9Z" />
      <path d="M6.7 7.9h.01M10.9 7.9h.01" />
      <path d="M16.5 10.4c-2.8 0-5.1 2-5.1 4.4s2.3 4.4 5.1 4.4c.6 0 1.2-.1 1.7-.3l2.2 1.1-.6-1.9c1.1-.8 1.8-1.9 1.8-3.3 0-2.4-2.3-4.4-5.1-4.4Z" />
      <path d="M14.7 14.2h.01M18.3 14.2h.01" />
    </svg>
  );
}
