...
import type { 详情外壳属性, 详情Tab, 顶栏信息 } from './类型';

/** Tab 名即屏上产品文案（spec §1/§3.1），按 端 投影：第一 Tab 恒「代谈进度」，
 * 第二 Tab 求职端「职位详情」、招聘端「在线简历」；Tab 键固定 进度 / 资料
 * （契约 A 冻结），深链 ?tab=job / ?tab=resume 仍由连接层映射到键「资料」。 */
const Tab名表: Record<顶栏信息['端'], readonly { 键: 详情Tab; 文: string }[]> = {
  求职: [
    { 键: '进度', 文: '代谈进度' },
    { 键: '资料', 文: '职位详情' },
  ],
