...
// 「XX缺失」一行）；空数组 = 提供了但一条没有 → 「暂无…」。
// 匹配分析区（Task 6，Spec §3.4）：唯一输入 = 信息.分析 的 C3 展示模型，由 共享
// 匹配分析块 渲染 —— 分析.分数 0 是合法数值照常显示；解释显式 null 由块给「暂无该次
// 匹配的详细分析」+ 有限依据，绝不补六条假状态。默认 藏环（在谈详情顶栏分数是唯一
// 总分，Spec §3.4）；聊天「看职位」全屏层没有顶栏，调用方传 分析藏环=false 让块内
// 画该层的唯一总分环。
// 无合法公司导航坐标时公司区与导航入口位置照旧保留，入口禁用并就地解释 ——
// 本组件绝不从公司名文本推 opaque ID。
...
export function 职位资料({
  ...
  信息: 职位资料信息;
  公司详情: 详情按钮;
  /** 匹配分析块是否藏环：默认 true（详情顶栏分数唯一）；聊天「看职位」层传 false */
  分析藏环?: boolean;
}) {
  ...
  return (
    <div className={样式.资料列}>
      ...
      {/* 1 匹配分析：C3 展示模型直接交给共享 匹配分析块（自带标题与缺失口径） */}
