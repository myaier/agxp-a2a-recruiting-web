...
// Spec §4.1：公司主页资料行按「调用方是否提供维护能力」渲染（缺省回调 = 不渲染该行），
// 由两适配各自按现有「从注册流」事实决定，展示层不代替流程来源判断。
//
...
export interface 招聘名片展示属性 {
  ...
  保存中: boolean;
  返回: () => void;
  /** 公司主页资料入口（Spec §4.1）：由调用方按流程来源决定是否提供能力 ——
   *  onboarding（注册流）不传 → 该行整行不渲染；日常编辑传入 → 照常可进入公司管理。
   *  展示层不读数据源模式，也不判断流程来源。 */
  打开公司资料?: () => void;
}
    ...
    <次级页外壳>
      ...
      <滚动区 样式覆盖={{ padding: '6px 22px 0' }}>
        ...
        {/* ── 公司主页资料：名片是「这个人」，公司主页是「这家公司」。
            Spec §4.1：onboarding 不出现公司维护入口 —— 缺省回调即整行不渲染
            （不是「点进去被拒」），公司资料仍由现有公司管理入口维护。 ── */}
        {打开公司资料 ? (
