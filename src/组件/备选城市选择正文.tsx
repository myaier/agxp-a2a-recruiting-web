...
//   当前/历史访问城市 → 国内/海外热门城市 → 行政区分组（或搜索结果）→ 底部已选 chips + 保存。
 ...
 *  Task 4：热门区拆成 国内热门城市 / 海外热门城市 两区，内容来自精选配置，
 *  不含港澳台。 */
        ...
        <div className={`${样式.列表区} 滚动区`}>
          {在搜索 ? (
            ...
            <>
              ...
              {/* 首页还没到（精选/分组都空）时沿用现有加载文案 */}
              {加载中 && 国内热门项们.length === 0 && 海外热门项们.length === 0 && 分组们.length === 0 ? (
