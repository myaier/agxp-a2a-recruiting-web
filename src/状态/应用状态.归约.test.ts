...
describe('应用状态 reducer', () => {
  ...
  it('DF-014 水合账号资料 不让缓存里的旧 null 覆盖已存的权威头像，其他允许项照常水合', () => {
    // 权威头像已在 state：服务端 account-profile 回读后经 存求职头像 写入
    const 有权威头像 = 归约(初始状态, { 型: '存求职头像', 图: '/api/v1/me/avatar/content?v=3' });
    const 水合后 = 归约(有权威头像, {
      型: '水合账号资料',
      范围键: 'AGXP账号资料v2:backend:stg:sub_1',
      // 缓存快照在 资料持久化 里与 空账号资料 合并后必带 求职头像: null，
      // 旧缓存的 null / 旧 URL / 旧本地图片都不得覆盖服务端值
      快照: { ...空账号资料, 求职头像: null, 未认证公司声明: '缓存里的声明' },
    });
    expect(水合后.求职头像).toBe('/api/v1/me/avatar/content?v=3');
    // 头像之外的缓存允许项照常水合，清空行为（清账号资料）不受影响
    expect(水合后.未认证公司声明).toBe('缓存里的声明');
    expect(水合后.资料缓存范围键).toBe('AGXP账号资料v2:backend:stg:sub_1');
    const 清空后 = 归约(水合后, { 型: '清账号资料' });
    expect(清空后.求职头像).toBeNull();
  });
  ...
  // ── 合同 C：薪资并入首屏后的周期守卫 —— 有效周期变化即清掉已确认薪资 ──
  // 20K 月薪绝不能被当成 20 元/天 继续用；同周期（含重复点击同一类型）原样保留，
  // 草稿其余字段与在飞 待写入 槽都不受影响（不加新持久化确认位）。
  describe('存求职筛选偏好 的薪资周期守卫', () => {
    const 空基底 = { 城市们: [], 职位: [], 城市引用们: [], 职位引用们: [] };
    /** 已确认薪资的起手状态：社招全职（月薪）+ 20-30K 已确认 */
    const 已确认月薪 = () =>
      归约(初始状态, {
        型: '存薪资预填', 下限: 20, 上限: 30, 单位: '月薪K', ...空基底,
      });

    it('同周期改其他偏好：已确认薪资原样保留', () => {
      const 下一 = 归约(已确认月薪(), {
        型: '存求职筛选偏好',
        偏好: { 求职类型: ['社招全职'], 办公方式: ['现场', '全远程'] },
        ...空基底,
      });
      expect(下一.引导预填?.薪资).toEqual({ 下限: 20, 上限: 30, 单位: '月薪K' });
    });

    it('有效周期变化（月薪 → 日薪）：删除薪资属性，要求重新确认', () => {
      const 下一 = 归约(已确认月薪(), {
        型: '存求职筛选偏好',
        偏好: { 求职类型: ['实习生'], 办公方式: ['现场'], 实习月数: 3, 每周到岗天数: 4 },
        ...空基底,
      });
      expect(下一.引导预填).not.toHaveProperty('薪资');
    });

    it('日薪 → 月薪同样清除；反方向再切回也不复活旧值', () => {
      const 已确认日薪 = 归约(初始状态, {
        型: '存薪资预填', 下限: 300, 上限: 500, 单位: '元/天', ...空基底,
      });
      const 切月薪 = 归约(已确认日薪, {
        型: '存求职筛选偏好', 偏好: { 求职类型: ['社招全职'], 办公方式: ['现场'] }, ...空基底,
      });
      expect(切月薪.引导预填).not.toHaveProperty('薪资');
      const 再切日薪 = 归约(切月薪, {
        型: '存求职筛选偏好', 偏好: { 求职类型: ['实习生'], 办公方式: ['现场'] }, ...空基底,
      });
      expect(再切日薪.引导预填).not.toHaveProperty('薪资');
    });

    it('清除只动薪资：草稿其余字段与在飞 待写入 槽原样保留', () => {
      const 带槽 = {
        ...已确认月薪(),
        引导预填: {
          ...已确认月薪().引导预填!,
          到岗: '在职 · 考虑机会',
          建档: { 待写入: { 种类: 'summary' as const, 幂等键: 'k1', 阶段: 'prepared' as const } },
        },
      };
      const 下一 = 归约(带槽, {
        型: '存求职筛选偏好', 偏好: { 求职类型: ['实习生'], 办公方式: ['现场'] }, ...空基底,
      ...
      expect(下一.引导预填).not.toHaveProperty('薪资');
      expect(下一.引导预填?.到岗).toBe('在职 · 考虑机会');
      expect(下一.引导预填?.建档?.待写入).toEqual({ 种类: 'summary', 幂等键: 'k1', 阶段: 'prepared' });
    });
  });
});
