 ...
 * 定义放状态层：这三个档是全局状态的取值，类型跟着状态走。
 * 第二批（2026-09-09 产品负责人定稿删筛选）：原编辑入口 组件/在谈筛选层 已删除，
 * 字段与归约保留（「我」页「待你拍」仍派发 看全部在谈 / 企业看全部在谈 写这个档），
 * 但两端 Mock 在谈列表不再按它过滤、固定显示全部单。
 */
...
import { 创建助手会话访问 } from './后端/助手会话访问';
import type { 助手会话访问 } from './后端/助手会话访问';
...
export function 归约(旧: 状态, 动作: 动作): 状态 {
  switch (动作.型) {
    ...
    // DF-014：求职头像同口径排除 —— 快照在 资料持久化 里与 空账号资料 合并后必带
    // 默认 null，旧缓存的 null/旧 URL/旧本地图片都不得覆盖 存求职头像 落下的权威值
    //（该 action 只承担缓存水合；权威路径走独立的 存求职头像）。清账号资料 仍照常清空。
    case '水合账号资料': {
      const {
        当前企业关系编号: _未校验编号,
        当前意向编号: _未校验意向编号,
...
interface 应用状态值 {
  ...
  /** 求职端助手会话访问 seam（合同 C）：Backend + 已登录 + candidate 主体才提供；其余 null */
  助手会话: 助手会话访问 | null;
}
...
export function 应用状态提供者({ children, 数据源 }: { children?: ReactNode; 数据源?: 招聘数据源选择 }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // P0 修复 Task 1：招聘方两个水合阶段同口径回底座 —— 上个会话可能停在 缺失|成功|失败，
      // 而新主体若是 candidate / null 角色，水合角色数据 根本不进招聘方分支，没有任何后续写入
      // 会覆盖它们；不在这里复位就会把上个招聘方的 onboarding 判定漏给下一个主体。
      设后端状态((旧) => ({
        ...重置Agent规则后端状态(旧),
        ...创建空招聘方组织水合状态(),
        附件简历库: null,
      ...
      if (会话失效) {
        ...
        // 初始化 也已随之落成 完成，这里只早退，不再重复清理。
        return;
      }
      // mount 收口前区分三种终局：清理（上面早退）/ 被更新会话取代 / 正常提交。
      // 水合在飞期间完成了新的短信登录（主体或代际已换）时，数据提交已由外层栅栏整包丢弃，
      // 这里只收口 初始化 —— 绝不把过时的主体/登录态写回去覆盖新会话。
      if (
        主体标识引用.current !== 主体.subject_id ||
        会话代际.current !== 本次代际
      ) {
  ...
  // 后端操作依赖全家桶：操作工厂与 助手会话访问 seam（见下方 助手会话 memo）共用同一份 ——
  // seam 的当前 401 统一清理必须绑全套既有依赖（与既有会话操作同口径），不引入第二套 deps 抽象。
  // 字段全部是稳定 ref / React setter / 幂等纯函数，逐次重建的对象字面量在两个 useMemo 间等价。
  const deps: 后端操作依赖 = {
    是后端,
    后端,
    派发,
  ...
  // ── 求职端助手会话访问 seam（Task 4 / 合同 C）────────────────────────────
  // Backend + 已登录 + candidate 主体才提供；范围键 = 环境 + subject + 角色 + 真实会话
  // 代际。每次渲染从 会话代际.current 读快照进 memo 依赖：登出 / 401 清理 / 重登（同
  // subject 也递增代际）/ 切身份全部伴随 后端状态 更新触发重渲染，seam 必在新代际下重建，
  // 旧 seam 的在飞结果由其身份 fence 抛 AbortError 丢弃。当前 401 收口 清账号状态 全套
  // 依赖（上方 deps 全家桶），与既有会话操作同口径。
  const 助手会话代际 = 会话代际.current;
  const 助手会话 = useMemo<助手会话访问 | null>(
    () => 创建助手会话访问({
      ...deps,
      环境,
      主体: 后端状态.主体,
    ...
    // 是后端 / 后端 在同一 Provider 实例下不变；主体 / 登录态 / 代际 任一变化都重建 seam
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [是后端, 后端, 环境, 后端状态.主体, 后端状态.已登录, 助手会话代际],
  );

  const 值 = useMemo<应用状态值>(
    () => ({ 状态, 派发, 数据源模式: 源.模式, 后端状态, 操作, 目录查询, 助手会话 }),
    [状态, 派发, 源.模式, 后端状态, 操作, 目录查询, 助手会话],
  );
