...
export function 创建候选操作(deps: 后端操作依赖): 候选操作 {
  ...
  /**
   * 权威简历快照落地 = 设 state + **同步**镜像引用（fix-r2 / 合同 B）。
   *
   * 后端状态引用.current 平时只在渲染期跟随 state（见 应用状态.tsx）。而聚合保存链是
   * 「保存简历 → 成功续立刻写个人优势」同一微任务里的两次调用：第二次合成 next 时读的就是
   * 这个镜像，渲染没落地就读到**链前**快照。那样 next 会带着链前的技能/经历，而 简历域保存
   * 又按服务端最新事实（如「已存条目不在链前快照 → 权威重读」）判定差异 —— 未改动的分区被
   * 当成回退写回；经历/教育/证书有缺项保护，**裸数组技能没有**，于是被清空
   * （e2e 实测 PATCH /me/resume/skills {"skills":[]}）。
   *
   * 因此每个「拿到权威简历快照」的点都同时推进镜像：渲染期赋值仍以 state 为准，二者同源，
   * 谁后到谁是同一份事实。
   */
  function 设权威简历快照(快照: BFF简历): void {
    设后端状态((旧) => ({ ...旧, 简历快照: 快照 }));
    后端状态引用.current = { ...后端状态引用.current, 简历快照: 快照 };
  }
  ...
  function 处理写入错误(错误: unknown, 本次主体: string | null, 本次代际: number): never {
    ...
    if (错误 instanceof BFF错误 && 栅栏仍立) {
      if (错误.status === 401) {
        ...
        水合简历并保留空身份草稿(页面);
        // 非 DTO（测试桩直接给页面快照）时没有 BFF 快照可落，保持既有行为
        if (是原始DTO) 设权威简历快照(权威 as never);
      }
    }
    throw 错误;
  }
  ...
  /**
   * 本旅程的跟踪实现（Global 5）：
   * - 发送前：单槽守卫（任何未结算槽 —— prepared 或 received —— 只要不是同一逻辑命令就拦下；
   *   同一逻辑命令推进/重试放行并复用原幂等键）；新键只为 create 种类铸造（CAS 走 ifMatch，无幂等合同）。
   *   received 槽也拦：简历域的 received 槽在 结算单槽 里已先落身份清槽，走不到这里；能带着回执
   *   到这里的只有非简历域槽（意向/头像/文件，Task 6–8 各自接线），拦下它们才不会把在手回执覆盖丢。
  ...
  async function 结算单槽(
    ...
    if (动作 === 'create' && 条目种类 !== null) {
      ...
      if (结算next === null) {
        // Task 6 review r3（裁决 C 扩展）：同样要可上屏 —— 裸 Error 会被 取后端错误文案
        // 收成「请求失败，请稍后再试」。此处早于 后端.保存简历，发送前 没跑过、
        // 取本槽命令() 仍是 null，所以新分类（status 0 + invalid_request = 确定拒绝）
        // 清不到这条正被合法保留的槽。
        throw new BFF错误(0, 'invalid_request', '上一条写入结果未确认，无法安全重放，请先核对原步骤');
      }
      const 结算快照 = await 后端!.保存简历(结算next, 权威, 跟踪);
      if (栅栏仍立()) 设权威简历快照(结算快照.服务端快照);
      return 结算快照.服务端快照;
    }
