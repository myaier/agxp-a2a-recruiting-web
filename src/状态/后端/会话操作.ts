...
export function 清账号状态(
  deps: Pick<后端操作依赖, '派发' | '设后端状态' | '后端' | '主体标识引用' | '会话代际'> &
    Partial<Pick<后端操作依赖, 'P4范围代际' | 'P4幂等意图' | 'P4可见范围'>>,
): void {
 ...
 *   candidate → 简历 + 意向 + 隐私（P3 起并行读取，各自独立派发；隐私响应带
 *               主体/代际栅栏，过时成败都丢弃）+ 附件库（P2 起第四路，同样带栅栏、只写快照）
 *               + P6 规则三路读取（Task 4）；
...
export async function 水合角色数据(
  deps: Pick<后端操作依赖,
    ...
    '读取恢复企业关系编号' | 'P4范围代际' | 'P4幂等意图' | 'P4可见范围'> &
    { 后端: HTTP招聘数据源 },
  主体: BFF主体,
  交互: boolean,
  generation: number,
  ...
  if (角色 === 'candidate') {
    ...
    // P2 Task 3：附件库是第四个并行水合域，与简历/意向/隐私各自独立提交，
    // 应用同一个 captured subject/generation fence：过时成败整包丢弃。
    const 结果 = await Promise.allSettled([
      后端.读取简历(), 后端.读取意向(), 后端.读取隐私(), 后端.读取附件简历库(),
    ]);
    const 隐私响应仍有效 = () => 主体标识引用.current === 起始主体标识 && 会话代际.current === 起始代际;
    const 附件响应仍有效 = () => 主体标识引用.current === 起始主体标识 && 会话代际.current === 起始代际;
    ...
    // 失败与隐私同口径：记入错误们（交互模式抛第一个）、401 标记会话失效、轻提示该域一次；
...
export function 创建会话操作(deps: 后端操作依赖): 会话操作 {
  ...
  /** 清账号状态 需要的子集（与 退出登录 共用，保持口径一致；含 P4 引用做 discovery 清理） */
  const 账号清理依赖 = {
  ...
  return {
    ...
    async 切身份(to) {
      ...
      设后端状态((旧) => ({ ...重置Agent规则后端状态(旧), ...创建空P4发现状态(), 附件简历库: null }));
      清P4发现引用({ P4范围代际, P4幂等意图, P4可见范围 });
      会话代际.current += 1;
