...
import type { 页面简历快照, 页面意向快照, 页面岗位快照, 建档待写入 } from '../../数据/招聘数据源类型';
import type { 候选引导草稿快照 } from '../../数据/资料缓存';
import type { HTTP招聘数据源 } from '../../数据/HTTP招聘数据源';
import { 轻提示 } from '../../组件/轻提示';
import type { 后端状态, 后端操作依赖, 会话操作, 建档草稿操作 } from './类型';
...
import { 创建空候选实名快照, 清候选实名引用 } from './候选实名操作';
...
const 空简历快照: 页面简历快照 = {
  // M：清账号后的空简历身份为 ''（未选择），不借「在职」当注册流默认
  基本信息: { 真名: '', 开始工作年: '', 身份: '' },
  个人优势: '',
  技能: [],
  经历: [],
...
/**
 * J-PILOT-01 Task 3（Spec §8）：委托待核对域的引用级清理 —— 内存未决命令表清空 +
 * 删除 outgoing owner 的 sessionStorage 恢复记录。退出 / 401 / 换主体 / 切离 candidate
 * 统一走这里（与 清候选预填引用 同款纪律：适配器此刻仍绑着 outgoing subject，
 * 下一次渲染才换绑或归 null）。未决 create/retry 命令绝不跨主体/跨角色存活；
 * 普通页面 scope 卸载不清这里（scope 生命周期与命令生命周期分离）。
 */
export function 清委托待核对引用(
  deps: Partial<Pick<后端操作依赖, '委托待核对内存' | '委托待核对存储'>>,
): void {
  deps.委托待核对内存?.current.clear();
  const 会话 = deps.委托待核对存储?.current;
  清除待核对(会话?.storage ?? null, 会话?.owner ?? null);
}
...
export function 清账号状态(
  ...
  设后端状态((旧) => ({
    ...
    ...创建空接触记录状态(),
    // 候选实名：owner summary 快照一并回空底座，不跨主体 / 不跨会话 / 不跨角色存活
    候选实名: 创建空候选实名快照(),
  ...
  清接触记录引用(deps);
  // 候选实名引用级清理：GET 单飞读锁复位 + create/cancel 两把 mutation 锁清空 + 待定 key 删除
  // （清候选实名引用 是必选 Pick 签名，旧依赖桩缺席时跳过，快照摊平已兜底）
  if (deps.候选实名读取锁 && deps.候选实名变更锁 && deps.候选实名提交意图) {
    清候选实名引用(deps);
  }
  // J-PILOT-02 Task 2：建档草稿的内存 ref 同步清空（不等 React 下一帧）——
  // 存储键的删除由 资料持久化 的 scope 变化清理口统一执行。
...
async function 水合角色数据域(
  ...
  if (角色 === 'candidate') {
    ...
    if (意向结果.status === 'fulfilled') {
      // 候选首次角色水合：允许会话缓存里的选择偏好参与本次选择（是否落地由权威列表定）
      deps.提交候选意向快照({
        快照: 意向结果.value,
        subjectId: 主体.subject_id,
...
/**
 * 单槽命令身份（Global 4/6）：除 阶段/回执 外的全部坐标一致 = 同一条命令 ——
 * 重试/推进（prepared → received）可换 阶段，第二条不同命令不得覆盖未结算槽。
 */
function 建档命令坐标(命令: 建档待写入): string {
  return JSON.stringify([
    命令.种类, 命令.本地编号 ?? null, 命令.资源编号 ?? null, 命令.父编号 ?? null,
    命令.请求体 ?? null, 命令.幂等键 ?? null, 命令.ifMatch ?? null, 命令.文件核对 ?? null,
  ]);
}
...
export function 创建会话操作(deps: 后端操作依赖): 会话操作 & 建档草稿操作 {
  ...
  /** 清账号状态 需要的子集（与 退出登录 共用，保持口径一致；含 P4/P7/P8 + 候选预填 + 候选实名 + 委托待核对引用清理） */
  const 账号清理依赖 = {
   ...
   * 候选实名运行时引用 —— 登录水合撞上当前轮 401 时，清账号状态 要带着全套引用一起复位
   * （与 mount / 切身份 同口径）。
   */
  ...
  return {
    /**
     * J-PILOT-02 Task 2（Global 8）：同步更新建档草稿。顺序固定为
     * ① 同步固定内存 ref（含未结算单槽守卫）→ ② 尝试写 subject-scoped session →
     * ③ 派发。存储写入失败只经既有轻提示说明刷新风险，不抛掉输入、不阻断当前
     * 明确保存动作；useEffect 的普通输入持久化只是补齐，不承担 mutation 前固定
     * 命令身份的职责。
     *
     * 单槽契约（调用方必读）：调用方构造 建档 时必须回带当前 待写入 槽（从
     * 建档草稿引用.current 或 引导预填.建档 取原对象原样带上）；省略 待写入
     * 即显式清槽（服务端确定拒绝 / 已确认结算后的清空走这一含义）。守卫只拦
     * 「携带另一条命令覆盖未结算槽」，不拦省略 —— Task 3/4 接线若在飞行命令
     * 未结算时构造不带槽的 建档，会丢命令身份。
     */
    更新候选建档草稿(建档: 候选引导草稿快照['建档']): void {
      if (建档 === undefined) return;
      // 单槽守卫：未结算（阶段 prepared、无回执）的命令不被另一条命令覆盖；
      // 同一条命令推进 received / 普通输入字段不受限制。
      const 现有槽 = deps.建档草稿引用?.current?.待写入;
      let 下一步 = 建档;
      if (现有槽 !== undefined && 现有槽.阶段 === 'prepared'
        && 下一步.待写入 !== undefined
        && 建档命令坐标(下一步.待写入) !== 建档命令坐标(现有槽)) {
        下一步 = { ...下一步, 待写入: 现有槽 };
      }
      if (deps.建档草稿引用) deps.建档草稿引用.current = 下一步;
      const 存储 = deps.候选建档草稿?.current ?? null;
      if (存储 !== null && !存储.写入(下一步)) {
    ...
    async 完成手机登录(code) {
      ...
      if (主体标识引用.current !== null && 主体标识引用.current !== 主体.subject_id) {
        ...
        // 候选实名：读锁/变更锁/key 一并复位，B 不继承 A 的在飞读写与未收口意图
      ...
      // 候选实名：新会话建立前快照回底座并清引用 —— 同 subject 重登也不继承
      // 上次会话未收口的 create intents（快照按需重新 GET）。
      派发({ 型: '清后端Agent规则' });
      设后端状态((旧) => ({
        ...重置Agent规则后端状态(旧),
        ...创建空招聘方组织水合状态(),
    ...
    async 切身份(to) {
      ...
      清候选预填引用({ 候选预填代际, 候选预填读取锁, 候选预填恢复 });
      // 候选实名：读锁/变更锁/key 一并复位，上个角色的在飞读写与未收口意图作废
      if (候选实名读取锁 && 候选实名变更锁 && 候选实名提交意图) {
        清候选实名引用({ 候选实名读取锁, 候选实名变更锁, 候选实名提交意图 });
      }
      ...
      const 本次代际 = 会话代际.current;
      // P7/P8/候选预填/候选实名引用随行：水合途中撞上当前轮 401 时，清账号状态 一并复位全部运行时引用
