...
export interface 公司自述覆盖 {
  ...
  主营业务?: string[];
  /** 公司相册：两组压缩后的 dataURL。公司视频原型阶段只留占位行，不落数据 */
  公司相册?: 公司相册;
  /** 人才发展：培养机制、晋升通道 */
  人才发展?: string;
  /** 在职感受的公司自述。刻意不叫「在职感受」——公司档案里的同名字段是平台侧的
   *  匿名评价统计（感受项[]），重名会在 `{ ...静态档, ...覆盖 }` 合并时把它顶成字符串，
   *  候选端 企业详情 那一块 .map 立刻炸 */
  在职感受自述?: string;
  产品介绍?: string;
  高管介绍?: 高管项[];
}

/** 公司相册：原型阶段只有两组图片，视频只在编辑页留一行占位 */
