 ...
 * 推荐）补值。
 * Task 6（Spec §3.4）：分析 = C3 展示模型的空档形态（无分无解释无旧依据）—— 解释缺失
 * 的缺失口径归 匹配分析块（「暂无该次匹配的详细分析」），这里绝不造六条假状态。
 */
...
export function 从冻结职位到资料(输入: {
  ...
  冻结: BFF安全职位资料 | null;
  分: number | null;
  /** Task 6（Spec §3.4/§5.4）：同一响应的六维解释（展开=对象或显式 null；缺席=未展开
   *  防御路径，模型同样保 null，不从分数/正文重造）。 */
  解释?: BFF匹配解释 | null;
}): 职位资料信息 {
  const 基础 = 从职位摘要到资料(投影冻结职位摘要(输入.摘要, 输入.冻结));
  // 同一响应的权威分与解释：0 合法；显式 null 保 null（缺失口径归 匹配分析块）
  基础.分析 = { 分数: 输入.分, 解释: 输入.解释 ?? null, 有限依据: [], 上下文: '有来源' };
  const 冻结 = 输入.冻结;
...
export function 从P5到详情分段(
  ...
  return view.阶段区块.map((区) => {
    ...
    // 非当前的 S1/S2 段只带它自己的发问块轮次说明。发问块的实时步骤说明（Spec §8B.2）
    // 只挂当前段 —— 非当前段不能套用另一阶段的实时步骤；轮次计数按段对得上就照常带。
    const 段首说明行们: string[] = [];
    const 进度 = view.对话进度;
    const 进度属本段 = 进度 !== null && 进度.stage === 区.stage;
    if (区.stage === currentStage && 态 !== '未到达') {
      段首说明行们.push(view.步骤说明);
      if (进度属本段) {
        if (进度.当前步骤说明 !== null) 段首说明行们.push(进度.当前步骤说明);
        段首说明行们.push(进度.轮次说明);
      } else {
