...
describe('求职匹配分析 · 三态文案保持事实性', () => {
  ...
  it('硬性行只覆盖单一维度时，墨句只声明该维度满足要求', () => {
    // 经验 none → 无行：只有学历行通过，不得宣称「经验与学历」都满足
    const 仅学历 = 求职匹配分析([
      { 要求: '学历 本科', 证据: '同济大学 · 硕士', 态: '有证据', 类: '硬性' },
    ]);
    expect(仅学历?.墨句).toBe('学历满足岗位要求。');
    const 仅经验 = 求职匹配分析([
      { 要求: '经验 3-5 年', 证据: '云衢科技 · 前端工程师', 态: '有证据', 类: '硬性' },
    ]);
    expect(仅经验?.墨句).toBe('经验满足岗位要求。');
  });

  it('双维度硬性行都在且全过时保持既有合并文案', () => {
    const 双过 = 求职匹配分析([
      { 要求: '经验 3-5 年', 证据: '云衢科技 · 前端工程师', 态: '有证据', 类: '硬性' },
      { 要求: '学历 本科', 证据: '同济大学 · 硕士', 态: '有证据', 类: '硬性' },
    ]);
    expect(双过?.墨句).toBe('经验与学历满足岗位要求。');
  });
