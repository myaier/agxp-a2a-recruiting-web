...
import { BFF匹配解释87分样本, BFF匹配解释92分样本 } from '../测试/BFF样本';
...
describe('从P5到职位资料', () => {
  ...
  // Task 6（Spec §3.4/§5.4）：Case 详情的六维解释进 分析 模型（同一响应，不拼接外源）
  it('从P5到职位资料：展开解释透传进分析模型（分数=同响应权威分），显式 null 保 null，缺席（未展开防御）同 null', () => {
    const 展开 = 从P5到职位资料(正常视图({ 匹配分: 87, 匹配解释: BFF匹配解释87分样本 }));
    expect(展开.分析.分数).toBe(87);
    expect(展开.分析.解释).toEqual(BFF匹配解释87分样本);
    expect(展开.分析.有限依据).toEqual([]);
    expect(展开.分析.上下文).toBe('有来源');
    const 显式null = 从P5到职位资料(正常视图({ 匹配分: 87, 匹配解释: null }));
    expect(显式null.分析.解释).toBeNull();
    expect(显式null.分析.分数).toBe(87);
    const 未展开 = 从P5到职位资料(正常视图({ 匹配分: 87 }));
    expect(未展开.分析.解释).toBeNull();
  });
});
...
describe('从冻结职位到资料', () => {
  ...
  it('旧 Case 双区 null：冻结缺席时沿用全缺失底座与缺口说明，不因 job_detail=null 抹去旧四事实', () => {
    ...
    expect(从冻结职位到资料({ 摘要, 冻结: null, 分: 73 }).分析).toEqual({ 分数: 73, 解释: null, 有限依据: [], 上下文: '有来源' });
    expect(从冻结职位到资料({ 摘要, 冻结: null, 分: 73 }).接口缺口说明).toBe('当前在谈详情数据未提供');
  });

  // Task 6（Spec §3.4）：解释入参进分析模型 —— 各详情入口把同响应解释带进来
  it('从冻结职位到资料：解释入参透传进分析模型；缺省（未传）与显式 null 都是 null 解释', () => {
    const 带 = 从冻结职位到资料({ 摘要, 冻结: null, 分: 92, 解释: BFF匹配解释92分样本 });
    expect(带.分析.解释).toEqual(BFF匹配解释92分样本);
    expect(带.分析.分数).toBe(92);
    const 缺省 = 从冻结职位到资料({ 摘要, 冻结: null, 分: 92 });
    expect(缺省.分析.解释).toBeNull();
    const 显式 = 从冻结职位到资料({ 摘要, 冻结: null, 分: 92, 解释: null });
    expect(显式.分析.解释).toBeNull();
  });

  it('真实 0 分合法：分析分槽原样 0（组件按「有分无证据」给缺失说明，不画假环）', () => {
    expect(从冻结职位到资料({ 摘要, 冻结: null, 分: 0 }).分析.分数).toBe(0);
  });
