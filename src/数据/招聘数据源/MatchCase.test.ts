...
/**
 * recruiter open 展开样本（C2）：工作区查询携带 include=candidate_summary,match_explanation，
 * 每个 item 必带两键；展开解释与 match_score=87 同分（解匹配解释 校验分项和=总分=同分）。
 */
const 招聘展开工作区项 = {
  ...P5招聘工作区项Wire,
...
/** recruiter 历史行展开样本：历史查询仅携带 match_explanation（candidate_summary 仍是禁键）。 */
const 招聘展开历史行 = (覆盖: Record<string, unknown> = {}) => ({
  ...P5招聘工作区项Wire,
  match_explanation: BFF匹配解释87分样本,
  ...覆盖,
});
...
const 招聘过滤Open路径 = `/api/v1/recruiter/match-cases?job_id=${职位ID}&limit=50&include=candidate_summary,match_explanation`;
const 候选已终止历史路径 = '/api/v1/me/match-cases/history?lifecycle=ended&limit=50';
const 招聘已完成历史路径 = `/api/v1/recruiter/match-cases/history?lifecycle=completed&job_id=${职位ID}&limit=50&include=match_explanation`;
...
describe('MatchCase数据源', () => {
  ...
  it('双端历史各自只装对应终态架子，行级 lifecycle 与 shelf 不符或终态行带待办都漂移', async () => {
    请求Mock
      ...
      .mockResolvedValueOnce(响应({ items: [招聘展开历史行({ state: P5已完成状态Wire })], next_cursor: null }))
      .mockResolvedValueOnce(响应({ items: [], next_cursor: null }));
  ...
  it('双端 open list、history 与 detail 接受显式空技能数组并精确保留 requiredSkills:[]', async () => {
    ...
    请求Mock
      ...
      .mockResolvedValueOnce(响应({
        items: [招聘展开历史行({ state: P5已完成状态Wire, job: 空技能Job })],
        next_cursor: null,
      }))
      .mockResolvedValueOnce(响应({ ...P5候选详情Wire, job: 空技能Job, match_explanation: BFF匹配解释92分样本 }))
      .mockResolvedValueOnce(响应({ ...P5招聘详情Wire, job: 空技能Job, match_explanation: BFF匹配解释87分样本 }));
  ...
  // ── candidate_summary + match_explanation 展开合同：仅 recruiter 行携带 include 且必带键 ──

  it('recruiter open 带 include=candidate_summary,match_explanation 一次、历史仅 match_explanation，candidate 两架子不携带', async () => {
    请求Mock
    ...
    const 路径们 = 请求Mock.mock.calls.map(([选项]) => 选项.path as string);
    // recruiter open：include 双键恰好一次
    expect(路径们[0].match(/include=candidate_summary,match_explanation/g)).toHaveLength(1);
    // recruiter 历史：仅 match_explanation，禁止列表专用 candidate_summary
    expect(路径们[2].match(/include=match_explanation/g)).toHaveLength(1);
    expect(路径们[2]).not.toContain('candidate_summary');
    // candidate open 两页：无任何 include（底层 Case 列表不增加评分展开）
    for (const 路径 of [候选过滤Open路径, 候选Open路径]) {
      expect(路径).not.toContain('include=');
    }
  });

  it('recruiter open 每项缺 candidate_summary / 缺 match_explanation 拒绝，两键显式 null 成功，坏解释拒绝', async () => {
    ...
    const { match_explanation: _缺解释, ...缺解释 } = 招聘展开工作区项;
    请求Mock.mockResolvedValueOnce(响应({ items: [缺解释], next_cursor: null }));
    await expect(source.读取P5Open列表('recruiter', 职位ID, null))
      .rejects.toMatchObject({ code: 'invalid_response' });

    请求Mock.mockResolvedValueOnce(响应({
      items: [{ ...招聘展开工作区项, candidate_summary: null, match_explanation: null }],
      next_cursor: null,
    }));
    await expect(source.读取P5Open列表('recruiter', 职位ID, null))
      .resolves.toMatchObject({ items: [{ candidateSummary: null, 匹配解释: null }] });

    // 解释总分与同响应 match_score 不一致：fail closed（87 分卡配 92 分解释）
    请求Mock.mockResolvedValueOnce(响应({
      items: [{ ...招聘展开工作区项, match_explanation: BFF匹配解释92分样本 }],
      next_cursor: null,
    }));
    await expect(source.读取P5Open列表('recruiter', 职位ID, null))
      .rejects.toMatchObject({ code: 'invalid_response' });
  ...
  it('招聘历史行展开：缺 match_explanation 拒绝、显式 null 合法、candidate open 行携带解释键即拒绝', async () => {
    // 真实缺键（键不在场）：历史查询带了 include，响应缺键即契约漂移
    const { match_explanation: _缺解释, ...缺解释 } = 招聘展开历史行({ state: P5已完成状态Wire });
    请求Mock.mockResolvedValueOnce(响应({ items: [缺解释], next_cursor: null }));
    await expect(source.读取P5历史('recruiter', 'completed', 职位ID, null))
      .rejects.toMatchObject({ code: 'invalid_response' });

    请求Mock.mockResolvedValueOnce(响应({ items: [招聘展开历史行({ state: P5已完成状态Wire, match_explanation: null })], next_cursor: null }));
    const null页 = await source.读取P5历史('recruiter', 'completed', 职位ID, null);
    expect(null页.items[0]).toMatchObject({ 匹配解释: null });
    expect('candidateSummary' in null页.items[0]).toBe(false);

    // candidate open 行是默认合同：携带 match_explanation 键即契约漂移（C1 不自动猜模式）
    请求Mock.mockResolvedValueOnce(响应({ items: [{ ...P5候选工作区项Wire, match_explanation: BFF匹配解释92分样本 }], next_cursor: null }));
    await expect(source.读取P5Open列表('candidate', 意向ID, null))
      .rejects.toMatchObject({ code: 'invalid_response' });
  });

  it('解P5详情解码模式显式传递：展开缺键失败、null 成功、默认带键失败（外层与嵌套同参数）', () => {
    // 展开模式：解释对象与同响应 match_score（92）同分 → 完整解码进 匹配解释
    const 展开 = { ...P5候选详情Wire, match_explanation: BFF匹配解释92分样本 };
    expect(解P5详情(展开, 'candidate', true).匹配解释).toEqual(BFF匹配解释92分样本);
    // 展开模式：显式 null 是合法无溯源档
    expect(解P5详情({ ...P5候选详情Wire, match_explanation: null }, 'candidate', true).匹配解释).toBeNull();
    // 展开模式：缺键即契约漂移（请求带了 include 响应缺键）
    expect(() => 解P5详情(P5候选详情Wire, 'candidate', true)).toThrow(契约漂移);
    // 展开模式：解释与 match_score 异分拒绝（解匹配解释 的 C1 一致性校验）
    expect(() => 解P5详情({ ...P5候选详情Wire, match_explanation: BFF匹配解释87分样本 }, 'candidate', true))
      .toThrow();
    // 默认模式：携带展开键即契约漂移；未带键合法且不产出 匹配解释 字段
    expect(() => 解P5详情(展开, 'candidate')).toThrow(契约漂移);
    const 默认 = 解P5详情(P5候选详情Wire, 'candidate');
    expect('匹配解释' in 默认).toBe(false);
  });
  ...
  it('候选完整记录与招聘空总结按角色严格解码', () => {
    ...
    expect(候选.stages[0].screeningRecords?.messages).toEqual([
      ...
      { id: 's0a_1', kind: 'answer', role: 'recruiter',
        ...
        exchangeRef: null, occurredAt: '2026-08-23T10:02:00Z' },
    ]);
  ...
  it('only-question、unknown／declined／not_available、空 messages＋initial、两数组空与轮次空档都按原样解码', () => {
    ...
    expect(未知.stages[0].screeningRecords?.messages[1]).toEqual({
      id: 's0a_1', kind: 'answer', role: 'recruiter',
      stage: 'anonymous_screening', askingRole: 'candidate', round: 1,
      answerStatus: 'unknown', answerSource: 'agent', exchangeRef: null,
    ...
    for (const 回答状态 of ['declined', 'not_available'] as const) {
      ...
      expect(详情.stages[0].screeningRecords?.messages[1]).toEqual({
        id: 's0a_1', kind: 'answer', role: 'recruiter',
        stage: 'anonymous_screening', askingRole: 'candidate', round: 1,
        answerStatus: 回答状态, answerSource: 'agent', exchangeRef: null,
        occurredAt: '2026-08-23T10:02:00Z',
      });
    }
  ...
  it('详情路径编码 case ID 且 include 精确一次并保持 no-store', async () => {
    请求Mock.mockResolvedValueOnce(响应({
      ...带S0记录(P5候选详情Wire, S0候选完整记录Wire),
      match_explanation: BFF匹配解释92分样本,
    }));
    await source.读取P5详情('candidate', 'mc/一?');
    expect(请求Mock).toHaveBeenCalledWith({
      path: '/api/v1/me/match-cases/mc%2F%E4%B8%80%3F?include=screening_records,match_explanation',
      不缓存: true,
    });
  });
  ...
  it('招聘列表行与历史行解出 match_score/candidate_identity/匹配解释；0 分、null 评分与匿名身份合法', async () => {
    请求Mock
      .mockResolvedValueOnce(响应({ items: [招聘展开工作区项], next_cursor: null }))
      .mockResolvedValueOnce(响应({
        items: [招聘展开历史行({ state: P5已完成状态Wire, match_score: null, match_explanation: null })],
        next_cursor: null,
      }))
      .mockResolvedValueOnce(响应({
        items: [{ ...招聘展开工作区项, match_score: 0, match_explanation: null }],
        next_cursor: null,
      }));
    const 首页 = await source.读取P5Open列表('recruiter', 职位ID, null);
    expect(首页.items[0]).toMatchObject({
      matchScore: 87, candidateIdentity: BFF候选身份匿名样本, 匹配解释: BFF匹配解释87分样本,
    });
    const 历史页 = await source.读取P5历史('recruiter', 'completed', 职位ID, null);
    expect(历史页.items[0]).toMatchObject({ matchScore: null, candidateIdentity: BFF候选身份匿名样本, 匹配解释: null });
    const 零分页 = await source.读取P5Open列表('recruiter', null, null);
    expect(零分页.items[0]).toMatchObject({ matchScore: 0, 匹配解释: null });
    expect(请求Mock).toHaveBeenCalledTimes(3);
  });

  it('招聘列表/历史行缺 match_score 或 candidate_identity 漂移；候选行携带招聘私有键漂移', async () => {
    const { match_score: _评分, ...缺评分 } = { ...P5招聘工作区项Wire, match_explanation: BFF匹配解释87分样本 };
    const { candidate_identity: _身份, ...缺身份 } = { ...P5招聘工作区项Wire, match_explanation: BFF匹配解释87分样本 };
  ...
  it('招聘列表行 candidate_identity 嵌套漂移按协议错误码拒绝', async () => {
    请求Mock.mockResolvedValueOnce(响应({
      items: [{ ...招聘展开工作区项, candidate_identity: { state: 'anonymous', name: '夹带', avatar_url: null, disclosed_at: null } }],
      next_cursor: null,
    }));
    await expect(source.读取P5Open列表('recruiter', 职位ID, null))
      .rejects.toMatchObject({ code: 'invalid_response' });
  ...
  it('详情 GET 走角色前缀的 case 路径、恒带唯一 include（screening_records,match_explanation）并解码 role 详情', async () => {
    请求Mock
      .mockResolvedValueOnce(响应({ ...P5候选详情Wire, match_explanation: BFF匹配解释92分样本 }))
      .mockResolvedValueOnce(响应({ ...P5招聘详情Wire, match_explanation: BFF匹配解释87分样本 }));
    await expect(source.读取P5详情('candidate', 'mc_1'))
      .resolves.toMatchObject({ role: 'candidate', context: { intentionId: 意向ID }, 匹配解释: BFF匹配解释92分样本 });
    await expect(source.读取P5详情('recruiter', 'mc_2'))
      .resolves.toMatchObject({ role: 'recruiter', state: { caseId: 'mc_1' }, 匹配解释: BFF匹配解释87分样本 });
    expect(请求Mock.mock.calls.map(([选项]) => 选项)).toEqual([
      { path: '/api/v1/me/match-cases/mc_1?include=screening_records,match_explanation', 不缓存: true },
      { path: '/api/v1/recruiter/match-cases/mc_2?include=screening_records,match_explanation', 不缓存: true },
    ]);
  });
...
const 交换ID2 = 'cex_abcdef0123456789abcdef0123456789';
/** 「键缺席」的哨兵：与显式 null 区分开（显式 null 是漂移，键缺席归一 null）。 */
const 缺席标记 = Symbol('缺席');
...
describe('MatchCase 连续筛选块（continuity_version 2）解码', () => {
  ...
  it('记录 exchange_ref：只属 S1/S2 的问答，S0 记录携带即漂移，空串/坏格式拒绝', () => {
    ...
    // 空串与坏格式拒绝（question 引用与 answer 引用统一满足 cex 32 位十六进制）
    expect(() => 解P5详情(S2块({ exchange_ref: '' }), 'candidate')).toThrow(契约漂移);
    expect(() => 解P5详情(S2块({ exchange_ref: 'cex_short' }), 'candidate')).toThrow(契约漂移);
    expect(() => 解P5详情(S2块({ exchange_ref: 'cdx_0123456789abcdef0123456789abcdef' }), 'candidate'))
      .toThrow(契约漂移);
    expect(() => 解P5详情(S2块({ exchange_ref: null }), 'candidate')).toThrow(契约漂移);
    // S0 的问题不会有待办可指
    expect(() => 解P5详情(
      带S0记录(P5候选详情Wire, S0块({ messages: [{ ...造S0消息(), exchange_ref: 交换ID }] })),
      'candidate',
    )).toThrow(契约漂移);
    // S0 的回答同样不指任何待办（引用只属 S1/S2 两个发问块）
    expect(() => 解P5详情(
  ...
  it('dialogue_progress.step：四闭词原样保留、缺席归一 null；显式 null/空串/未知词/非字符串拒绝', () => {
    const S2态 = { ...S0基态Wire, stage: 'needs_coordination', status: 'waiting', step: 'coordinating' };
    const 详情带 = (step: unknown) => {
      const 进度: Record<string, unknown> = {
        stage: 'needs_coordination', asking_role: 'recruiter',
        recruiter_round: 1, candidate_round: 0, round_budget: 2,
      };
      if (step !== 缺席标记) 进度.step = step;
      return 解P5详情(v2详情({ state: S2态, dialogue_progress: 进度 }), 'candidate');
    };
    // 表驱动：四个合法词原样保留；整个块缺席时归一为 null（不猜动作）
    for (const 词 of ['assessing', 'answering', 'awaiting_human', 'complete'] as const) {
      expect(详情带(词).dialogueProgress).toMatchObject({ step: 词 });
    }
    expect(详情带(缺席标记).dialogueProgress).toMatchObject({ step: null });
    // 反例：显式 null、空串、未知词、大小写不符、非字符串
    const 反例们: readonly [string, unknown][] = [
      ['显式null', null], ['空串', ''], ['未知词', 'reassessing'], ['大小写', 'Assessing'], ['数字', 42],
    ];
    for (const [标记, 坏值] of 反例们) {
      expect(() => 详情带(坏值), 标记).toThrow(契约漂移);
    }
  });

  it('S1/S2 answer 的 exchange_ref：与同块同侧同轮 question 的引用完全相等才配对；缺引用兼容旧响应', () => {
    // S1：招聘方发问、候选 Agent 作答；S2：招聘方发问、候选本人补答（无正文 unknown 也可带引用）
    const 配对 = 解P5详情(v2详情({
      ...带S0记录(P5候选详情Wire, {
        messages: [
          {
            ...v2问题({
              id: 'q_s1', stage: 'resume_submission', role: 'recruiter', asking_role: 'recruiter',
              text: '请说明你的职责。',
            }),
            exchange_ref: 交换ID,
          },
          {
            ...v2回答({
              id: 'a_s1', stage: 'resume_submission', role: 'candidate', asking_role: 'recruiter',
              text: '我负责产品设计。',
            }),
            exchange_ref: 交换ID,
          },
          {
            ...v2问题({
              id: 'q_s2', stage: 'needs_coordination', role: 'recruiter', asking_role: 'recruiter',
              text: '远程比例最多多少？',
            }),
            exchange_ref: 交换ID2,
          },
          略S0键({
            ...v2回答({
              id: 'a_s2', stage: 'needs_coordination', role: 'candidate', asking_role: 'recruiter',
              answer_source: 'agent', answer_status: 'unknown',
            }),
            exchange_ref: 交换ID2,
          }, 'text'),
        ],
    ...
    const 消息 = 配对.stages[0].screeningRecords?.messages ?? [];
    expect(消息.map((条) => 条.exchangeRef)).toEqual([交换ID, 交换ID, 交换ID2, 交换ID2]);
    // question 有引用、answer 缺引用合法（旧响应），两者都缺也合法
    const 旧响应 = 解P5详情(v2详情({
      ...带S0记录(P5候选详情Wire, {
        messages: [
          { ...v2问题({ id: 'q_1', stage: 'resume_submission', role: 'recruiter', asking_role: 'recruiter' }), exchange_ref: 交换ID },
          v2回答({ id: 'a_1', stage: 'resume_submission', role: 'candidate', asking_role: 'recruiter' }),
          v2问题({ id: 'q_2', stage: 'resume_submission', role: 'recruiter', asking_role: 'recruiter', round: 2 }),
          v2回答({ id: 'a_2', stage: 'resume_submission', role: 'candidate', asking_role: 'recruiter', round: 2 }),
        ],
        summaries: [],
      }),
      continuity_version: 2, pending_actions: [], dialogue_progress: null,
    ...
    const 旧消息 = 旧响应.stages[0].screeningRecords?.messages ?? [];
    expect(旧消息.map((条) => 条.exchangeRef)).toEqual([交换ID, null, null, null]);
  });

  it('answer 引用反例：引用不等、跨轮/跨侧/跨阶段借引用、坏格式与无前置问题都拒绝', () => {
    const 解块 = (messages: Record<string, unknown>[]) => 解P5详情(v2详情({
      ...带S0记录(P5候选详情Wire, { messages, summaries: [] }),
      continuity_version: 2, pending_actions: [], dialogue_progress: null,
      reconsideration: null, confirmation_summary: null,
      needs_action: false, available_actions: [],
    }), 'candidate');
    const S1问 = (覆盖: Record<string, unknown> = {}) => ({
      ...v2问题({
        id: 'q_s1', stage: 'resume_submission', role: 'recruiter', asking_role: 'recruiter',
      }),
      ...覆盖,
    });
    const S1答 = (覆盖: Record<string, unknown> = {}) => ({
      ...v2回答({
        id: 'a_s1', stage: 'resume_submission', role: 'candidate', asking_role: 'recruiter',
      }),
      ...覆盖,
    });
    const S2问 = (覆盖: Record<string, unknown> = {}) => ({
      ...v2问题({
        id: 'q_s2', stage: 'needs_coordination', role: 'recruiter', asking_role: 'recruiter',
      }),
      ...覆盖,
    });
    // 引用不等：question 带引用、answer 带另一个引用
    expect(() => 解块([
      S1问({ exchange_ref: 交换ID }),
      S1答({ exchange_ref: 交换ID2 }),
    ])).toThrow(契约漂移);
    // question 缺引用而 answer 带引用：无处配对
    expect(() => 解块([S1问(), S1答({ exchange_ref: 交换ID })])).toThrow(契约漂移);
    // 跨轮：同块第 2 轮的 answer 拿第 1 轮的引用（即便第 2 轮也有自己的问题）
    expect(() => 解块([
      S1问({ id: 'q_r1', round: 1, exchange_ref: 交换ID }),
      S1答({ id: 'a_r1', round: 1, exchange_ref: 交换ID }),
      S1问({ id: 'q_r2', round: 2, exchange_ref: 交换ID2 }),
      S1答({ id: 'a_r2', round: 2, exchange_ref: 交换ID }),
    ])).toThrow(契约漂移);
    // 跨侧：S2 招聘方问、候选方答的引用，拿到另一侧（asking_role=candidate）的 answer 上
    expect(() => 解块([
      S2问({ exchange_ref: 交换ID }),
      {
        ...v2回答({
          id: 'a_wrong_side', stage: 'needs_coordination', role: 'recruiter',
          asking_role: 'candidate',
        }),
        exchange_ref: 交换ID,
      },
    ])).toThrow(契约漂移);
    // 跨阶段：S1 的引用拿到 S2 的 answer 上（同侧同轮也不行）
    expect(() => 解块([
      S1问({ exchange_ref: 交换ID }),
      S1答({ exchange_ref: 交换ID }),
      S2问(),
      {
        ...v2回答({
          id: 'a_cross_stage', stage: 'needs_coordination', role: 'candidate', asking_role: 'recruiter',
        }),
        exchange_ref: 交换ID,
      },
    ])).toThrow(契约漂移);
    // 坏格式与显式 null
    expect(() => 解块([S1问({ exchange_ref: 交换ID }), S1答({ exchange_ref: 'cex_short' })]))
      .toThrow(契约漂移);
    expect(() => 解块([S1问({ exchange_ref: 交换ID }), S1答({ exchange_ref: null })]))
      .toThrow(契约漂移);
    // 无前置问题（同块同轮根本没有 question）
    expect(() => 解块([S1答({ exchange_ref: 交换ID })])).toThrow(契约漂移);
  });

  it('完整包：dialogue_progress.step 与 S1/S2 问答引用同时在场，独立 Case 整包解码', () => {
    const S2态 = { ...S0基态Wire, stage: 'needs_coordination', status: 'waiting', step: 'coordinating' };
    const 详情 = 解P5详情(v2详情({
      state: S2态,
      ...带S0记录(P5候选详情Wire, {
        messages: [
          { ...v2问题({ id: 'q_s0' }) },
          { ...v2回答({ id: 'a_s0' }) },
          { ...v2问题({ id: 'q_s1', stage: 'resume_submission', role: 'recruiter', asking_role: 'recruiter' }), exchange_ref: 交换ID },
          { ...v2回答({ id: 'a_s1', stage: 'resume_submission', role: 'candidate', asking_role: 'recruiter' }), exchange_ref: 交换ID },
          { ...v2问题({ id: 'q_s2', stage: 'needs_coordination', role: 'recruiter', asking_role: 'recruiter' }), exchange_ref: 交换ID2 },
          { ...v2回答({ id: 'a_s2', stage: 'needs_coordination', role: 'candidate', asking_role: 'recruiter' }), exchange_ref: 交换ID2 },
        ],
        summaries: [],
      }),
      pending_actions: [],
      dialogue_progress: { ...P5对话进度Wire, recruiter_round: 2 },
      reconsideration: null, confirmation_summary: null,
      needs_action: false, available_actions: [],
    }), 'candidate');
    expect(详情.dialogueProgress).toMatchObject({ stage: 'needs_coordination', step: 'assessing' });
    expect(详情.stages[0].screeningRecords?.messages.map((条) => 条.id)).toEqual([
      'q_s0', 'a_s0', 'q_s1', 'a_s1', 'q_s2', 'a_s2',
    ]);
  });
});
