...
describe('HTTP 招聘数据源', () => {
  ...
  it('保存简历按快照 diff 写 singleton/entries 后重新 GET', async () => {
    ...
    const 新页面 = {
      ...
      技能: [...旧页面.技能, 'React'],
      经历: [...旧页面.经历, { ...旧页面.经历[0], 编号: 'local-new', 组织编号: 'org_xin', 公司: '新公司' }],
    };
  ...
  it('新建带项目的经历会 POST 经历后再 POST 项目到新经历下', async () => {
    ...
    const 新页面 = {
      ...旧页面,
      经历: [...旧页面.经历, {
        编号: 'local-new', 组织编号: 'org_xin', 公司: '新公司', 行业: '互联网', 行业引用: { id: 'tax_i', display_name: '互联网' }, 职位: '工程师', 开始: '2022-01', 结束: null, 内容: '', 隐藏: false,
  ...
  // Task 1（J-PILOT-01）：第十七个域 facade（候选 me/negotiations 连续代谈）一并组合进根 facade。
  // Task 1（求职端助手聊天）：第十九个域 facade（me/assistant 助手会话）一并组合进根 facade。
  it('根 facade 组合全部域且不丢公开方法', () => {
    ...
    expect(Object.keys(source).sort()).toEqual([
      ...
      '决定P5S0', '决定P5S1', '决定P5S2', '决定P5S3', '回答P5对话', '重新考虑P5',
      '新增P5叮嘱', '读取P5简历PDF',
      // P7：真人会话域
      ...
      // 求职端助手会话域（me/assistant 聊天）
      '读取助手历史', '发送助手消息', '读取助手轮次', '重试助手轮次',
    ].sort());
