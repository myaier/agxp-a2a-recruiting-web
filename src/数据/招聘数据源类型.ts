...
export interface 页面简历快照 {
  ...
  教育: 简历教育段[];
  证书: 简历证书[];
  /** J-PILOT-02：读侧必返；旧 BFF 响应缺 portfolio_url 字段按 null 回显（显示空串）。 */
  作品集链接: string | null;
...
/**
 * J-PILOT-02：写侧 URL 三态 —— 属性缺省（含 undefined）= 未改保留服务端值、
 * null = 清空、string = 规范化后设置。用“有无属性”区分省略与 null，
 * 不以 truthy 判断清空；普通资料编辑不带该字段，有意编辑才携带。
 */
export type 页面简历写入 = Omit<页面简历快照, '服务端快照' | '作品集链接'> & {
  作品集链接?: string | null;
};
...
export interface 首次意向输入 {
  ...
  筛选偏好: 求职初筛偏好;
  /** 仅四张固定偏好卡的原标签（大小周 / 纯外包 / 乙方 / 全现场办公 / 频繁出差）。
   *  按 Plan Global 3，映射只把勾选卡片换算成私有诉求行、不写硬排除；自定义文本的
   *  合同载体是 自定义诉求（由 Task 7 把向导 UI 接过来）。Task 7 接线完成前，
   *  引导问答 的排除题 UI 仍会把用户自定义文本混进本字段 —— 那段文本会在映射中被
   *  静默丢弃（既非卡片、也不在 自定义诉求 里）；接线前的调用方不得在本字段传
   *  自定义文本。 */
  排除项: string[];
  /** 用户自定义排除原文，逐字另存进 private_preferences（全空白段不产生诉求行）；
   *  Plan Global 3 规定的自定义文本唯一载体，Task 7 完成向导 UI 接线。 */
  自定义诉求?: string[];
