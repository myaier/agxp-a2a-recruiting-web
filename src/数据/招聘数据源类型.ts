...
export interface 首次意向输入 {
   ...
   *  按 Plan Global 3，映射只把勾选卡片换算成私有诉求行、不写硬排除。
   *  Task 7 起 引导问答 的排除题已按载体分流：只有点中固定卡才进本字段，
   *  用户自己写的文本（哪怕与卡片同名）一律进 自定义诉求，不再有文本被静默丢弃。 */
  排除项: string[];
  /** 用户自定义排除原文，逐字另存进 private_preferences（全空白段不产生诉求行）；
   *  Plan Global 3 规定的自定义文本唯一载体，由 引导问答 排除题 的行内输入填充。 */
  自定义诉求?: string[];
