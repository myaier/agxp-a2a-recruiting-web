...
/** 年薪月数快捷片：点一下选中，省得手机上打字 */
const 年薪月数选项 = ['13 薪', '14 薪', '15 薪', '16 薪'];

/** 办公方式快捷片 */
const 办公方式选项 = ['现场', '混合 3+2', '全远程'];
...
export default function 发布岗位() {
  ...
  const [年薪月数, 设年薪月数] = useState('15 薪');
  ...
  return (
    <滚动区 样式覆盖={{ paddingBottom: 12 }}>
      ...
      <div className={样式.表单区}>
        ...
        <div className={样式.编辑条目}>
          <div className={样式.条目标签}>年薪月数</div>
          <div className={样式.快捷片行}>
            {年薪月数选项.map((项) => (
              <button
                key={项}
                className={`${样式.快捷片} ${年薪月数 === 项 ? 样式.快捷片选中 : ''} 可点`}
                onClick={() => 设年薪月数(项)}
              >
                {项}
              </button>
