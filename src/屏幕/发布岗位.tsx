  ...
  return (
    <次级页外壳>
      <div className={样式.发布壳}>
        ...
        {待删 && 编辑目标 ? (
          <div className={样式.删遮罩} onClick={() => 设待删(false)}>
            <div className={样式.删确认框} onClick={(事件) => 事件.stopPropagation()}>
...
function 职位描述步({
  ...
  return (
    <滚动区 样式覆盖={{ paddingBottom: 12 }}>
      ...
      {/* 职位关键词：词池多选，落 在招岗位.职位关键词，岗位详情的「职位详情」区按 chips 展示 */}
      <div className={样式.表单区}>
        <div className={样式.编辑条目}>
          <div className={样式.条目标签}>职位关键词</div>
          <div className={样式.快捷片行}>
            {职位关键词池.map((词) => (
              <button
                key={词}
