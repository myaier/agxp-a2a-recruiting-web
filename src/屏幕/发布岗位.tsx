    ...
    <次级页外壳>
      <div className={样式.发布壳}>
        ...
        {编辑态 ? (
          ...
          /* 新建态：分段进度 + 步名（对照 BOSS 的「①填写基础信息 —— ②选择职位要求」，
             但保持我们的细轨风格，步名小字挂在轨下） */
          <div>
            ...
            <div className={样式.步名行}>
              {步骤顺序.map((步, 序) => (
                <span key={步} className={`${样式.步名} ${序 === 第几步 ? 样式.步名当前 : ''}`}>
                  {步}
                </span>
              ))}
            </div>
...
function 职位要求步({
  ...
  return (
    <滚动区 样式覆盖={{ paddingBottom: 12 }}>
      <页面大标题
        标题="选择职位要求"
        说明="经验、学历与薪资会进入 AI 代理的核对口径，薪资只作区间披露。"
      />
      ...
      <div className={样式.表单区}>
        ...
        {按年限筛(当前类型) ? (
          ...
          <div className={样式.适配注}>
            {当前类型 === '校园招聘' ? '校园招聘不按年限筛，经验要求已收起。' : '实习岗不按年限筛，经验要求已收起。'}
          </div>
        )}
      ...
      <div className={样式.要求文标}>职位要求（写给人看）</div>
      <div className={样式.描述卡}>
        <textarea
          className={样式.描述输入}
          value={文本}
