...
/** 职位类别（BOSS 的「职位类型」对照）。发布后不可改，用于匹配与频道归类 */
const 职位类别选项 = [
  '后端开发',
  '前端开发',
  '算法',
...
export default function 发布岗位() {
  ...
  const 组装岗位 = (编号: string, 底: 在招岗位 | null): 在招岗位 => ({
    ...
    职位描述: 职位描述.trim(),
    职位要求: 职位要求.trim(),
    // 合同 = 结构化档位（学历在前、经验在后）+ 手动条件。「不限」不产生合同条
    硬性条件: [
      ...(最低学历 !== '不限' ? [`${最低学历}及以上`] : []),
      ...(经验要求 !== '不限' ? [`${经验要求}经验`] : []),
      ...硬性条件,
...
// ── D0 第一步：基础信息（招聘类型 / 名称 / 类别 / 城市 / 办公地 / 薪资带 / 月数 / 办公方式）──
function 基础信息步({
  编辑态,
  岗位名称,
  设岗位名称,
  ...
  return (
    <滚动区 样式覆盖={{ paddingBottom: 12 }}>
      ...
      <div className={样式.表单区}>
        {/* 招聘类型：BOSS 的「社招全职 / 应届校园招聘 / 实习生招聘」，加上我们的兼职 */}
        <div className={样式.编辑条目}>
          <div className={样式.条目标签}>招聘类型{编辑态 ? <span className={样式.锁标}>发布后不可改</span> : null}</div>
          <div className={样式.快捷片行}>
            {招聘类型选项.map((项) => (
              <button
                key={项}
                className={`${样式.快捷片} ${当前招聘类型 === 项 ? 样式.快捷片选中 : ''} 可点`}
                onClick={() => (编辑态 ? 提示不可改() : 设招聘类型(项))}
              >
                {项}
              </button>
            ))}
        ...
        <div className={样式.编辑条目}>
          <div className={样式.条目标签}>岗位名称{编辑态 ? <span className={样式.锁标}>发布后不可改</span> : null}</div>
          <input
            className={样式.条目输入}
            value={岗位名称}
        ...
        {/* 职位类别：匹配与频道归类用（BOSS 的「职位类型」）*/}
        <div className={样式.编辑条目}>
          <div className={样式.条目标签}>职位类别{编辑态 ? <span className={样式.锁标}>发布后不可改</span> : null}</div>
          <div className={样式.快捷片行}>
            {职位类别选项.map((项) => (
              <button
                key={项}
                className={`${样式.快捷片} ${当前职位类别 === 项 ? 样式.快捷片选中 : ''} 可点`}
                onClick={() => (编辑态 ? 提示不可改() : 设职位类别(项))}
              >
                {项}
              </button>
            ))}
        ...
        <div className={样式.编辑条目}>
          <div className={样式.条目标签}>工作城市{编辑态 ? <span className={样式.锁标}>发布后不可改</span> : null}</div>
          <input
            className={样式.条目输入}
            value={工作城市}
            placeholder="如：上海"
...
// ── D0c 第三步：职位要求 + 经验/学历档位 + 硬性条件（可增可删）──
function 职位要求步({
  编辑态,
  文本,
  设文本,
  ...
  const { 跳转 } = use导航();
  // 档位生成的合同条（学历在前、经验在后），与手动条件一起给用户一个「所见即合同」的预览
  const 档位条件 = [
    ...(当前学历 !== '不限' ? [`${当前学历}及以上`] : []),
    ...(当前经验 !== '不限' ? [`${当前经验}经验`] : []),
  ];
  ...
  return (
    <滚动区 样式覆盖={{ paddingBottom: 12 }}>
      ...
      {/* 经验 / 最低学历：结构化档位（BOSS 截图 5 的两项），选中即并入下方合同 */}
      <div className={样式.硬性区}>
        ...
        <div className={`${样式.条目标签} ${样式.档位间距}`}>最低学历</div>
        <div className={样式.快捷片行}>
          {最低学历选项.map((项) => (
            <button
              key={项}
          ...
          <div className={样式.已选片行}>
            {档位条件.map((项) => (
              <button
                ...
                onClick={() => 轻提示('这一条来自上方的经验/学历档位，在那里修改')}
              >
                {项}
              </button>
            ))}
      ...
      {/* 发布即同意（BOSS 截图 5 底部的合规行）：编辑态不重复展示 */}
      {!编辑态 ? (
