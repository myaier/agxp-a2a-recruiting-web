...
import { use城市默认页, use城市搜索, 按行政区分组, 本地城市搜索结果, type 查询Location方法 } from './城市查询钩子';
import { Mock默认城市字典 } from '../数据/城市与行业';
...
function 岗位编辑表单({ 路由岗位编号 }: { 路由岗位编号?: string }) {
  ...
  /** 正文保存：一次性原子回填显示名与 地点引用（键 = 目录稳定 ID / Mock 城名，
   *  不从名称重建）；关闭子视图，本次临时选择与查询随之销毁 */
  const 保存城市选择 = (已选: 城市项[]) => {
    const 选 = 已选[0];
    if (选) {
      设工作城市(选.名称);
      if (是后端) 设地点引用({ id: 选.键, display_name: 选.名称 });
    }
    设城市子视图(false);
  };
    ...
    <次级页外壳 白底>
      ...
      {城市子视图 ? (
        <岗位城市选择层
          ...
          初始已选={
            是后端
              ? (地点引用 ? [{ 键: 地点引用.id, 名称: 地点引用.display_name }] : [])
              : (工作城市.trim() !== '' ? [{ 键: 工作城市, 名称: 工作城市 }] : [])
          }
          关闭={() => 设城市子视图(false)}
          保存={保存城市选择}
        />
  ...
  const 列表: 城市列表状态 = 搜索态
    ? {
        组: [{
          ...
          名称: '搜 索 结 果',
          城市: 查询Location
            ? 结果.map((项) => ({ 键: 项.id, 名称: 项.display_name }))
