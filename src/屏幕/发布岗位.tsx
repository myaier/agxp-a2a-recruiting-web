...
// ── Task 5：四问硬性事实 ──
// 服务端 Owner Job 的 hard_requirements 四员投影。键是页面字段名，第二个元素沿用
// 产品经理定下的现有条件词（纯外包 / 乙方 中间带空格斜杠），本任务不得改写。
const 硬性事实选项 = [
  ['大小周', '大小周'],
  ['纯外包乙方', '纯外包 / 乙方'],
  ['全现场办公', '全现场办公'],
...
/** 点按循环：未说明 → 必须 → 不要求 → 回到未说明。「未说明」≠「不要求」，不许两档合并 */
const 下一档 = { 未说明: '必须', 必须: '不要求', 不要求: '未说明' } as const;
/** 三档的可见符号：中性横线 / 要求对勾 / 明确不要的叉 —— 全部是符号，不加解释文案 */
...
export default function 发布岗位() {
  ...
  /** Task 5：点一枚事实钮换到下一档；一次只动这一问，其余三问原样保留 */
  const 切硬性事实 = (键: keyof 岗位硬性事实) => {
    设硬性事实((旧) => ({ ...旧, [键]: 下一档[旧[键]] }));
  };
  ...
  return (
    <滚动区 样式覆盖={{ paddingBottom: 12 }}>
      ...
      <div className={样式.硬性区}>
        {/* Task 5：四问硬性事实。节名沿用 PM 原文「硬性条件」，钮上只有条件词和 —/✓/×，
            点按在 未说明→必须→不要求→未说明 间循环；「未说明」是服务端还没说，
            绝不与「不要求」合并。存量手动条在下方原样展示，不在这块里改写。 */}
        ...
        <div className={样式.硬性事实行}>
          {硬性事实选项.map(([键, 标签]) => {
            const 档 = 硬性事实[键];
            return (
              <button
                key={键}
                className={`${样式.硬性事实片} ${档 === '必须' ? 样式.硬性事实必须 : ''} ${档 === '不要求' ? 样式.硬性事实不要求 : ''} 可点`}
                aria-label={`${标签} ${档}`}
                onClick={() => 切硬性事实(键)}
              >
                {标签}
                <span className={样式.档符号}>{档符号[档]}</span>
              </button>
            );
          })}
        </div>
