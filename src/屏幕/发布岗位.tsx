...
import { 今天日期 as 当前日期文本, 校验未过期日期 } from '../流程/onboarding配置';
...
type 招聘紧急度 = NonNullable<在招岗位['招聘紧急度']>;
...
const 招聘紧急度选项: 招聘紧急度[] = ['尽快到岗', '两周内', '一个月内', '持续招聘'];
...
export default function 发布岗位() {
  ...
  const [实习最晚开始日期, 设实习最晚开始日期] = useState(编辑目标?.实习最晚开始日期 ?? '');
  ...
  const [面试轮次, 设面试轮次] = useState<number | null>(编辑目标?.面试轮次 ?? null);
  const [面试轮开, 设面试轮开] = useState(false);
  const [招聘紧急度, 设招聘紧急度] = useState<招聘紧急度 | ''>(编辑目标?.招聘紧急度 ?? '');
  ...
  const 第一步缺失 = (): { 文案: string; 步骤: number } | null => {
    ...
    if (招聘类型 === '实习生') {
      if (实习转正 === null) return { 文案: '请确认是否提供转正机会', 步骤: 0 };
      const 日期错误 = 校验未过期日期(实习最晚开始日期, '最晚可接受实习开始日期');
      if (日期错误) return { 文案: 日期错误, 步骤: 0 };
    }
    return null;
  };
  ...
  const 岗位信息缺失 = (): { 文案: string; 步骤: number } | null => {
    ...
    if (面试轮次 === null) return { 文案: '请确认面试轮次', 步骤: 2 };
    if (招聘紧急度 === '') return { 文案: '请选择招聘紧急度', 步骤: 2 };
    return null;
  };
      ...
      <div className={样式.表单区}>
        ...
        {当前招聘类型 === '实习生' ? (
          <div className={样式.编辑条目}>
            ...
            <div className={`${样式.条目标签} ${样式.实习日期标签}`}>最晚可接受实习开始日期</div>
            <input
              className={样式.日期输入}
              type="date"
              min={当前日期文本()}
              required
              value={实习最晚开始日期}
              aria-label="最晚可接受实习开始日期"
              aria-invalid={Boolean(实习日期错误)}
              onChange={(事件) => 设实习最晚开始日期(事件.target.value)}
            />
        ...
        >
          ...
          <span className={样式.选择条目值行}>
            <span className={样式.条目值}>
              {当前职位类别 ? `${查大类(当前职位类别)} · ${当前职位类别}` : '请选择职位类别'}
            </span>
...
function 职位要求步({
  ...
  const 初筛合同 = [
    ...
    ...(当前类型 === '实习生'
      ? [
          `实习：至少 ${实习月数} 个月 · 每周 ${每周天数} 天`,
          `开始日期：最晚 ${实习最晚开始日期 || '待填写'}`,
        ]
      : []),
    ...职位关键词.map((词) => `技能：${词}`),
    '薪资：只判断区间是否匹配',
  ...
  return (
    <滚动区 样式覆盖={{ paddingBottom: 12 }}>
      ...
      <div className={样式.表单区}>
        ...
        <button className={`${样式.选择条目} 可点`} onClick={() => 设面试轮开(true)}>
          <span className={样式.条目标签}>预计面试轮次</span>
          <span className={样式.选择条目值行}>
            <span className={`${样式.条目值} 等宽数字`}>{面试轮次 === null ? '请选择' : `${面试轮次} 轮`}</span>
            <span className={样式.尖括号}>›</span>
          </span>
        </button>

        <div className={样式.编辑条目}>
          <div className={样式.条目标签}>招聘紧急程度</div>
          <div className={样式.快捷片行}>
            {招聘紧急度选项.map((项) => (
              <button
                key={项}
                className={`${样式.快捷片} ${招聘紧急度 === 项 ? 样式.快捷片选中 : ''} 可点`}
                onClick={() => 设招聘紧急度(项)}
                aria-pressed={招聘紧急度 === 项}
              >
                {项}
              </button>
            ))}
