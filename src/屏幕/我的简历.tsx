...
import { 创建候选编辑来路, type 简历编辑分区 } from '../流程/候选日常编辑';
...
/** 身份 → 简历上的状态说法（Spec §6：只显示真实三态，空值未填写）。
 *  不再把 employed 翻译成「保密求职中」、也不给在校/离职包装到岗节奏 —— 那些是
 *  onboarding 到岗档位的语义，不是 profile 的持久化状态。 */
...
/**
 * 简历分区入口（合同 A URL 表，Task 2）：一个分区进它自己的列表/编辑上下文 ——
 * 条目直达按稳定条目编号定位（item=new 为新增），不存在的条目由目标屏显示已不可用。
 * 与 编辑入口 同一来源标记，保存/取消后统一回本页。
 */
function 分区入口(分区: 简历编辑分区, 条目?: string): string {
  const 查询 = new URLSearchParams({ from: 'resume', section: 分区 });
  if (条目 !== undefined) 查询.set('item', 条目);
  return `${路径.工作经历}?${查询.toString()}`;
}
 ...
 * 去处按字段路由（Task 2）：基本信息 / 求职状态进各自屏，简历内容按分区进
 * `/experience?from=resume&section=…`，不再所有缺项都指向整份聚合页。
 */
export function 检查资料完整度(input: {
  ...
  if (input.基本信息.真名.trim() === '') {
    待补全.push({ 文案: '姓名还没填写', 去处: 编辑入口(路径.基本信息) });
  }
  // M：空身份是真实的「当前状态」缺口，不算成在职
  if (input.基本信息.身份 === '') {
    待补全.push({ 文案: '当前状态还没选择', 去处: 编辑入口(路径.求职状态) });
  }
  if (input.基本信息.身份 !== '在校' &&
      折算工作年限(input.基本信息.开始工作年) === null) {
    待补全.push({ 文案: '开始工作年还没正确填写', 去处: 编辑入口(路径.基本信息) });
  }
  if (input.基本信息.身份 !== '在校' && input.经历.length === 0) {
    待补全.push({ 文案: '工作经历还没填写', 去处: 分区入口('work') });
  }
  const 缺内容 = input.经历.filter((段) => 段.内容.trim() === '').length;
  if (缺内容 > 0) {
    待补全.push({ 文案: `${缺内容} 段工作经历还没写工作内容`, 去处: 分区入口('work') });
  }
  if (input.教育.length === 0) {
    待补全.push({ 文案: '教育经历还没填写', 去处: 分区入口('education') });
  }
  if (input.技能.length === 0) {
    待补全.push({ 文案: '专业技能还没填写', 去处: 分区入口('skills') });
  }
  if (input.经历.length > 0 && input.经历.every((段) => (段.项目 ?? []).length === 0)) {
    可提升.push({ 文案: '可以补充关键项目', 去处: 分区入口('work') });
  }
  if (input.证书.length === 0) {
    可提升.push({ 文案: '如有资格证书，可以补充（选填）', 去处: 分区入口('certificates') });
  }
  return { 待补全, 可提升 };
}
...
export default function 我的简历() {
  ...
  const 诊断项 = (() => {
    ...
    if (!全局.基本信息.真名.trim() || !全局.基本信息.开始工作年) {
      项.push({ 文案: '基本信息没填全 —— 匿名初筛靠它核对年限要求', 去处: 编辑入口(路径.基本信息) });
    }
    ...
    if (无项目.length === 经历列表.length && 经历列表.length > 0) {
      项.push({ 文案: '还没填关键项目 —— 这是代理判断方向对口最直接的依据', 去处: 分区入口('work') });
    }
    if (全局.简历技能.length < 5) {
      项.push({
    ...
    if (全局.简历教育.length === 0) {
      项.push({ 文案: '教育经历还没填 —— 很多岗位把学历写进硬性条件', 去处: 分区入口('education') });
    }
    if (全局.简历证书.length === 0) {
      项.push({ 文案: '还没填资格证书（选填）—— 有的话是硬性核对的加分项', 去处: 分区入口('certificates') });
    }
    return 项;
  })();
  ...
  /**
   * 点条目/缺项进对应编辑上下文：去处由 编辑入口 / 分区入口 给出（都带日常编辑标记），
   * state 带合同 A 的来路证明 —— 编辑页保存/取消后退一格回本页（刷新/深链无证明时
   * 安全替换回本页，不盲退）。姓名与工作年限同一屏，当前状态进共用状态编辑，
   * 其余区/条目按 section（带 item 即直达该条目）进各自分区。
   */
  const 去编辑 = (去处: string) => 跳转(去处, 创建候选编辑来路('resume'));
      ...
      <滚动区>
        <div className={样式.列表}>
          {/* 基本信息：姓名在意向确认前对企业不可见，这里是本人视角所以照实显示。
              姓名与工作年限进同一屏「编辑基本信息」（Spec §5.1），不再行内即时提交；
          ...
          <div className={样式.卡}>
            ...
            <div className={样式.条目区}>
              ...
              <表单条目
                ...
                按下={() => 去编辑(编辑入口(路径.基本信息))}
              />
              {/* 最高学历是教育分区的最高一条：点它进教育分区列表（允许添加/选择条目），
                  而不是整份聚合页（Spec §5.1） */}
              <表单条目
                标签="最高学历"
                值={教育列表[0] ? `${教育列表[0].学历} · ${教育列表[0].专业}` : '未填'}
                按下={() => 去编辑(分区入口('education'))}
              />
              <表单条目
                标签="当前状态"
                值={状态文案[基本.身份]}
          ...
          {/* 工作经历：全部段落来自全局简历切片，点任意一段按稳定编号直达该条编辑器 */}
          <div className={样式.卡}>
          ...
          {/* 教育经历：与工作经历同源，点任意一条按稳定编号直达该条教育编辑器 */}
          <div className={样式.卡}>
            ...
            {教育列表.map((条, 序) => (
              <button
                ...
                onClick={() => 去编辑(分区入口('education', 条.编号))}
              >
                <span className={样式.经历主体}>
            ...
            {证书列表.length > 0 ? (
              证书列表.map((条, 序) => (
                <button
                  ...
                  onClick={() => 去编辑(分区入口('certificates', 条.编号))}
                >
                  <span className={样式.经历主体}>
              ...
              <button
                className={`${样式.标签行} 可点`}
                onClick={() => 去编辑(分区入口('certificates', 'new'))}
              >
