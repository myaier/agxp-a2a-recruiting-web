...
export default function 我的简历() {
  ...
  /**
   * 点条目进对应编辑屏：地址带日常编辑标记，state 带来路证明 —— 编辑页保存/取消后
   * 退一格回本页（刷新/深链无证明时安全替换回本页，不盲退）。姓名与工作年限是同一屏
   *（编辑基本信息），当前状态进共用状态编辑，其余区/条目进各自编辑屏。
   */
  const 去编辑 = (目标: string) => 跳转(编辑入口(目标), 创建候选编辑来路('resume'));
