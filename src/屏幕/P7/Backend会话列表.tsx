...
// 绝不本地清零。「全部」「仅会话」展示同一已加载集合；「通知」= 固定 AI 动态入口行。
// 固定 AI 入口行只在展示层组装（零快照/分页/未读写入），键 agent-entry:* 与十进制
// conversationId 空间不相交；受页签/搜索约束但不参与 P7 的错误/读入提示与加载更多。
// 搜索只过滤已加载项与该入口行，不声称搜全部。
...
export default function Backend会话列表({ 角色: role }: { 角色: P7角色 }) {
  ...
  const 行们: 会话行数据[] = [
    ...
    ...(通知页签 ? [] : 过滤后.map((条) => 从Backend消息行(条, role, () => 打开会话(条)))),
  ];
