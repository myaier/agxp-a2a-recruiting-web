        ...
        右侧={
          <button
            ...
            onClick={() => {
              ...
              // 提交的就是这份局部草稿本身（先落 state 再去重名）：数据源在证书 POST 成功后
              // 把服务端 id 回写到被提交的对象上（review-r2），失败后重试提交同一份草稿 → 按
              // 同一资源判定。若每次另交一份 {...草稿} 副本，回写会落在被丢弃的副本上，
              // 重试又按临时编号判新增（再 POST）并把服务端已建那条判为缺失（DELETE）。
              const 段 = { ...草稿, 名称: 草稿.名称.trim() };
              设草稿(段);
              保存(段);
            }}
          >
