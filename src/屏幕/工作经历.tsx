...
export default function 工作经历() {
  ...
  const [待确认解除, 设待确认解除] = useState<屏蔽项[] | null>(null);
  ...
  /** 契约B：校验与确认之后的实际保存链 —— 待提交屏蔽意图按用户表达顺序先行，
   *  每项成功后权威回读核对并移除，然后才是现有 保存简历；保存中 锁覆盖整条链
   *  （重复点击不重发，也不让部分成功冒充全部成功）。 */
  const 执行保存 = async () => {
    if (保存中) return;
    设保存中(true);
    try {
      for (const [组织编号, 想要屏蔽] of [...屏蔽意图.entries()]) {
        ...
        try {
          if (想要屏蔽) {
            if (是后端) {
              ...
              const 名称 = 经历列表.find((段) => 段.组织编号 === 组织编号)?.公司.trim();
              if (名称 === undefined || 名称 === '') {
                写屏蔽意图(组织编号, undefined);
                continue;
              }
  ...
  /** 保存整份简历（single-flight）：经历/作品集/屏蔽意图的校验与确认在前（零写请求
   *  前提下拦下），实际保存链见 执行保存；成功以最终权威 GET 的水合为准，之后才
   *  轻提示 并跳转下一屏。 */
  const 保存 = async () => {
    if (保存中) return;
    ...
    // Mock 的 保存简历 是本地归约、历来允许无目录行，不设此门。
    if (是后端) {
    ...
    // ── 契约B：屏蔽意图的校验与确认（仍在零写请求阶段）──
    if (屏蔽意图.size > 0) {
      for (const [组织编号, 想要屏蔽] of 屏蔽意图) {
        const 条 = 全局.屏蔽名单.find((块) => 块.组织编号 === 组织编号);
        if (想要屏蔽 && 条 !== undefined && 条.来源 !== '手动添加' && !全局.设置开关.对现雇主隐身) {
          // inactive derived 不算 manual：开关不显示生效，也不默默开全局保护/删旧来源
          轻提示('这家公司的自动屏蔽当前未生效，请到「设置」开启对现雇主隐身后回来重试');
          return;
        }
      }
      // derived 来源的解除保留既有风险确认：确认后才进保存链，取消零写、意图保留
      const 待确认 = [...屏蔽意图.entries()]
        .filter(([, 想要屏蔽]) => !想要屏蔽)
        .map(([组织编号]) => 全局.屏蔽名单.find((块) => 块.组织编号 === 组织编号))
        .filter((条): 条 is 屏蔽项 =>
          条 !== undefined && 条.来源 !== '手动添加' && 全局.设置开关.对现雇主隐身);
      if (待确认.length > 0) {
        设待确认解除(待确认);
        return;
      }
