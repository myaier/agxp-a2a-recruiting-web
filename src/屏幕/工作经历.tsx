...
export default function 工作经历() {
  ...
  const 存作品集链接 = (值: string) => {
    ...
    if (是后端) 操作.更新候选建档草稿(并入建档草稿(建档, { 资料: { 作品集链接: 值 === '' ? null : 值 } }));
  };
  ...
  const 存 = (
    ...
    if (是后端) 操作.更新候选建档草稿(并入建档草稿(建档, { 资料: 下一, ...草稿增量 }));
  };
  ...
  const 写编辑中 = (编辑中: 建档编辑中草稿) => {
    if (是后端) 操作.更新候选建档草稿(并入建档草稿(建档, { 编辑中 }));
  };
  const 丢弃编辑层 = () => {
    设恢复编辑(undefined);
    if (是后端) 操作.更新候选建档草稿(并入建档草稿(建档, { 编辑中: undefined }));
  };
  ...
  const 保存 = async () => {
    ...
    // 只在注册旅程（建档草稿在场）拦：日常编辑里 结束 为空是合法的「至今在读」
    // （wire 上是 end_month: null），不能顺手把在读的人锁死在这一屏。
    const 教育缺项数 = 建档 === undefined ? 0 : 教育列表.filter(教育段缺项).length;
    if (教育缺项数 > 0) {
    ...
    try {
      await 操作.保存简历({
        ...
        ...(是后端 && 链接已改 ? { 作品集链接: 建档!.资料!.作品集链接 ?? null } : {}),
      });
