// 真人会话 · P1C Task 5：「看职位」全屏层的公司槽接线契约。
// Backend 下这一层的公司名来自未接线演示域（无 CandidateJob/canonical ref）：
...
describe('真人会话 · Backend 公司槽只读', () => {
  ...
  it('看职位层公司槽只读不可点，不调 路径.企业详情，也不把 yunqu 当 opaque ID 请求', async () => {
    await 打开看职位层();
  ...
  it('匹配对齐卡 仍在且位于职位条件段与公司区块之前', async () => {
    await 打开看职位层();
    断言匹配卡在条件段与公司之前();
  });
});
