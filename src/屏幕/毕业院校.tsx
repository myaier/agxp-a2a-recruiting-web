...
export default function 毕业院校() {
  ...
  // J-PILOT-02 Task 4：这一步的学校只在草稿里成立 —— 教育段此时还缺专业与起止，
  // BFF 会整条跳过，保存成功后的权威水合会把 Context 里的它清掉。
  ...
  // Mock 候选：本地名录过滤。review-r1 F5：按实际查询词过滤 —— 点选只改显示值，
  // 若按显示值过滤，点选后短词变全名会把兄弟候选整批挤掉（§5.5 点选保留当前成功结果列表）
  const mock候选 = 查询词 === '' ? [] : 高校名录.filter((名) => 名.includes(查询词));

  /** 同名学校按稳定 ID 区分：写草稿时学校文本与引用一起落，改文字即作废旧引用。 */
  const 写教育草稿 = (改: { 学校: string; 学校引用?: 目录选择值 }) => {
    if (!旅程中 || 首段 === undefined) return;
    操作.更新候选建档草稿(并入建档草稿(建档, {
      资料: {
        教育: 教育基底.map((条, 序) => (序 === 0 ? { ...条, 学校: 改.学校, 学校引用: 改.学校引用 } : 条)),
      },
    }));
  };
  ...
  const 下一步 = async () => {
    ...
    const 新教育 = 教育基底.map((条, 序) => (序 === 0 ? { ...条, 学校: 词, 学校引用 } : 条));
