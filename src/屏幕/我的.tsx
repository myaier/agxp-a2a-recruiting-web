...
export default function 我的() {
  ...
  //
  // 「在谈」「待你拍」两个数读的是**全量**在谈单（跨意向），而职位 Tab 的列表按当前意向过滤，
  // 所以这两个数点得动：落到「全部意向」档的在谈列表上（拦路 6）。
  // 选的是「统计不动、给列表补一个全部意向档」这条路，而不是把统计缩到当前意向 ——
  // 「一份简历挂多个意向、各自独立谈」是产品主线，统计缩到单意向等于把这条主线砍了，
  // 用户从此再没有任何一个视图能一次看全自己在谈的全部机会。
  // 另外两个数（初筛中 / 已归档）没有对应的档位，保持不可点，不做假入口。
  const 统计: 统计格[] = [
    {
