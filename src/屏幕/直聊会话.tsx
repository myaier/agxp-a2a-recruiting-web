...
export default function 直聊会话() {
  ...
  // 工作包 B：Backend 直聊没有权威会话（:id 是岗位坐标，不是 Case ID）——
  // fixture 查找整条不执行，对方/岗 恒为 null，以下全部槽位按模式分流；
  // hooks 顺序不变，Backend 的 lazy state 只落空集合
  ...
  // 模式/参数变化时重置：Backend 清成中性空态（不复制 fixture）；
  // Mock 在同一条路由上换参数（换个岗再点直接聊）时回到那一岗的既有开场白
  useEffect(() => {
    if (是后端) {
      设消息([]);
      设草稿('');
  ...
  // 发送：草稿去空后追加到本地消息流，随即清空输入框（Backend 无输入条，双保险）
  const 发送 = () => {
    if (是后端) return;
    const 内容 = 草稿.trim();
    if (!内容) return;
  ...
  return (
    <次级页外壳 对话底 白底>
      <返回栏
        ...
        标题={对方 === null ? '直聊' : 对方.姓名}
        副标题={
          对方 === null ? undefined : [对方.机构, 对方.职务].filter(Boolean).join(' · ')
        }
        居中标题
        右侧={
          是后端 ? undefined : (
      ...
      {/* 旁听提示条：交回按钮把话头还给 AI 代理，因此跳到「问AI代理」对话页。
          工作包 B：Backend 直聊没有权威会话，这条只说明现状并指路在谈列表 */}
...
export function use自动滚到底(依赖: unknown, 启用 = true) {
