...
// 双盲约束：回复里只出现代号（A-01 / B-04），绝不出现真名；薪资只答「有无交集」，不给数值。
function 生成回复(问题: string): string {
  ...
  if (问题.includes('带宽') || 问题.includes('薪资'))
    return '带宽内有交集的还有 3 位：B-04 / B-09 / B-15。按双盲规矩我只核「有无交集」，拿不到对方数值，也不会把咱们的带外余量透出去。';
  if (问题.includes('A-01') || 问题.includes('到岗'))
        ...
        <div className={样式.漏斗块}>
          {人才漏斗.map((档) => (
            <div key={档.名称} className={样式.漏斗行}>
              ...
              <div className={样式.漏斗槽}>
                <div className={样式.漏斗条} style={{ width: `${档.宽度}%` }} />
              </div>
