...
//   · 目录换代（页版本变化）：该父项整组重开、后代缓存与展开状态作废，新版本页成为
//     新缓存起点；子页换代只作废被摘子树（缓存摘除 + 在飞标记释放 + 迟到响应逐请求
//     序号作废），不误伤兄弟分支在飞请求；根换代/目录身份（模式+主体）变更作废全部
//     缓存并重拉根页；
...
export function use行业目录({ 查询, 目录身份 }: { 查询: 查询行业页; 目录身份: string }): 行业目录状态 {
  ...
  // 代际：目录身份变更/根页换代清理时 +1，在飞响应回写前核对，不符即静默作废。
  // 子页换代不升代际（只作废被摘子树）；被摘键的迟到响应由载入子页的逐请求序号核对作废。
...
const 写子页 = (父键: string, 页: 行业页, 首页: boolean) => {
    ...
    if (旧缓存.版本 !== '' && 页.版本 !== 旧缓存.版本) {
      // 目录换代（只作废被摘子树，不伤兄弟分支）：新版本页成为该父项新缓存起点；
      // 旧子项名下的后代展开与缓存一并失效。不升全局代际 —— 兄弟分支的在飞请求
      // 不得被静默作废成「成功空页」；被摘键的迟到响应由 载入子页 的逐请求序号核对作废。
  ...
  const 载入子页 = async (父键: string, 首页: boolean) => {
    ...
    try {
      ...
      // 逐请求所有权：本请求的缓存若已被换版摘除、或被摘后用户重新发起的新请求
      // （新序）接手，迟到响应凭序号核对作废，不覆盖新缓存（子页换代不升全局代际）
      if (数据引用.current.子表[父键]?.序 !== 序号) return;
      写子页(父键, 页, 首页);
    } catch (错误) {
      if (代际.current !== 本次) return;
      const 当前 = 数据引用.current;
      const 本条 = 当前.子表[父键];
      // 失败路径同样凭序号核对：新请求接手后旧失败不得清掉新请求的加载态
      if (!本条 || 本条.序 !== 序号) return;
      if (首页) {
