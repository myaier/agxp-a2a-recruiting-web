...
import { 主按钮, 单选点, 开关, 次级页外壳, 滚动区, 页面大标题, 返回栏, 表单条目 } from '../组件/通用';
...
import { Mock城市搜索结果, Mock默认城市字典, 行业字典 } from '../数据/城市与行业';
...
export default function 引导问答() {
  ...
  // 表示「只编辑个人优势」—— 题序直接为个人优势单题、初值取已水合 全局.个人优势、
  // 「保存」只调 保存个人优势 后回我的简历。与 基本信息 / 工作经历 同一标记，
  // 不新增第二种编辑口径。
  ...
  // 个人优势编辑（Task 4）不走 段/题序 配置：固定地址 /wizard?from=resume 只问
  // 个人优势一题，不新增通用 stage 枚举。
  ...
  // 个人优势编辑（Task 4）不消费任何候选预填 / Mock 默认建议：初值只有已水合现值。
  const 预填状态 = 后端状态?.候选预填状态 ?? 创建空候选预填状态();
  const [自我介绍, 设自我介绍] = useState(() =>
    来自简历 ? 全局.个人优势 : 取个人优势预填(预填状态, 段, 全局.个人优势)
  );
  // DF-004：提取说明只认「挂载时真正采用了当前有效建议」这一事实。来源在初始化算
  // 一次，判定沿用 取个人优势预填 的既有门卫（建议 ready/eligible/未确认且非空 +
  // 已有个人优势为空），绝不按文本相等反推 —— 已有用户文本恰与建议相同仍属用户文本；
  // 恢复按钮沿用原动作，不回改该标记，后到建议也不让说明误称已应用。
  const [初值来自建议] = useState(
    () => 取可恢复个人优势建议(预填状态, 段) !== null && 全局.个人优势.trim() === ''
  );
  ...
  // Task 6：城市显示名经 精选城市显示名 归一（ID 命中用规范名），提交仍用引用原 ID。
  const 已选职位名们 = 是后端 ? 已选职位引用.map((条) => 条.display_name) : 已选职位;
  const 已选城市名们 = 是后端
    ? 已选城市引用.map((条) => 精选城市显示名(条.id, 条.display_name))
    : 已选城市;
  ...
  const 下一题 = async () => {
    ...
    if (当前题 === '个人优势') {
      // 个人优势编辑（Task 4）：只保存 summary 一件事 —— 不做作品集规范化回写、
      // 不确认分区、不发首次意向、不写建档草稿。失败保留输入留在本页，可再试。
      if (来自简历) {
        if (优势保存中) return;
        设优势保存中(true);
        try {
    ...
    <次级页外壳 白底>
      <div className={样式.引导壳}>
        {/* 日常编辑（from=resume）的返回=取消：走统一出口（无来路证明时安全替换回我的简历，
            不盲退外站）；注册流仍按题序回上一题、第一题退出本屏 */}
        <返回栏 返回={来自简历 ? 退出日常编辑 : 上一题} />
        ...
        {当前题 === '个人优势' ? (
          <优势题
            文本={自我介绍}
            设文本={设自我介绍}
            标题={来自简历 ? '编辑个人优势' : '分享一下自己的个人优势'}
            // 编辑场景（Task 4）不带提取说明、不带恢复动作：初值已是用户现值，
            // 与简历解析建议无关。注册流（DF-004）只在挂载时真正把有效建议种进了
            // 空输入且输入仍非空时才声称已提取，其余一律中性说明。
            说明={
              来自简历
                ? undefined
                : 初值来自建议 && 自我介绍.trim() !== ''
            ...
            恢复={来自简历 || 恢复文本 === null ? null : () => 设自我介绍(恢复文本)}
          />
        ) : null}
        ...
        <主按钮
          ...
          禁用={
            ...
            // 注册流末题因作品集错误挡保存；编辑场景不碰 URL，不受它牵连
            (!来自简历 && 当前题 === '个人优势' && Boolean(作品集错误)) ||
            (来自简历 && 优势保存中)
          }
        />
      </div>
...
function 城市题({
  ...
  // Task 6：旧中文海外名（别名）与目录规范名都命中，结果归一到规范名并去重。
  const 搜索结果 = Mock城市搜索结果(搜词);

  // ── 两模式的展示输入（下面只有一套 JSX）──
  // 点击入参收窄为 id + display_name（显示名经有限映射归一），已选 state 与落盘引用里只有目录选择值
  const 引用片 = (引用: 目录选择值, 键?: string): 城市片 => {
    const 名称 = 精选城市显示名(引用.id, 引用.display_name);
    return {
      键: 键 ?? 引用.id,
      文字: 名称,
      选中: 已选引用.some((条) => 条.id === 引用.id),
      按下: () => 切换后端({ id: 引用.id, display_name: 名称 }),
    };
  };
  const 后端片 = (项: BFFLocationItem, 键?: string): 城市片 => 引用片(项, 键);
  // Task 6：Mock 城名按规范名归一（热门/分省/搜索同名同键，选择比较统一）
  ...
  const 搜索片们: 城市片[] = 是后端
    ...
    : 搜索结果.map((城) => Mock片(城));
  const 已选条目们: 已选条目[] = 是后端
    ? 已选引用.map((条) => ({ 键: 条.id, 文字: 精选城市显示名(条.id, 条.display_name) }))
    : 已选.map((城) => ({ 键: 城, 文字: 城 }));
  const 移除已选 = (键: string) => {
    if (!是后端) {
      切换(键);
...
// ── A3c 期望薪资：选择行 + 共用薪资区间层（bottom-drawer 统一 Task 5）─────────
// 原页面自写的 薪资轮/档表/算法 已整体移除：档表与 动态上限 纯函数迁到
// 组件/引导薪资档位（页面不被共用组件反向依赖），滚轮正文归 薪资区间层
// （用途='求职引导'）。本行按项目选择行视觉显示「面议」或区间，点开抽屉；
// 确定才调用两侧 setter，取消 / 遮罩 / Escape 不回填。
...
function 薪资题({
  ...
  const 是日薪 = 单位 === '元/天';
  const [层开, 设层开] = useState(false);
  // 行上只显示回显值：面议 或区间。单位是显示后缀（/天），数据单位仍是 求职薪资单位
  const 行文字 = 下限 === 0 ? '面议' : `${下限}-${上限}${是日薪 ? '/天' : 'K'}`;
  return (
    <滚动区 样式覆盖={{ padding: '12px 18px 10px' }}>
      <页面大标题 标题={是日薪 ? '期望实习日薪是？' : '期望现金月薪是？'} />

      <表单条目
        标签={`薪资要求（${是日薪 ? '日薪 · 元/天' : '月薪 · K'}）`}
        值={行文字}
        按下={() => 设层开(true)}
      />
