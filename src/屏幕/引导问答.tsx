...
export default function 引导问答() {
  ...
  const 题序: readonly 题名[] = 全局.引导预填
    ? 题目顺序.filter((题) => 题 !== '期望职位' && 题 !== '工作城市')
    : 题目顺序;
  const 当前题: 题名 = 题序[第几题];

  // ── 七题的答案（前两题被跳过时，初值直接取引导预填）──
  const [已选职位, 设已选职位] = useState(
    全局.引导预填?.职位 ?? ['后端开发', '交易 / 支付系统', '金融科技（行业）']
  );
  const [已选城市, 设已选城市] = useState(
    全局.引导预填 ? [全局.引导预填.城市] : ['上海']
  );
  const [薪资下限, 设薪资下限] = useState(50);
  const [薪资上限, 设薪资上限] = useState(60);
  const [排除项, 设排除项] = useState(['大小周', '纯外包 / 乙方']);
  // 到岗默认档跟着上一屏填的身份走：在校生的「随时到岗」和社会人的不是一回事
  // （前者是毕业后才生效），所以给它单独一档，并在身份=在校 时默认选中
  const [到岗, 设到岗] = useState(
    全局.基本信息.身份 === '在校'
      ? '在校 · 离校随时到岗'
      : 全局.基本信息.身份 === '离职'
        ? '离职 · 随时到岗'
      ...
      <div className={样式.引导壳}>
        ...
        {当前题 === '到岗节奏' ? (
          <到岗题
            当前={到岗}
            设置={设到岗}
            选项={全局.基本信息.身份 === '在校' ? 学生到岗选项 : 社会人到岗选项}
          />
        ) : null}
