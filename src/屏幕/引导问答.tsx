...
export default function 引导问答() {
  ...
  // 原 Mock 网格里两者显示在一起（不改布局）；按标签是不是固定卡分流到各自载体。
  // 与卡片同名的用户原话仍走 自定义诉求 —— 只有点卡片本身才动 排除项。
  const 排除已选 = [...排除项, ...自定义诉求];
  const 切换排除 = (项: string) => {
    const 设值 = 排除候选.includes(项) ? 设排除项 : 设自定义诉求;
  ...
  const 落盘当前题 = () => {
    ...
    // Task 7：硬性排除 的两个载体随离开该题落进建档草稿（刷新/返回不丢用户答案）
    if (当前题 === '硬性排除' && 旅程中) {
      操作.更新候选建档草稿(并入建档草稿(建档, { 排除项, 自定义诉求 }));
    }
  ...
  return (
    ...
    <次级页外壳 白底>
      <div className={样式.引导壳}>
        ...
        {当前题 === '硬性排除' ? <排除题 已选={排除已选} 切换={切换排除} /> : null}
        {当前题 === '个人优势' ? (
          <优势题
            文本={自我介绍}
            设文本={设自我介绍}
...
export function 排除选项({ 已选, 切换, 自动聚焦 = true }: { 已选: string[]; 切换: (项: string) => void; 自动聚焦?: boolean }) {
