...
export default function 看市场() {
  ...
  // 本次要委托的卡：确认层只在它非空时挂载，取消即清、确认后立刻清（不复用授权）
  const [待确认委托, 设待确认委托] = useState<P4候选岗位页面 | null>(null);
  ...
  const 执行候选委托 = async (视图: P4候选岗位页面) => {
  ...
  return (
    <主页外壳>
      ...
      {待确认委托 ? (
        // 不可逆动作的披露确认：正文交代 S0 通过后会提交什么、披露什么；
        // 取消 / 遮罩 / Esc 都是零请求零状态，确认后才发起委托
        <确认层
          标题="确认委托AI代理？"
