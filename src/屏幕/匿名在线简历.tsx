...
import { Mock匹配分数, Mock匹配解释, Mock有快照 } from '../数据/Mock匹配快照';
...
interface 简历正文兼容属性 {
  ...
  /** C3 匹配分析模型（Spec §3.3）：已展开解释的六维版式；与 匹配依据行们 互斥
   *  （与 Backend 分支同一互斥处理，二者不同现）。 */
  匹配分析?: 匹配分析模型;
  完整布局?: boolean;
  缺失说明?: string | null;
}
...
export function 简历正文({
  ...
  return (
    <在线简历正文
      ...
      匹配依据行们={匹配分析 === undefined ? 匹配依据行们 : undefined}
      匹配分析={匹配分析}
      已确认={已确认}
      完整布局={完整布局}
      缺失说明={缺失说明}
...
function Mock匿名简历() {
  ...
  // review-r1 Finding 1：同一记录的固定快照 → C3 匹配分析模型（分数 = total_points
  // 或种子回落，解释 = 固定表值，有限依据 = 档.推荐依据 的已知正向码）。快照 null
  // 条目保留分数与有限依据（模型缺位形态），绝不补六条假状态。
  const Mock快照在场 = Mock有快照(编号);
  // 顶栏唯一总分：与列表环/分析模型同一来源（Spec §7）——快照键存在用固定快照分，
  // 无快照记录沿用 档.适配分 既有回退（同屏不出现两个不同总分）。
  const 栏匹配分 = Mock快照在场 ? Mock匹配分数(编号) : 档?.适配分;
  const Mock匹配分析: 匹配分析模型 = useMemo(() => ({
    分数: Mock匹配分数(编号),
    解释: Mock匹配解释(编号),
    有限依据: 映射推荐依据(档?.推荐依据.highlights ?? []),
    上下文: '有来源' as const,
  }), [编号, 档]);
    ...
    <次级页外壳>
        ...
        右侧={
          <span className={样式.栏右组}>
            ...
            <span className={样式.栏匹配组}>
              ...
              {/* review-r2（Spec §7 同源）：快照键存在时顶栏与分析模型/列表环共用
                  Mock匹配分数（null 条目回落种子）；无快照记录沿用 档.适配分 回退 */}
              <span className={`${样式.栏匹配分} 等宽数字`}>{栏匹配分}</span>
            </span>
      ...
      <滚动区 样式覆盖={{ paddingBottom: 8 }}>
        <简历正文
          档={档}
          薪资结论={薪资结论}
          /* review-r1 Finding 1：Mock 记录与 Backend 同构消费固定快照 —— 在快照表内
             （含显式 null）走 C3 匹配分析模型版式（顶栏 匹配 N 是唯一总分，分析区
             藏环）；不在表内才是未展开防御路径，保留六行有限依据。二者互斥不双渲染，
             与 Backend 分支同一处理。 */
          匹配分析={Mock快照在场 ? Mock匹配分析 : undefined}
          匹配依据行们={Mock快照在场 ? undefined : 映射招聘匹配依据(档.推荐依据)}
...
function Backend匿名简历({ 岗位编号, 推荐编号 }: { 岗位编号: string; 推荐编号: string }) {
  ...
  // （仅作为 include=match_explanation 未展开读取的防御路径：C2 之后生产详情恒展开，
  //   展开读取走下面的 匹配分析模型 六维版式。）
  ...
  // Spec §3.3：已展开批次解释 → C3 匹配分析模型（藏环：总分唯一在顶栏 匹配 N）。
  // 分数/解释/有限依据全部来自同一条推荐：解释缺席（显式 null）时同区标有限依据
  // （本卡 highlights 的已知原因），绝不补六条假状态、不借其他记录/批次。
  const 匹配分析模型 = useMemo(() => (卡 === null ? null : {
    分数: 卡.match_score,
    解释: 卡.match_explanation ?? null,
    有限依据: 映射推荐依据(卡.highlights),
    上下文: '有来源' as const,
  }), [卡]);
  const 解释已展开 = 卡 !== null && 卡.match_explanation !== undefined;
    ...
    <次级页外壳>
      ...
      <滚动区 样式覆盖={{ paddingBottom: 8 }}>
        {/* ── 正文唯一出处（Task 5 接入；Task 6 走唯一正文内容）：candidate_resume 投影的
            ...
            自 Task 5 起的缺失布局（任一区块缺源不整区消失）。
            匹配区（Spec §3.3）：已展开解释 → 匹配分析 模型版式（六维直接展开、藏环、
            无「当前接口仅提供部分匹配依据」）；未展开读取的防御路径才走六行有限依据；
            顶栏匹配分恒为唯一总分。 ── */}
        <在线简历正文
          完整布局
          内容={从安全资料到简历正文(从BFF到在线简历展示(视图.candidateResume))}
          匹配分析={解释已展开 ? 匹配分析模型 ?? undefined : undefined}
          匹配依据行们={解释已展开 ? undefined : (匹配依据行们 ?? [])}
        />
      </滚动区>
