...
// 收藏与委托走 P4 操作，成功后原地停留，进行中回执交给轮询钩子。Mock 分支原样保留。
      ...
      <div className={样式.头区}>
        <div className={样式.头文}>
          <div className={样式.代号行}>
            ...
            <span className={样式.匿名标}>{已披露 ? '已披露身份' : '匿名'}</span>
          </div>
...
function Backend匿名简历() {
  ...
  useEffect(() => {
    ...
    const 读取 = 操作.读取招聘候选详情?.(当前岗位编号, 推荐编号, true);
    if (读取) {
      void 读取.catch((错误: unknown) => 设读取错误(P4错误文案(错误)));
    }
    return () => 操作.设置发现推荐范围?.('recruiter', null);
  }, [是后端, 当前岗位编号, 推荐编号, 操作]);

  // 权威卡与不可用标记都来自操作层提交的缓存
  const 卡 = 推荐编号 ? 后端状态.招聘候选详情?.[推荐编号] ?? null : null;
  ...
  // 带文案的错误，catch 呈现即可。成功不导航 —— 原地切成「AI代理已接手」状态条。
  const 委托候选 = async () => {
    if (反馈中 || !当前岗位编号 || !推荐编号) return;
    设反馈中(true);
    try {
  ...
  // 本页唯一可见的进行中委托（accepted/evaluating）
  const 进行中委托 = useMemo(() => {
    const 委托 = 视图?.委托 ?? null;
  ...
  const 进度未知 = use发现推荐委托轮询({
    开启: 是后端,
    委托: 进行中委托,
    // 生产 Provider 恒注入全表；部分桩宿主缺它时安全兜底，
  ...
  const 委托文字 = 已委托 && 进度未知.has(委托摘要.delegation_id)
    ? P4委托进度未知文案
    : 'AI代理已接手';
  ...
  return (
    <次级页外壳>
      ...
      <div className={样式.底栏}>
        <div className={样式.键行}>
          {已委托 ? (
            // 委托中/已接手态：换成不可点的状态条（代理已接手，不需要再点第二次）
