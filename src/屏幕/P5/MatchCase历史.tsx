...
//     页面、不新增结果筛选布局），经 映射连续列表项 投影进原白卡视觉；点卡按 canonical
//     record_id 开同一在谈详情 —— needs_action=false 不等于禁止恢复（按钮权限读取
//     actions.retry，恢复控件在详情动作槽，本组件零动作控件）。
...
import type { 连续列表视图 } from '../../数据/连续代谈展示映射';
...
export function MatchCase历史(props: { role: P5角色 }) {
  ...
  return props.role === 'candidate'
    ? <候选连续历史 />
    : <招聘终局架子组 role={props.role} />;
}

/** J-PILOT-01 Task 4：候选历史 = 单一服务端分页集合（shelf=history）。原序渲染、显示
 *  结果/失败原因；「加载更多」接 追加连续列表，无运行 timer，手动刷新用 force 首屏。 */
function 候选连续历史() {
...
/** 候选连续终局卡：冻结职位事实 + 阶段标 + 结果/失败原因行；零徽标零控件（读-only）。 */
function 连续终局卡({ 视图, 按下 }: { 视图: 连续列表视图; 按下: () => void }) {
  const 事实段们 = [视图.城市, 视图.薪资带].filter((段) => 段 !== null);
  return (
    <白卡 按下={按下} 类名={列表样式.卡}>
      <div className={`${列表样式.职位名} 单行`}>{视图.职位名}</div>
      {事实段们.length > 0 ? (
        <div className={`${列表样式.事实行} 单行`}>{事实段们.join(' · ')}</div>
      ) : null}
      <div className={列表样式.阶段区}>
        ...
        {/* 结果/失败原因（初评失败/拒绝/attention 的安全说明）：单独一行，复用既有类 */}
        {视图.注意说明 !== null ? (
...
/** 招聘端终局历史：completed/ended 两个独立架子（原 Task 7 结构原样保留）。 */
function 招聘终局架子组({ role }: { role: P5角色 }) {
  const { 后端状态, 操作 } = use应用状态();
...
/** 冻结职位四事实的展示段（城市·薪资带/技能标签），招聘端终局卡共用（同列表卡）。 */
