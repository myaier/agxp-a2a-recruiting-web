...
//     招聘卡卡主体 = 候选摘要（头行/工作/教育/个人亮点），在谈无匹配分不补造匹配环。
...
/** 招聘端卡（2026-09-09 摘要接线）：卡主体 = candidate_summary 摘要（头行 性别符 + 年限｜
 *  学历｜求职状态、最近工作/教育信息行、个人亮点标签行）+ 阶段区；别名/通用匿名头像/
 *  冻结职位事实段（城市薪资带/技能）退场 —— 职位段 仍供候选卡使用，共享映射不删。
 *  在谈接口无匹配分，不补造匹配环；待办徽标、阶段段、Case 跳转与白卡可点击保留。
 *  摘要 null 或头行全空显示中性「候选信息暂未披露」（只剩性别时不加中性文字）。 */
function 招聘在谈卡({ 视图, 按下 }: { 视图: P5列表正常视图; 按下: () => void }) {
  ...
  return (
    <白卡 按下={按下} 类名={样式.卡}>
      ...
      {摘要?.工作 != null ? (
        <div className={样式.摘要行}>
          <公文包图标 尺寸={14} 色="var(--次要浅)" />
          <span className={`${样式.摘要文} 单行`}>{摘要.工作}</span>
        </div>
      ) : null}
      {摘要?.教育 != null ? (
        <div className={样式.摘要行}>
          <学帽图标 尺寸={14} 色="var(--次要浅)" />
          <span className={`${样式.摘要文} 单行`}>{摘要.教育}</span>
        </div>
      ) : null}
