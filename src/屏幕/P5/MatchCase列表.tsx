...
//     招聘卡卡主体 = 组件/列表卡片/招聘在谈卡（2026-09-10 卡片统一，与 Mock 在谈候选同一张
//     卡面）：摘要经 从招聘摘要到卡信息 投影，P5 open 无匹配分传 null（未知分占位），
//     阶段/待办徽标经 从P5到阶段 投影进卡底阶段区。
          ...
          {视图们.length === 0 ? (
            ...
            视图们.map((视图, 下标) =>
              视图.kind === '契约错误' ? (
                ...
                // 冻结职位事实段退场（职位段 仍供候选卡使用），Case 跳转坐标不变。
                <招聘在谈卡
                  key={视图.caseId}
