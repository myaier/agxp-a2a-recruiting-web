...
export function MatchCase详情(props: { role: P5角色 }) {
  ...
  return (
    <详情外壳
      ...
      资料={
        // 第二 Tab（详情统一 Task 3）：共用 职位资料 渲染 spec §3.3 全部区块 —— 摘要来自
        // Case 冻结的职位四事实，其余字段 P5 detail 未提供，一律按约定缺失显示（投影见
        // 从P5到职位资料），不用公司/岗位/推荐查询补值，也不发任何新请求。
        // 无合法公司导航坐标：入口位置保留、真实禁用并就地解释，不从公司文本推 opaque ID。
        <职位资料
          信息={从P5到职位资料(正常)}
          公司详情={{
            键: '公司详情',
