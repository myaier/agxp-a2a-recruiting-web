...
// 稳定回答在飞表，见 屏幕/详情控制/use后端详情控制），按返回联合二选一：
//   · kind 不可用（契约错误 / 首载失败 / 读入中）：原样的错误/加载页 —— 不进共用外壳、
//     不调 mapper、不摆 Tab，不给虚假详情；
//   · kind 正常：keyed 后端正常详情（按 主体/角色/单 重挂载 —— 换任一者 Tab 回进度、
//     弹层销毁、动作/PDF 草稿与租约回收），内部无条件调用 use后端详情动作 与
//     useCasePDF预览 并组装纯展示，见 屏幕/详情控制/后端正常详情。
//
// 模式边界（spec §5/§6/§8/§10.3 与 P5 冻结契约）：
//   · 详情只凭 URL case_id + 已认证角色强制 GET（读取详情 恒 force=true），绝不读列表
...
import { 后端正常详情 } from '../详情控制/后端正常详情';
...
export function MatchCase详情(props: { role: P5角色 }) {
  ...
  // 正常控制子组件按 主体/角色/单 key 重挂载（plan 生命周期固定）：换 Case、换角色或
  // 换账号都整体重置 —— Tab 回进度、弹层销毁、原草稿清空、PDF 租约回收；同一单的
  // 正常↔错误交替只卸载子组件，父实例（含回答在飞表）不重建。
  const 主体键 = 后端状态.主体?.subject_id ?? '无主体';
  return <后端正常详情 key={`${主体键}:${role}:${caseId}`} 资源={资源} />;
}
