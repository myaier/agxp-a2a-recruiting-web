...
export default function 基本信息() {
  ...
  // L：滚轮没有「空态」总显示一个落点 —— 显示值与确认态分离。Backend 只有已有完整
  // 年月、完整真实建议或用户滚动过滚轮，才算确认（保存双值）；未确认保存 null 语义
  // （不写入出生年月）。Mock 保持原型默认（1998/6 演示数据直接确认）。
  ...
  // 任一滚轮交互代表用户确认屏上显示的完整年月（不管动的是哪一轮）
  const 设出生年并确认 = (值: number) => { 设出生年(值); 设出生年月已确认(true); };
  const 设出生月并确认 = (值: number) => { 设出生月(值); 设出生年月已确认(true); };
