...
function 候选卡({ 单, 按下 }: { 单: 候选; 按下: () => void }) {
  ...
  // 画像 = 「年限 · 方向 · 现职类型」：方向+年限当大标题，现职类型进头行副行
  //（与求职端在谈卡的 公司简介 / 职位名 两个槽位一一对应）
  const [年限, 方向, 现职类型] = 单.画像.split(' · ');
  // 标签行：能回查到推荐库的用结构化亮点；回查不到就不摆标签 ——
  // 头行与大标题已交代过年限/方向/现职，重复一遍只是噪音
  const 推 = 推荐列表.find((条) => 条.编号 === 单.编号);
  const 标签组 = 推 ? [...推.亮点, 推.学历] : [];

  return (
    <白卡 按下={按下} 类名={样式.卡}>
      {/* 头行与求职端在谈卡一比一：字标 + 代号/现职 + 右列[匹配环 + 交集结论] */}
      <div className={样式.头行}>
        <公司字标
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className={`${样式.画像行} 单行`}>{现职类型 ?? 单.画像}</div>
        </div>
        <div className={样式.右列}>
          <适配环 分={单.匹配分} 标={null} />
        </div>
      ...
      {/* 大标题：方向 + 年限（求职端「职位名」的位置） */}
      <div className={`${样式.方向名} 单行`}>
        {方向 ?? 单.画像}{年限 ? ` · ${年限}` : ''}
      </div>

      {/* 标签行：同求职端标签片（没有结构化亮点的单不摆空行） */}
      {标签组.length > 0 ? (
