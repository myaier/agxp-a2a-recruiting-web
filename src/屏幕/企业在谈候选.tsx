...
// 卡片结构：代号头行（头像 + 代号/真名 + 画像 + 匹配分）→ 一行状态评语
...
export default function 企业在谈候选() {
  ...
  const 排序后 = [...状态.企业候选列表].sort((甲, 乙) => Number(乙.需要你) - Number(甲.需要你));
  const 待拍板数 = 状态.企业候选列表.filter((单) => 单.需要你).length;
...
function 候选卡({ 单, 按下 }: { 单: 候选; 按下: () => void }) {
  ...
  return (
    <白卡 按下={按下} 红描边={单.需要你} 类名={样式.卡}>
      ...
      {/* 一行状态评语：代理对该候选的当前判断（正=深绿 中=次要） */}
      <div
        className={`${样式.状态评语} ${单.评语色 === '正' ? 样式.评语正 : 样式.评语中} 单行`}
      >
        {单.状态评语}
      </div>
