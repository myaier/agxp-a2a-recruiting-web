...
const 自有能做 = (代理名: string) => [
  `你的 agent 与${代理名}直接对谈，不用人转述`,
...
export default function 接入飞书({ 企业端 = false }: { 企业端?: boolean }) {
  ...
  const 做的事 = 企业端 ? '招聘' : '求职';
  // 注册走完落在主页，而不是上次停留的 Tab（用户 2026-08-20）
  const 完成 = () => {
    if (企业端) {
      派发({ 型: '企业切Tab', Tab: '人才' });
      进企业主壳();
  ...
  return (
    <次级页外壳>
      <div className={样式.页}>
        {/* 标题固定且短（标注 18:49：原来那句太长）：差异由副题承担，顶部不跳版 */}
        <h1 className={样式.标题}>接入你的{企业端 ? '招聘' : '求职'}代理</h1>
        <p className={样式.副题}>
          {方式 === '飞书'
