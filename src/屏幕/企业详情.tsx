...
//
// 三条与常规招聘 App 的刻意差异，都由双盲机制推导而来：
//   1. 不做真人 HR 头像墙 —— 招聘方的真人身份在意向确认前一律不出现；
//   2. 不做「竞争者人数 / 你超过百分之多少」—— 那是可反推他人的数字；
//   3. 核对结论一律定性（覆盖 / 一致 / 偏窄），不出现任何薪资数字。
//
...
export default function 企业详情() {
  ...
  const 市场的 = 市场列表.filter((条) => 条.公司行.startsWith(档.名称));
  const 可点岗位 = [
        ...
        <div className={样式.正文区}>
          ...
          <div className={样式.卡}>
            ...
            <div className={样式.卡正文}>
              接触与谈判由双方代理进行，这一页不会出现招聘方的真人身份。
              双方确认意向后才互换姓名与联系方式，在那之前谁都看不到对面是谁。
            </div>
          </div>
