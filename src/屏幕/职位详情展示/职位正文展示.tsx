  ...
  return (
    <div className={样式.内容}>
      ...
      <div className={样式.卡}>
        ...
        {/* DF-011 推荐依据（仅在职位名/薪资之后、JD 之前的这张匹配卡内）：当前推荐批次
            返回的已知原因，映射/去重已在准备层完成。undefined = 消费者未启用（Mock/Case
            原样）；[] = 启用但无已知原因 → 暂无推荐依据。只是说明文字：不升级为逐条证据、
            不新增分数环。 */}
        {数据.推荐依据 !== undefined ? (
          <>
            <div className={样式.卡小标题}>推荐依据</div>
            {数据.推荐依据.length > 0
              ? 数据.推荐依据.map((依据) => (
                <div key={依据} className={样式.卡正文}>{依据}</div>
              ))
              : <div className={样式.卡正文}>暂无推荐依据</div>}
          </>
        ) : null}
      </div>
