...
describe('选专业 Backend', () => {
  ...
  it('点候选保存专业引用', async () => {
    ...
    // Task 4：可访问名就是专业名称（专业无副文，不往 accessible name 里拼东西）
    expect(screen.getByRole('button', { name: '计算机科学与技术' }).textContent).toBe('计算机科学与技术');
  ...
  it('同名不同 ID 时保存的是点中那行的目录 ID', async () => {
    ...
    const 行 = await screen.findAllByRole('button', { name: '同名专业' });
    expect(行).toHaveLength(2);
    // 点第二行（同名，id 不同）：保存的必须是 maj_b
    await 用户.click(行[1]);
    await 用户.click(screen.getByRole('button', { name: '下一步' }));
    await waitFor(() => expect(保存简历).toHaveBeenCalled());
    expect(保存简历).toHaveBeenCalledWith(
      expect.objectContaining({
        教育: [expect.objectContaining({ 专业引用: { id: 'maj_b', display_name: '同名专业' } })],
      }),
    );
  });
...
describe('选专业 Mock', () => {
  ...
  it('本地专业名录过滤，无引用', async () => {
    ...
    // Task 4：可访问名就是专业名称（选中行也不把 ✓ 拼进 accessible name）
    expect(screen.getByRole('button', { name: '经济学' })).toBeTruthy();
  });

  // Task 4 真实缺口：Mock 保持自由文本 —— 名录外的专业名不点候选也能继续，
  // 不引入「必须点候选 / 必须有目录 ID」的新规则
  it('Mock 手输名录外的专业也能继续（自由文本，无目录必选）', async () => {
    const { 保存简历 } = render选专业({ 数据源: 'mock' });
    const 用户 = userEvent.setup();
    await 用户.type(screen.getByRole('textbox'), '某某交叉学科');
    // 名录里没有这个专业：过滤后无候选行，也不因此拦住下一步
    expect(screen.queryByRole('button', { name: '某某交叉学科' })).toBeNull();
    const 下一步 = screen.getByRole('button', { name: '下一步' }) as HTMLButtonElement;
    expect(下一步.disabled).toBe(false);
    await 用户.click(下一步);
    await waitFor(() => expect(保存简历).toHaveBeenCalled());
    expect(保存简历).toHaveBeenCalledWith(
      expect.objectContaining({ 教育: [expect.objectContaining({ 专业: '某某交叉学科' })] }),
    );
    const 调用 = 保存简历.mock.calls[0][0] as { 教育: { 专业引用?: unknown }[] };
    expect(调用.教育[0].专业引用).toBeUndefined();
  });
...
// ── J-PILOT-02 Task 4：专业写在草稿的同一条教育段上 ──
// 反例针对的真实故障：BFF 跳过不完整教育条目，上一屏保存成功后权威快照里根本没有
// 这条教育，Context 被权威水合清成空 —— 只读 Context 的话，这一屏会把学校丢掉。
describe('选专业 · 建档草稿接线（Task 4）', () => {
  beforeEach(() => {
    mock更新草稿.mockClear();
  });

  it('Context 教育为空但草稿有学校：保存带上同一条教育段的学校与本次专业', async () => {
    const 查询Taxonomy = vi.fn(async () => ({
      items: [{ id: 'maj_cs', display_name: '计算机科学与技术', selectable: true, has_children: false }],
      nextCursor: null,
      catalogVersion: 'v2',
    }));
    const { 保存简历 } = render选专业({
    ...
    expect(末次.资料.教育[0]).toEqual(expect.objectContaining({ 学校: '复旦大学', 专业: '计算机科学与技术' }));
  });
});
