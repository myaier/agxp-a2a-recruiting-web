  ...
  return (
    <>
      <招聘名片展示
        ...
        保存中={保存中}
        返回={返回}
        // Spec §4.1：注册流（onboarding）不提供公司主页维护能力 → 展示层不渲染该行；
        // 应用内日常编辑（企业我的/企业设置进来的名片）照常保留公司管理入口。
        // 判据就是本屏既有的 从注册流 事实，不新建完成标记、不按数据源类型分流。
        打开公司资料={从注册流 ? undefined : () => 跳转(路径.公司档案编辑)}
      />
      {抽屉开 ? (
        <公司选择抽屉接线
          查询={查询}
...
function Mock名片() {
  ...
  const { 状态, 派发 } = use应用状态();
  // Spec §4.1：公司与 Backend 同一来源判定 —— 注册流标记写在 history.state（选身份/路由
  // 守卫写），不按数据源模式猜。企业我的/企业设置 进来的日常名片没有这个标记。
  const 位置 = useLocation();
  const 从注册流 = Boolean((位置.state as { 从注册流?: boolean } | null)?.从注册流);
  ...
  return (
    <招聘名片展示
      ...
      保存中={false}
      返回={返回}
      // Spec §4.1：Mock 注册流同样不渲染公司主页资料行；日常编辑保留既有公司入口
      打开公司资料={从注册流 ? undefined : () => 跳转(路径.公司档案编辑)}
    />
  );
}
