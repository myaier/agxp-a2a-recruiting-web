...
//              admin+verified+active 才可编辑，member（verified+active 但非管理员）
//              只读（行仍可进只读分区页）。分区表、算分区状态 与编辑页共用。
//              pending/revoked/suspended 根本没有可用企业关系，由 招聘方组织门 收口成
//              申请空态，而不是拿别人的快照渲染一份只读清单。
// 两条分支的页面外壳与分区清单行是同一份 JSX（公司清单页面 / 公司分区清单）：
// 差异只有「分区状态从哪来」和「点行后由谁跳转」，展示层不再各写一遍。
...
import type { 分区键, 分区状态 as 分区状态形 } from '../数据/公司主页资料';
...
// ── 两分支共用展示（Mock 是设计源头，Backend 复用同一份行结构）──────

/** 清单页外壳：返回栏 + 页面大标题 + 滚动区。两分支的标题与内边距一致，
 *  差别只在滚动区里放什么（Backend 还要在里面套 招聘方组织门）。 */
function 公司清单页面({ 返回, children }: { 返回: () => void; children: ReactNode }) {
  return (
    <次级页外壳>
      <返回栏 返回={返回} />
...
/** 分区清单本体：一行一个分区，点行进该分区的编辑页（/hr/company-profile/:area）。
 *  只消费算好的 分区状态 与 选分区 回调 —— 不读 Context、不知道数据源模式，
 *  导航由连接层在回调里决定；固定分区表是静态数据，直接读。 */
function 公司分区清单({
  分区状态,
  选分区,
}: {
  分区状态: Record<分区键, 分区状态形>;
  选分区: (段: string) => void;
}) {
  return (
    <div className={样式.清单}>
...
/** 门后的清单本体：进到这里就意味着 current 已选、权威快照已在手。
 *  这里不再有「快照为 null → 正在加载」的分支 —— 那条歧义分支已交给 招聘方组织门。 */
function 权威企业分区清单() {
  const { 状态 } = use应用状态();
  const { 跳转 } = use导航();
  const 快照 = 状态.企业档案快照;
  ...
  const 分区状态 = 算分区状态(从BFF企业档案(快照), 快照.logo?.url ?? null);

  return (
    <>
      {/* 只读说明留在 Backend 连接层：Mock 没有企业关系这回事，也不该看到这句话 */}
      {!可编辑 ? (
        <div
          className={样式.分区摘要}
      ...
      <公司分区清单 分区状态={分区状态} 选分区={(段) => 跳转(路径.公司档案分区(段))} />
    </>
  );
}
