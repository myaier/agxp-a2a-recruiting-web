...
export function 硬性行们(
  ...
  // 2026-08-26 用户裁定:去掉「· 我 XX」的对照后缀——✓/✗ 已经是结论,
  // 再摆一遍我的值是重复;只写岗位的要求,匹配与否由勾叉说话
  if (源.经验要求) {
    const 需 = parseInt(源.经验要求, 10);
    行.push({ 文: `经验 ${源.经验要求}`, 过: !Number.isFinite(需) || 需 <= 9 });
  }
  if (源.学历要求) {
    const 阶 = (值: string) => (值.includes('硕士') ? 2 : 值.includes('本科') ? 1 : 0);
    行.push({ 文: `学历 ${源.学历要求}`, 过: 阶(源.学历要求) <= 2 });
  }
  const 城 = 源.标签?.[0]?.split(' · ')[0];
  if (城) {
    // 全远程岗不卡城市：任一标签带「远程」即视为通过(这不是对照,是通过的原因,保留)
