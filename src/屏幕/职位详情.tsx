...
export function 职位正文({ 岗, 藏直聊 = false }: { 岗: 市场职位; 藏直聊?: boolean }) {
  ...
  const { 跳转 } = use导航();
  const 详 = 取市场岗位详情(岗);
  // P1C Task 5：Backend 下候选入口仍来自静态市场列表，没有 CandidateJob/canonical ref。
  // 公司名照常展示（演示域），但公司槽只读：不传 按下、不调 路径.企业详情，
  // 也不把 'yunqu' 之类的静态键当 opaque Organization ID 请求。Mock 仍按原 slug 导航。
  ...
  return (
    <div className={样式.内容}>
      ...
      {/* 公司(加厚,2026-08-26 用户指定「所有页面都要改」):共用 公司区块 组件,
          ...
          Backend(P1C Task 5)公司槽只读:显式 资料(演示域一行简介兜底),不传 按下 */}
      <div className={样式.卡}>
        <公司区块
          名称={详.公司.名称}
          首字={详.公司.首字}
