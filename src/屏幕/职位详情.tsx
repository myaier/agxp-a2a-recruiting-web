...
import { 从P4候选岗位, 从P4CandidateJob, P4已开案, 映射P4委托展示 } from '../数据/发现推荐映射';
...
function Backend职位详情() {
  ...
  // 开案成功（case_started + 非空 case_id）复用 Mock 的成功文案与 disabled 主键，
  ...
  return (
    <次级页外壳>
      ...
      <div className={样式.浮动条}>
        ...
        {/* 一切委托态（含开案成功）都保持 Mock 的 disabled 主键形态 —— 成功槽不再是
            「查看进展」，也不绑定任何 Case 导航（Spec §7.1） */}
        <button
    ...
    <div className={样式.内容}>
      ...
      <div className={样式.卡}>
        {视图 === null || 视图.匹配依据已确认 === true ? (
          (() => {
            const 对齐行们 = 求职侧对齐行(
              // Backend:对齐生成器只吃岗位事实（none → null 不生成行），
              // 不读卡片里的「不限」；Mock 仍传岗卡（原型演示口径逐字不变）
              视图 !== null
