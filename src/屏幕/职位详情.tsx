...
import type { 职位正文展示属性 } from './职位详情展示/类型';
...
function Backend职位详情() {
  ...
  // 推荐坐标只认 URL 精确四坐标（C2）：求职推荐导航用 query 携带 recommendation_id /
  // batch_id / intention_id，job_id 取路由段并核对。三键齐全才进推荐恢复路径；缺任何
  // 一键就是通用岗位直达 —— 绝不扫描当前意向/其他意向的快照来拼数据。
  const 当前意向编号 = 状态.当前意向编号 ?? null;
  const 坐标 = useMemo(() => {
    const 参数 = new URLSearchParams(位置.search);
    const 推荐 = 参数.get('recommendation_id') ?? '';
    const 批次 = 参数.get('batch_id') ?? '';
    const 意向 = 参数.get('intention_id') ?? '';
    return 推荐 !== '' && 批次 !== '' && 意向 !== '' && 编号
      ? { 推荐编号: 推荐, 批次编号: 批次, 意向编号: 意向 }
      : null;
  }, [位置.search, 编号]);
  // 缓存仅四坐标全等时可用：按 URL 意向取那一份快照（复用已有读取的缓存），再在其中
  // 找 recommendation_id + batch_id + intention_id + job_id 全等的卡 —— 批次不符或同岗位
  // 其他意向/批次的卡都不可用，绝不替换。账号/角色一致性沿用既有状态隔离：快照缓存
  // 是会话内存，操作层栅栏（主体 ID + last_used_role + 会话代际 + scope 代际）保证跨
  // 账号/角色的读取与迟到响应不落库，页面不另造第二套身份门。
  const 坐标意向快照 = 坐标 === null ? undefined : 后端状态.候选岗位推荐?.[坐标.意向编号];
  const 推荐卡 = useMemo(() => {
    if (坐标 === null || 坐标意向快照 === undefined) return null;
    return 坐标意向快照.items.find((卡) =>
      卡.recommendation_id === 坐标.推荐编号
      && 卡.batch_id === 坐标.批次编号
      && 卡.intention_id === 坐标.意向编号
      && 卡.job.job_id === 编号) ?? null;
  }, [坐标, 坐标意向快照, 编号]);

  // 直链/刷新进详情时坐标意向的快照可能还没进内存：只补加载 URL 指定这一份来恢复原
  // 记录（复用已有读取）。已结算（成功/失败）的快照绝不自动重发 —— 失败不换 scope 重试
  //（既有合同）；进行中的快照交给 操作.加载候选岗位 的单飞/锁主接管。无坐标的通用直达
  // 一个快照都不发。
  useEffect(() => {
    if (坐标 === null || 后端状态.初始化 !== '完成') return;
    if (坐标意向快照 !== undefined && (坐标意向快照.阶段 === '成功' || 坐标意向快照.阶段 === '失败')) return;
    void 操作.加载候选岗位(坐标.意向编号).catch(() => undefined);
  }, [坐标, 后端状态.初始化, 坐标意向快照, 操作]);
  ...
  const 重试读取 = () => {
    if (!编号) return;
    设读取错误(null);
    // 坐标路径的快照若也失败，同一次显式重试一并按原 scope 补读（不换 scope、不猜新批次）
    if (坐标 !== null && 坐标意向快照?.阶段 === '失败') {
      void 操作.加载候选岗位(坐标.意向编号, true).catch(() => undefined);
    }
  ...
  // 推荐坐标恢复的三种非命中态（坐标路径直链进详情时快照可能还在途）：
  //   · 快照未开始或进行中 → 恢复中，坐标回来前绝不给可点的委托键；
  //   · 快照失败 → 临时读取失败：主键给通用不可用文案（既有合同），不换 scope 重试，
  //     分析区保持中性缺失 —— 不冒充「该次推荐上下文已不可用」；
  //   · 快照成功但没有这张卡（穷尽未找到 / 批次不符）→ 原推荐不可用：分析区显示
  //     「该次推荐上下文已不可用」，通用职位资料照常可看，绝不替换成新批次。
  //   · 无 URL 坐标（通用岗位直达）→ 不恢复、不扫描，分析区显示无推荐上下文。
  const 恢复中 = 坐标 !== null && 推荐卡 === null &&
    (坐标意向快照 === undefined || 坐标意向快照.阶段 === '未开始' || 坐标意向快照.阶段 === '进行中');
  // C3 匹配区上下文：有卡 = 有来源；无坐标 = 无推荐上下文；穷尽未找到 = 原推荐不可用；
  // 恢复中/临时失败给中性 '有来源'（只出「暂无该次匹配的详细分析」，不提前宣判）。
  const 推荐上下文: 匹配分析上下文 = 推荐卡 !== null
    ? '有来源'
    : 坐标 === null
      ? '无推荐上下文'
      : 坐标意向快照?.阶段 === '成功' ? '原推荐不可用' : '有来源';
    ...
    : 核对在手
      ...
      : 委托展示 !== null
        ...
        : 推荐卡 !== null
          ...
          : 恢复中
            ? '正在恢复推荐信息…'
            : 坐标 !== null && 坐标意向快照?.阶段 === '失败'
              ? (坐标意向快照.error ?? '服务暂时不可用，请稍后再试')
              : '当前求职意向暂无这条推荐';
  ...
  return (
    <次级页外壳>
      <职位页面展示
        {...职位正文属性({ 岗: 视图.卡, P4视图: 视图, 是后端: true, 跳转, 公开企业, 推荐上下文 })}
        返回={安全返回}
        更多打开={抽屉展开}
        改更多打开={设抽屉展开}
        主按钮={{
...
// ── 职位正文属性收拢（连接侧共用）────────────────────────────────
// （旧「匹配核对的简历侧事实收拢」已随 JD 核对路径删除退役：匹配只吃记录的
//   已展开批次解释，准备函数不再读应用状态。）
...
function 职位正文属性({
  ...
  公开企业?: BFF公开企业 | null;
  /** C3 匹配上下文：连接层按 URL 四坐标恢复状态给（有来源/无推荐上下文/原推荐不可用） */
  推荐上下文?: 匹配分析上下文;
}): 职位正文展示属性 {
...
// 导航并调用两条准备函数的兼容连接器，直聊会话等调用方调用方式不变。
