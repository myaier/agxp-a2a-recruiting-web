...
export function 职位正文({ 岗, 藏直聊 = false }: { 岗: 市场职位; 藏直聊?: boolean }) {
  ...
  // 公司区块的加厚数据(2026-08-26):读公司档案;合成兜底档案(待补充)不算数,
  // 那种情况下区块退回原来的一行简介,不把"信息待补充"这类占位话摆到台面上
  const 档案 = 取公司档案(公司路由键(详.公司.名称));
  const 档案未补全 = 档案.规模行 === '规模与融资信息待补充';
  const 公司介绍段 = 档案未补全 ? null : 档案.简介[0];
  // 规模行约定格式「融资 · 规模 · 行业」;拆不出三段就不硬贴标签
  const 规模三段 = (档案未补全 ? 详.公司.简介 : 档案.规模行).split(' · ');
  const 成立年 = 档案.工商信息.find((条) => 条.项 === '成立日期')?.值.slice(0, 4);
  const 公司元行组: [string, string][] =
    规模三段.length >= 3
      ? ([
          ['融资阶段', 规模三段[0]],
          ['规模', 规模三段[1]],
  ...
  return (
    <div className={样式.内容}>
      ...
      {/* 公司(加厚,2026-08-26 用户指定):介绍段 + 融资阶段/规模/行业/成立/地址,
          数据读 公司档案;合成兜底档案(信息待补充)时退回原来的一行简介,不摆占位空话。
          黑底橄榄字的字标是品牌向的定稿样式。整卡可点,进公司主页 */}
      <button
        className={`${样式.卡} ${样式.公司卡} 可点`}
        onClick={() => 跳转(路径.企业详情(公司路由键(详.公司.名称)))}
      >
        <span className={样式.公司头行}>
          <公司字标
            首字={详.公司.首字}
            尺寸={40}
            圆角={14}
        ...
        {公司介绍段 ? <span className={样式.公司介绍段}>{公司介绍段}</span> : null}
        {公司元行组.length > 0 ? (
          <span className={样式.公司元表}>
            {公司元行组.map(([标, 值]) => (
              <span key={标} className={样式.公司元行}>
                <span className={样式.公司元标}>{标}</span>
                <span className={`${样式.公司元值} 单行`}>{值}</span>
              </span>
            ))}
          </span>
        ) : (
