...
export default function 在谈详情() {
  ...
  const 发叮嘱 = () => {
    const 内容 = 叮嘱草稿.trim();
    if (内容 === '') return;
    派发({ 型: '加叮嘱', 归属: 编号, 文本: 内容, 序号: 本单叮嘱.length + 1 });
    设叮嘱草稿('');
  };
  ...
  return (
    <次级页外壳>
      ...
      {/* 底部就地输入（2026-08-20 谈判页介入设计）：不跳页，写下的话作为「你的叮嘱」
          挂到当前阶段节点下，代理紧跟一条执行回执 */}
      <真输入条
        占位={
          单.阶段 === '需要协调' && 单.需要你
            ? '你的条件是什么？直接告诉代理'
...
/** 叮嘱块（2026-08-20 方案 A）：收在当前阶段卡内部的最后一块。
 *  不用聊天气泡 —— 时间线是文档语言，右对齐绿气泡会和它打架 */
