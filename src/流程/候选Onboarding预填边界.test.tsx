...
describe('消费位置判定（只有六个资料页消费，日常编辑标记写在 query 上）', () => {
  ...
  // 它与 resume 一样不属于预填消费位；离开状态页它是错配来源，判定与无来源逐字相同。
  it('from=intentions 只在状态页去掉消费位；错配来源与白名单外来源都按原判定', () => {
  ...
  // 对同一 URL 给出互相矛盾的语义（页面 /basic 只认 from=resume，其余按无来源渲染）。
  it('错配来源与无来源同判定：/basic?from=intentions 仍按消费位/活跃位处理', () => {
    expect(是预填消费位置(路径.基本信息, '?from=intentions'))
      .toBe(是预填消费位置(路径.基本信息, ''));
    expect(是预填消费位置(路径.工作经历, '?from=intentions'))
      .toBe(是预填消费位置(路径.工作经历, ''));
    // 状态页是 intentions 的合法落点：那里照旧去掉消费位
    expect(是预填消费位置(路径.求职状态, '?from=intentions')).toBe(false);
  });
...
describe('活跃 Onboarding 集合：以 Onboarding流程 为唯一事实源', () => {
  ...
  // 不活跃。离开状态页它是错配来源，活跃判定与无来源完全一致（不触发「已离开活跃集合」
  // 的引导状态清理，而页面也按无来源渲染 —— 两层同一语义）。
  it('from=intentions 只在状态页去掉活跃位；错配来源与白名单外来源按原判定', () => {
    expect(是活跃Onboarding位置(路径.求职状态)).toBe(true);
    expect(是活跃Onboarding位置(`${路径.求职状态}?from=intentions`)).toBe(false);
    expect(是活跃Onboarding位置(`${路径.基本信息}?from=intentions`)).toBe(true);
    expect(是活跃Onboarding位置(`${路径.基本信息}?from=intentions`))
      .toBe(是活跃Onboarding位置(路径.基本信息));
    expect(是活跃Onboarding位置(`${路径.基本信息}?from=evil`)).toBe(true);
    expect(是活跃Onboarding位置(`${路径.求职状态}?from=evil`)).toBe(true);
  });
});
