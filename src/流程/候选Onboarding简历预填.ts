...
/**
 * 经历缺项字段（DF-002）：卡片提示与保存拦截共用的唯一缺项判定。按原 经历未完成
 * 的比较值逐项判定（=== '' / === undefined，不把空串改成 trim 后空导致扩大校验）；
 * 公司名称空或组织编号 undefined 合并为一个「公司」项（名称与引用缺失不产生两条
 * 提示）；行业引用 undefined 算「行业」；顺序 公司→行业→职位→入职时间。
 */
export function 取经历缺项(段: 简历经历段): Array<'公司' | '行业' | '职位' | '入职时间'> {
  const 缺项: Array<'公司' | '行业' | '职位' | '入职时间'> = [];
  if (段.公司 === '' || 段.组织编号 === undefined) 缺项.push('公司');
  if (段.行业引用 === undefined) 缺项.push('行业');
  if (段.职位 === '') 缺项.push('职位');
  if (段.开始 === '') 缺项.push('入职时间');
  return 缺项;
}

/** 经历条目还有未选目录或缺失必填（公司/职位/入职时间/行业引用/真实企业 ID）。
 *  合同 C：解析或旧草稿的公司文本只作搜索词，缺 organization_id 就算未完成 ——
 *  绝不按解析文本自动匹配首命中或创建。复用 取经历缺项 的判定。 */
function 经历未完成(段: 简历经历段): boolean {
  return 取经历缺项(段).length > 0;
}
