...
describe('多角色 onboarding 合同', () => {
  ...
  it('选择实习时必须补齐时长和每周到岗天数', () => {
    expect(求职初筛缺失项({ 求职类型: ['实习生'], 办公方式: ['混合'] })).toEqual([
      '可实习月数',
      '每周可到岗天数',
      '实习开始日期',
    ]);
    expect(求职初筛缺失项({ 求职类型: ['实习生'], 办公方式: ['混合'], 实习月数: 3, 每周到岗天数: 4, 实习开始日期: '2026-09-01' })).toEqual([]);
  });

  it('校招主要方向必须采集预计毕业时间', () => {
    expect(求职初筛缺失项({ 求职类型: ['校园招聘'], 办公方式: ['现场'] })).toContain('预计毕业时间');
    expect(求职初筛缺失项({ 求职类型: ['校园招聘'], 办公方式: ['现场'], 毕业时间: '2027-06' })).toEqual([]);
  });
