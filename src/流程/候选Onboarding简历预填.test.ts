...
// ── DF-002：卡片缺项提示与保存首错定位共用的唯一缺项判定 ──
// 反例口径：按原 经历未完成 的比较值（=== '' / === undefined），不把空串改成 trim
// 后空导致扩大校验；公司名称空或组织编号 undefined 合并为一个「公司」项；行业引用
// undefined 算「行业」；顺序 公司→行业→职位→入职时间；数未完成项 仍只算未完成条数。
describe('DF-002 取经历缺项', () => {
  /** 已补齐的物化经历（与 数未完成项 组的 已补齐经历 同一口径）。 */
  const 完整经历: 简历经历段 = {
    编号: 'prefill:exp:0',
    公司: 'Example Systems',
    行业: 'Software',
  ...
  it('公司名在但组织编号缺 → 只报公司一项', () => {
    expect(取经历缺项({ ...完整经历, 组织编号: undefined })).toEqual(['公司']);
  });

  it('行业原文在但引用缺 → 只报行业一项', () => {
    const { 行业引用: _仅省略引用, ...无引用 } = 完整经历;
    expect(取经历缺项(无引用)).toEqual(['行业']);
  });

  it('公司名称空与组织编号缺合并为一个公司项；多缺项按 公司→行业→职位→入职时间 排序', () => {
    const 全缺: 简历经历段 = {
      编号: 'prefill:exp:1', 公司: '', 行业: '', 职位: '', 开始: '', 结束: null, 内容: '', 隐藏: true,
    };
    expect(取经历缺项(全缺)).toEqual(['公司', '行业', '职位', '入职时间']);
  });

  it('完整项返回空数组', () => {
    expect(取经历缺项(完整经历)).toEqual([]);
  });

  it('按原判定比较值：非空串（含纯空白）不算缺，不扩大成 trim 校验', () => {
    expect(取经历缺项({ ...完整经历, 公司: ' ' })).toEqual([]);
    expect(取经历缺项({ ...完整经历, 职位: ' ' })).toEqual([]);
    expect(取经历缺项({ ...完整经历, 开始: ' ' })).toEqual([]);
  });

  it('数未完成项 仍只算未完成条数而非字段数，教育/证书计数不变', () => {
    // 一条缺两个字段（公司无 ID + 行业无引用）仍只计一条
    const 公司与行业双缺 = { ...完整经历, 组织编号: undefined };
    expect(数未完成项([公司与行业双缺], [], [])).toBe(1);
    const { 行业引用: _仅省略引用, ...行业缺 } = 完整经历;
    expect(数未完成项([公司与行业双缺, 行业缺], [], [])).toBe(2);
    // 教育与证书的既有计数口径不受影响
    expect(数未完成项(
      [],
      [{ 编号: 'prefill:edu:1', 学校: 'A', 学历: '硕士', 专业: 'B', 开始: '2021-09', 结束: '' }],
      [{ 编号: 'prefill:cer:0', 名称: '', 年份: '' }],
    )).toBe(2);
  });
});
