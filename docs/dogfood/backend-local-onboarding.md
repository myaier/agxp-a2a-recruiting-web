...
- 目录中间层缺口：`selectable && has_children` 的节点无法表达「选此节点」——选期望职位左栏根条目、小类卡、引导问答行业卡、方向细选页细选项均只有单一点击动作。Backend 行为保留导航下钻、绝不猜选择；「能选中某个中间层节点」不得计 PASS，逐处记录缺口。
- 向导城市题分页（2026-09-14 起实际分页断言，替换旧「仅默认首页」缺口）：共享滚动区仍不暴露 onScroll/ref，但列表尾有「加载更多」按钮——默认态按 CN/TW/HK/MO 四支分页、各支带各自游标续页，失败支保留可重试（还有仍 true，可再点），初始加载与翻页显示中性「加载中…」不当无结果；搜索态经同一按钮翻搜索下一页。断言两个精选区标题「国内热门城市」「海外热门城市」（12+10 项，不含港澳台精选）、三个中文分组标题「台湾省 / 香港特别行政区 / 澳门特别行政区」且组内为 API 原名（英文/拼音）、搜索不带国家限制（全球搜索）、清空恢复默认目录且已选不丢。
...
- **Mock 基线无布局 diff（结构骨架口径）**：候选 onboarding 共享屏（完善资料、选工作城市、毕业院校等）与 Mock 同名屏使用同一布局组件——在 Mock 与 Backend 两模式各取**结构骨架**（可见交互元素的 tag 与首个 CSS module 类名，忽略文案与同类节点数量）对比，出现两套布局结构即为 FAIL。注意两侧数据不可能对齐：Backend 侧吃 fixture 数据（Fixture 市、预选档位），Mock 侧是本地数据（上海、未选），「相同展示数据的两份渲染」在设计上不存在；`e2e/J-PILOT-02接线.spec.ts` 已在拦截边界固化该骨架对比，真实 Backend 侧在此用页面观察复核。发现原型自身布局缺陷时取证交 PM，不自行改 CSS。
