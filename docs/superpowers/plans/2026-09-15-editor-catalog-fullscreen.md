...
日期：2026-09-15。版本：v1.2。当前阶段只交付文档，不实施产品代码。
...
- [ ] 实现 A 并测试焦点闭环、Escape 调用关闭；外壳卸载时不恢复焦点（恢复由父页按 A 负责）；不要修改共享弹层。
...
- [ ] 父基本信息输入/媒体/权限继续挂载；在公司分区编辑根层设置打开状态和兄弟子页，将含返回/保存栏的整个父表单 wrapper 隐藏。将现有后端行业区的查询 state、请求生命周期及回调上移至公司分区编辑根层，根层直接传展示 props 给兄弟子页；基本信息后代只保留字段行，不再持有行业查询状态。不使用 portal，不新增跨页查询层。按 A 记录/恢复滚动与焦点；只触及行业路径。
...
用户明确要求由 Claude 复审修正并给出最终结论，因此恢复同一 reviewer 会话。候选 HEAD `7e1ee8fd`、Plan blob `4f3f6ffa2fd085eb12102074c8f95a89cd01dc48`，冻结范围与批准 Spec 不变。轮前/轮后 HEAD、状态、指纹一致。Claude 确认 R1-1/R1-3 修复成立，指出两处残留歧义：

- R2-1：Minor / 真实缺陷 / required / 复杂度不变。接受并修复 Task 1 的旧“卸载恢复”文案；与 A 一致，外壳不恢复，父页显示后负责恢复。
