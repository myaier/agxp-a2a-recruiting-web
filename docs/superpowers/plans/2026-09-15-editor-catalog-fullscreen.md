...
日期：2026-09-15。版本：v1.0。当前阶段只交付文档，不实施产品代码。
...
- `查询Taxonomy('job-categories', { parentId?, q?, cursor?, limit:50 })` 延续现有签名：无 q/parent 返回根，有 parent 返回直接子项，q 是全局搜索、可命中父节点。使用现有返回类型的 `items/nextCursor/version`，以源码类型名核对，禁止改传输合同。
...
新增 `src/组件/全屏选择外壳.tsx`，命名导出 `全屏选择外壳`，props：`{ 标题: string; 关闭: () => void; children: React.ReactNode }`。使用已有 `返回栏`，`role="dialog"`、`aria-modal="true"` 与可访问名称；首次焦点在返回按钮，Tab/Shift+Tab 限于子视图，Escape 调用关闭；卸载恢复调用前触发元素（若仍连接）。父层负责隐藏自己，不给壳增加父状态/模式参数。
...
- [ ] 子页复用 A 和 `教育目录候选列表`，每次打开初始化当前显示值并加载可见候选；查询去抖/过期请求/版本保持现有合同，ID 标记选中；保留重试/加载更多。重复打开不能成为永久空列表。
- [ ] 不沿用父草稿作为临时搜索词；编辑搜索不取消父选择。后台模式未选有效项不得把自由文本当 ref。
...
- [ ] 正文改用 A、保留行业列表唯一滚动区；父经历编辑页 hidden 保持挂载，正文在兄弟位置。
...
在该文件提取私有 `公司行业选择正文`，只收 `{ 搜索词, 改搜索词, 行们, 加载更多, 关闭, 选定, 展开 }`；行展示模型含稳定键/名称/层级/可选/展开态/选中，保留已有公司目录父子导航，分页状态用页面现有父 ID。无 BFF/Context；Mock 行业池映射成可选行、原值不变，Backend DTO 在现有后端行业区映射。A 提供外壳；不新建跨页行业查询层。
...
- [ ] 父基本信息输入/媒体/权限继续挂载；隐去父区时不能隐去自己的子页；只触及行业路径。
...
- [ ] 使用视图 generation 防止根切换、搜索变化、关闭后的旧响应回写。页版本不匹配时放弃旧 cursor/缓存，重置该活动浏览/搜索视图并从首屏恢复，不混版本；不得清除页面已选 refs。
...
待对冻结 Spec/本 Plan 进行 Claude WORKFLOW_DOCUMENT_REVIEW；只审这两份文档及必要仓库规则，不审分支产品代码、不跑测试。裁决和修复在本节更新，不另建 review 文档。
