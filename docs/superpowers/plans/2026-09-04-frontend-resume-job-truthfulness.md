...
Backend 标题只按 `待补全.length` 显示“资料完整度检查 · N 处待补全”或“资料完整度检查 · 资料已补全”。不新增分组标题或布局节点：把两类结果按待补全在前、可提升在后投影到现有单列表，每行复用既有 `样式.诊断项/诊断项文/诊断项去`，只在文案内加类别前缀。证书只在“可提升”行。Mock 继续用“AI代理诊断”和现有原型标题，但不得让 Mock 的五技能阈值进入 Backend 结果。
...
Backend 的既有摘要 `<span>` 读取 `Backend摘要`；现有展开按钮只在 `Backend诊断项.length > 0` 时出现并读取 `Backend展开键文案`。列表行文字为 `${项.类别} · ${项.文案}`，尾部同一 `<span className={样式.诊断项去}>` 对待补全显示“去补 ›”、对可提升显示“去完善 ›”。不得在 `样式.诊断详情` 内增加标题、分组 `<div>` 或新 class。测试增加“零待补全 + 零证书”场景，断言标题含“资料已补全”、摘要含“可提升”、按钮为“看建议”，且 DOM 中不出现无类别前缀的裸建议。
