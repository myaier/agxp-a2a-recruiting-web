...
2. previous 为空 profile，next 仍空身份但带 `/basic` 本地姓名/生日，同时改技能或经历：跳过 profile，非 profile mutation 照常发送；
...
在 `候选操作.test.ts` 用不含本地姓名的权威快照覆盖三条水合路径：`保存简历` 成功、`保存个人优势` 成功、带 `权威简历` 的写失败。三者都应先派发 `水合后端简历`，再派发 `存简历`：基本信息取调用前的空身份草稿，其余简历切片取权威快照。身份已明确时不得补派发。在 `工作经历.test.tsx` 增加“空身份 + `/basic` 本地姓名草稿”的完整经历保存用例，断言页面仍把非 profile 修改交给 `操作.保存简历`，没有自行改成“在职”。在 `我的简历.test.tsx` 钉住空身份显示“未填写”且完整度包含“当前状态”；尝试改名时提示“请先选择求职状态”并跳 `路径.求职状态`，零保存。`个人信息.test.tsx` 对姓名保存增加同一空身份门，联系方式的现有独立保存不受影响。
...
在 `候选操作.ts` 增加同文件小函数 `水合简历并保留空身份草稿(快照, 本地基本 = 状态引用.current.基本信息)`：先记录 `本地基本.身份 === ''` 时的草稿，再派发 `水合后端简历`；有草稿时紧接着派发 `存简历`，基本信息取该草稿，经历/教育/技能/证书取权威快照。`保存简历` 成功时以 `next.基本信息` 作为第二参数；`保存个人优势` 成功和 `处理写入错误` 的权威水合使用调用前的 `状态引用.current.基本信息`。三处原有 `水合后端简历` 派发都替换为该函数。`后端状态.简历快照` 仍只保存服务端快照，helper 不写它。
