...
- [ ] 添加组件失败用例：两张折叠卡显示具体缺项；点保存打开第一条且第一字段控件获焦；修正或删除后重算；只有教育/证书缺项时不打开完整经历；已有草稿内容保留。运行下列 Vitest 选集加 `-t 'DF-002'` 确认红因。
- [ ] 提取上述局部 helper；列表卡按缺项数组显示“待补充：公司、行业”等已有样式可容纳的行。不得用公司为空推断权限或造企业值。
...
依赖：Task 2 后，无接口依赖。保留 `资料缓存快照` 和 action 形状；无需新 mode 参数：现有 Backend 专用 `水合账号资料` 路径可在合并前排除头像。先核对 action 所有调用点仍属于 Backend 水合；Mock 自身缓存路径不排除。
...
依赖：Task 4 后，无接口依赖。仅保存页面局部“本次文本是否来自有效建议”来源，不加全局 store 或持久化字段。保持现有 summary 初值函数签名及写入行为。

- [ ] 增加 `DF-004` 组件用例：无上传/无建议、空白/不可用建议、有效建议实际初始化、已有个人优势优先、独立编辑、用户清空、恢复有效建议。后到建议不能覆盖用户已有文本或让说明误称已应用。
...
- [ ] 非独立编辑且实际采用过有效建议且当前文本 trim 非空时显示原提取说明；其他注册流显示“可以介绍你的经验、技能和擅长的事情。”。独立编辑仍不传说明。恢复按钮实际应用有效建议时更新同一局部来源；清空当前文本立即回中性文案，不新增自动恢复行为。
- [ ] Mock 同样依据实际初始化/恢复来源，不因为存在 Mock 种子建议就声称已应用。保存个人优势、确认 summary、保存首次意向的原顺序不变。
...
生产者/消费者：候选 `准备Backend职位正文` 对当前 `视图.卡.对得上` 原原因数组应用映射；直取无推荐时给 [] 并保留原 null 分数。招聘 `从P4招聘推荐` 的既有 `亮点` 字段改用同一映射函数（仍是推荐原因，不是个人亮点）；匿名简历 Backend 入口传 `视图.亮点` 给可选 prop。不得改候选卡主体去消费这些原因，也不把它们写入 personal_highlights。

- [ ] 添加 `DF-011` mapper 用例：四码中文、重复稳定去重、未知/原型键不展示；分数 0 保留；推荐缺席与当前 scope 切换不从其他卡取值。
...
本文候选提交后执行审查，返回结果及逐项裁决记在本节；未完成文档 review 前不得生成执行提示词。执行交付仅一个 `docs/superpowers/prompts/2026-09-16-dogfood-frontend-fixes.md`，含 Claude Code/Codex 两节，各自完整 text 代码框；绑定最终文档 revision/blob，用 development-workflow 的 `scripts/validate_prompt_grading.py --plan ... --prompt ...` 校验。规划 session 不执行上述产品 Task。
