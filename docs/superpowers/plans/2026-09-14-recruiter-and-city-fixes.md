...
- [ ] **真实ID前置：** 读 Spec§6.2的22项及后端快照出处；定位执行环境提供的后端checkout，读其规则和local启动说明。按dogfood文档使用健康的测试栈/账号，GET `http://localhost:5173/api/v1/catalog/locations` 核对每项ID、国家、城市与可选返回。大陆CN首页仅作核验快捷路径；未命中的国内精选和海外精选都继续按各自countryCode分页，或按countryCode+API原名查询并翻页，直到命中精确ID或结果穷尽。首页未命中不等于不可验证。仅开发核验可以逐项查，运行产品禁止首载22次搜索。后端未运行时按现有启动文档准备，不能改其代码或占用/关闭他人的栈；外部前置缺失记BLOCKED。保存非敏感核验摘要和原始证据于 `dogfood-output/<run-id>/`，记录backend commit、catalog_version。
...
- **Task 4**（四国目录+精选+三入口）：提交 `13afd991` + 修复轮 `3ba6b1c5`。真实 ID 前置核验由主控完成：**22/22 命中**（catalog_version `cities500-2026-08-22-59d087eca781`，后端 `agxp-monorepo@bf1ae2fd` local 栈，经 5173 代理逐国分页比对；证据 `dogfood-output/2026-09-14-city-ids/`）。spec review（opus）首轮 ❌ 1 Important（引导问答缺清空恢复/全球搜索无 countryCode 页面级断言）+ quality review（opus）Approved 伴 1 Important（Mock 死按钮：查询禁用时 `还有` 恒 true）；fix round 1 修复两项，scoped re-review 全 ADDRESSED 无新破坏；定向 221 测试 + typecheck + 全仓 4981 全过。
- **宿主内部全局 review**（SDD final whole-branch，opus，30bb1e3c..3ba6b1c5）：裁定 **With fixes**——1 Important：默认目录四支网络失败完全静默（偏离 Spec §6.1「沿用现有轻提示」，旧实现的失败 toast 在本分支被删）；修复波 `2f00598a` 一并处理该 Important 与 3 个一行级 Minor（候选推荐 blocked 死分支、映射参数化行名与输入不符、本分支制造的 `热门城市` 死导出）。修复波 scoped re-review：4/4 ADDRESSED，无新 Critical/Important。14 项跨任务 deferred Minor 逐条 triage：13 项 accept-as-is，1 项（失败静默）升级为 fix-before-merge 并已修复。
- **候选 HEAD**（异构 review 冻结点）：`2f00598a`。
- **异构代码 review**（Codex，codex-review-loop FEATURE_BRANCH_REVIEW，冻结范围 `30bb1e3c..2f00598a`，绑定批准 Spec/Plan blob 与共用 review-contract；codex-cli 0.153.4，gpt-5.6-sol/high，read-only，未运行测试）：**Round 1 即精确 `NO FINDINGS`**，按合同结束条件收敛；post-round guard 状态/HEAD 无变化（thread 01a09e86…，证据 /tmp/codex-review-loop/session-omYlWuRy/）。0 required、0 optional。
- **完整前端验证**（证据 `dogfood-output/2026-09-14-verification/`）：`npm test` 218 文件 4986/4986；`npm run typecheck`、`npm run lint` 干净；`npm run build` 成功；`npm run ui:check -- --base 30bb1e3c` 18/18 场景 pass、0 warning/0 blocked/0 new/0 removed/0 infrastructure（report 模式 exit0，已读报告确认无任何差异告警，未改阈值/基准/标签）。
- **Backend/local 定向浏览器验收**（证据 `dogfood-output/2026-09-14-recruiter-city-fixes/`，3 独立 CDP Chrome 已清理）：推荐 unverified+ref 前提修复、正确 job_id GET 200、refresh POST 200、轮询进行、无认证 CTA=**PASS**；名片两入口未保存即改预览、清空反映空态=**PASS**；我页本人名/职务/诚实标签/首字 fallback=**PASS**；城市入口 1（选工作城市：四国首请求+各自 cursor 续页、12+10 精选、三中文组+API 原名、全球搜索、清空恢复）与入口 2（选择城市：主城市排除、加载更多键真实分页、海外精选 canonical ID 提交+权威回读 Singapore/Tokyo 双确认）=**PASS**。**BLOCKED×2**：①推荐列表成功展示——真实 BFF（bf1ae2fd）返回 `"recommendations": null` 违反其冻结合同（mobile-v1.yaml DiscoveryCandidatePage.recommendations 为非空 array），前端严格解码按合同如实报错，不冒充成功；②向导城市题 live 到达——社招/学生流程完善资料均强制城市、预填恒存在而跳过该题（该题行为由确定性测试覆盖，同款加载更多键已在入口 2 真实验证）。B01–B05/H01–H04 未跑，NOT_RUN。
- **Final gate 与合入事实**：用户于 2026-09-14 明确确认（候选 `b0b423a4`，pre-gate target `5825ff47`）。确认后 `git fetch` 记录 **final_target_base = `5825ff47`**（未推进），`git merge origin/main` 为 no-op；责任重算 `5825ff47..b0b423a4`，与确认前证据树（`2f00598a`）的 delta 仅 `docs/superpowers/plans/*.md`（零 L0–L2 失效，PASS_REUSED；L3 selection=none → N/A）；二次 fetch 复核 target 未推进后 **fast-forward push 成功：origin/main `5825ff47..b0b423a4`**。BLOCKED 两项保持记录在案（后端推荐 null 违约；向导城市题流程预填），不随合入冒称已解决。
...
- **Task 4 修复轮 2（同一批 findings 的重复派发，无新代码改动）**：重派的两条 Important（向导城市题清空恢复/countryCode 页面级断言、Mock 死按钮）即 fix round 1 已修复项（提交 `3ba6b1c5`，行号描述均为修复前状态），在当前树上复核仍成立：聚焦 引导问答+城市查询钩子 85/85、brief 定向 8 文件 225/225、typecheck 干净。未按本轮建议另立 fix commit——`3ba6b1c5` 已随 final gate 进入 origin/main，改写其主题需重写已推送历史。事故与修复（如实记录）：实施 agent 误对并行会话的 `fab0a355`（docs，已推送）执行了 `git commit --amend`，产生的游离重复提交（树无任何独有内容）已通过 reset 回 `origin/main` 完全丢弃；并行会话各提交内容零改动，本地 HEAD 与 origin/main 重新对齐于 `3f9efe4b`。
