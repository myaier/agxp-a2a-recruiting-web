...
export const 候选简历原件表: Record<string, 候选简历原件投影> = {
  ...
  'B-02': 建Mock原件({ 真名: '许清和', 手机: '139 0000 0004', 邮箱: 'qinghe.xu@example.com' }, 匿名简历表['B-02']),
};
```
...
const 原件投影 = 候.真名 ? 候选简历原件表[候.编号] : undefined;
...
`简历原件层` 增加可选的 `候选原件?: 候选简历原件投影`；存在时，`简历纸身` 从该投影读取真名、手机、
邮箱、自述、经历、教育和技能。不存在且传入 `匿名代号` 时继续走
现有打码分支；两者都不存在时仍是求职者查看自己的全局原件。三个分支复用现有 DOM 与 CSS，不新增解释文本。
