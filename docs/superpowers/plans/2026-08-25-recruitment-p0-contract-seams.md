...
继续按过时注释实现；这些注释修改不影响 DOM。`简历预览层.tsx` 文件头同步注明：候选原件投影分支不再
跟随求职端全局简历编辑，只有求职者查看自己的原件继续读取全局状态。`用户协议.tsx` 文件头“二、三、四节
不可改写”同步注明：本次二节变更由 P0 产品契约重新批准，后续仍不可自行改写。
...
首段职位和经验取投影；抬头职位沿用现有规则，只显示 `首段职位.split(' · ')[0]`，避免改变单行高度。工作经历
继续复用现有经历 DOM，以数组索引作 key，`起止` 整串进入时间槽、`说明`
...
`src/屏幕/企业真人会话.test.tsx` mock `use应用状态` 返回已经披露真名的 A-01，mock `use导航`，打开现有
“看简历”操作层并确认它消费候选投影而不是全局简历：
...
```bash
npm test -- src/数据/披露契约.test.ts src/屏幕/职位详情.test.tsx src/屏幕/企业真人会话.test.tsx src/组件/简历预览层.test.tsx src/状态/应用状态.test.ts
npm run typecheck
...
git add src/数据/模拟数据.ts src/数据/企业端模拟数据.ts src/数据/披露契约.test.ts \
  ...
  src/屏幕/匿名在线简历.tsx src/屏幕/企业真人会话.tsx src/屏幕/企业真人会话.test.tsx src/屏幕/真人会话.tsx \
