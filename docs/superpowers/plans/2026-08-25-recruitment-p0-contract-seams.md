...
- Modify: `src/数据/模拟数据.ts:1216-1227,1350-1535`
- Modify: `src/数据/企业端模拟数据.ts:162-282,529-895,928-1010,1519-1545`
...
- Modify: `src/屏幕/企业真人会话.tsx:52-80`
...
- Produces: `协议正文` 具名导出；S1 已递交 Mock 的 `真名` 和 PDF 原件事实；不再由阶段名称推导身份可见性的招聘方页面。
...
describe('招聘渐进披露契约', () => {
  ...
  it('已递交原件的候选有真名，S0 候选仍匿名', () => {
    for (const id of ['A-01', 'A-02', 'A-03', 'B-02']) {
      expect(在谈候选列表.find((候) => 候.编号 === id)?.真名).toBeTruthy();
    }
    expect(在谈候选列表.find((候) => 候.编号 === 'A-07')?.真名).toBeNull();
  });

  it('企业收到的 S1 附件不再声称匿名或隐去联系方式', () => {
    const 企业说明们 = Object.values(各候选阶段对话)
      .flatMap((各段) => 各段.递交简历 ?? [])
      .flatMap((条) => 条.附件?.说明 ? [条.附件.说明] : []);
    const 求职说明们 = [
      ...Object.values(各单阶段小结).flat().flatMap((条) => 条.附件?.说明 ? [条.附件.说明] : []),
      ...Object.values(各单阶段对话)
        .flatMap((各段) => 各段.递交简历 ?? [])
        .flatMap((条) => 条.附件?.说明 ? [条.附件.说明] : []),
    ];
    const 说明们 = [...企业说明们, ...求职说明们];
    expect(说明们.length).toBeGreaterThan(0);
    expect(说明们.every((说明) => /原件/.test(说明))).toBe(true);
    expect(说明们.some((说明) => /匿名|隐去/.test(说明))).toBe(false);
  });
});
```
...
```ts
// src/数据/模拟数据.ts
{ 编号: '递交简历' as const, 说明: '初筛通过才递交 PDF 原件，姓名与联系方式同时显示' }
...
// src/数据/企业端模拟数据.ts：只替换现有对象的 真名 字段
'A-01': 真名 = '沈亦舟'
'A-02': 真名 = '顾晚舟'
'A-03': 真名 = '宋之远'
'B-02': 真名 = '秦望舒'
...
// 双端所有 S1 附件
附件: { 文件名: '沈亦舟_简历.pdf', 说明: 'PDF 原件 · 包含姓名与联系方式' }
```
...
// src/屏幕/候选详情.tsx
文件名={`${候.真名 ?? 候.代号}_简历.pdf`}
// 简历原件层不再传 匿名代号
<简历预览层 文件名={`${候.真名 ?? 候.代号}_简历.pdf`} 关闭={() => 设看简历(false)} />

// 意向确认节点
待推进说明: 名 === '意向确认' ? '双方确认后进入真人沟通与约面' : '前一阶段通过后 AI 代理自动推进'
```

保留 `真名 === null` 时的代号和匿名样式。将 `简历预览层` 的“企业端匿名版”注释和调用分支删除；组件仍保留在线简历所需的匿名 `简历纸身` 变体，不能删除匿名在线简历。
...
`src/组件/简历预览层.test.tsx` mock `use应用状态`，渲染原件并断言真实信息：

```tsx
it('招聘方收到的 PDF 原件显示真实姓名和联系方式', () => {
  render(<简历原件层 文件名="沈亦舟_简历.pdf" 关闭={vi.fn()} />);
...
npm run typecheck
! rg -n "匿名版(原件|简历)|简历.*隐去.*联系方式|原件.*只在意向确认后|意向确认后.*(互换|披露|解除|必然是实名).*(身份|姓名|真名|联系方式|双盲|号码)|意向确认后.*(身份|姓名|真名|联系方式|双盲|号码).*(互换|披露|解除|必然是实名)|(已确认意向|双方已确认).*联系方式.*互换|确认后才.*(露|显示).*(真名|姓名|联系方式)" src --glob '!**/*.css'
! git diff --name-only | rg '\.css$|\.module\.css$'
git diff --check
```

Expected: 测试和类型检查全部 PASS；旧口径扫描无输出；`git diff --name-only | rg '\.css$|\.module\.css$'` 无输出。
...
在 `应用状态.test.ts` 增加 table test，覆盖每个将搬移的 action 至少一个代表：

```ts
it.each([
  ['候选资料', { 型: '存个人优势', 文本: '新的介绍' }, (状态: 状态) => 状态.个人优势],
  ['组织岗位', { 型: '存企业认证', 姓名: '陆知遥', 公司: '示例科技' }, (状态: 状态) => 状态.企业认证],
  ['隐私设置', { 型: '拉黑', 名称: '示例公司' }, (状态: 状态) => 状态.屏蔽名单],
] as const)('%s action 通过根 reducer 保持结果', (_域, action, 读取) => {
  expect(读取(归约(初始状态, action as 动作))).not.toEqual(读取(初始状态));
});
```
...
```ts
it.each([
  ['发现推荐', { 型: '切收藏候选', 编号: 'A-01' }, (状态: 状态) => 状态.收藏候选],
  ['MatchCase', { 型: '接受方案', 编号: 'J-02' }, (状态: 状态) => 状态.决策],
  ['Agent规则', { 型: '新增规则', 内容: '不接受大小周', 来源: '测试' }, (状态: 状态) => 状态.全局规则],
  ['消息', { 型: '读消息', 编号: 'M-01' }, (状态: 状态) => 状态.消息未读],
] as const)('%s action 通过根 reducer 保持结果', (_域, action, 读取) => {
  expect(读取(归约(初始状态, action as 动作))).not.toEqual(读取(初始状态));
});
```
