...
> 2026-09-09 更新（第二批「删筛选」）：在谈筛选层与顶栏「筛选 ▾」已整体删除，`candidate-me-overlay` 场景随之从 `e2e/视觉回归/场景.ts` 移除（清单 18 → 17）；上表与上方 ID 数组里的该行只作历史记录。「我」入口与「待你拍」跳转改由 `src/屏幕/我的.test.tsx` / `src/屏幕/在谈首页.test.tsx` 覆盖。
