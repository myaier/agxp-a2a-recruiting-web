...
### Task 2（2026-09-10）：职位正文展示与缺失投影

- 变更：新建 `src/屏幕/职位详情展示/`（`类型.ts` / `准备职位正文.ts` / `准备职位正文.test.ts` / `职位正文展示.tsx` / `职位正文展示.test.tsx`）；`职位详情.tsx` 的 `export function 职位正文` 收成兼容连接器（参数 `{ 岗, 藏直聊?, P4视图? }` 不变，直聊会话消费者零改动）；`职位详情.module.css` 仅追加 `.未知分数位`（44×44 中性分数位）、`.公司图位`、`.发布人图位` 三个缺失占位类，未删除/修改任何既有选择器（`.卡`/`.发布人头像` 等仍被 在谈详情 等借用）。
- TDD：先写两条新测试文件 + 抛「未实现」桩，`npm test -- src/屏幕/职位详情展示/准备职位正文.test.ts src/屏幕/职位详情展示/职位正文展示.test.tsx` RED = 32 failed / 32（全部指向 `未实现（Task 2 RED 桩）`，非模块加载错误）；实现后 GREEN = 114 passed（4 文件：新目录 2 文件 + `职位详情.test.tsx` + `直聊会话.test.tsx`）。首轮 GREEN 有 4 条断言失败，均为测试预期笔误（经验行年限不足且存在经历段时原口径是「不满足」；样本 办公地点 在场应出「办公地点」行；原认段规则把「机器人」含「人」归入规模；未补全档段来自一行简介），实现未改，已按原组件可观察输出修正测试。
- 全量：`npm test` 183 文件 / 3798 用例全过；`npm run typecheck` 退出码 0；`npx oxlint`（新目录 + 两个改动文件）退出码 0。
- Backend 缺失投影落位（Spec §3.2，仅 Backend 路径；Mock 已知字段/空值原样）：wire 无图源 → 公司/发布人中性图位（可访问名「公司图片未知」/「发布人图片未知」，不取 wire 派生首字充当真实图片）；公司元行 融资阶段/规模/行业/成立/地址 分别给对应未知；空简介给「公司简介未知」；发布人缺席生成占位对象（姓名/公司/职务/备注未知），在场时已知姓名/公司/职务照常、备注（wire 无字段）给未知；JD 合法空正文保留原标题并给「职位详情未知」/「职位要求未知」，事实行不掩盖缺失；详情直取无推荐分 → 说明分支保留 44×44 分数位尺寸的中性「—」占位（可访问名「匹配分未知」），不伪造 0 分；推荐卡真实分（含 0）仍渲染进度环。`职位详情.test.tsx` 仅按 Spec 更新了两条藏分断言（直取 → 占位在场 + 环缺席；推荐卡 → 「匹配分未知」缺席）并新增发布人占位断言，其余业务测试未动。
- Spy 证据：`准备Backend职位正文`（推荐卡与详情直取两路径）对 `取市场岗位详情` / `取公司档案` / `公司路由键` 零调用；Mock 准备按原静态档规则生成显式 `资料`（已补全/未补全、不足两段空元行、成立有无、重复段原顺序均有输出对照，且与 `公司区块` 原组件同一输入的 DOM 输出对照一致）。
- 视觉回归：`P1_CAPTURE_DIR=ui-regression-output/p1/task2 npm run test:e2e:data-source -- e2e/P1展示统一.spec.ts --project=mock-stg --grep 'P1 Mock视觉' --workers=1` 退出码 0，28 passed（36.5s）；`tsx e2e/视觉回归/比较命令.ts --candidate ui-regression-output/p1/task2 --reference ui-regression-output/p1/reference --output /tmp/p1-task2-report` → pass=28 / warning=0 / blocked=0 / 退出码 0 —— 职位与直聊覆盖层场景逐像素/几何与 Task 1 基准一致，消息基线（无关本任务）零变化。
- 禁改核对：`git status` 仅 3 个允许修改文件 + 新目录；`e2e/数据源模式.spec.ts`、`e2e/视觉回归/场景.ts` 未触碰；`git diff --check` 干净。未运行 `scripts/task_intents.py`（不在本仓库，登记属主控会话）。

### Task 3（2026-09-10）：职位页外壳与操作展示统一

- 变更：新建 `src/屏幕/职位详情展示/职位页面展示.tsx`（返回栏 ⋯ 入口 / 滚动区正文 / 浮动胶囊条 / 「⋯」更多操作抽屉这一份 JSX，正文复用 `职位正文展示`；不渲染 `次级页外壳` —— 附件选择 / 确认 / 举报层与加载 / 错误 / 404 页仍由连接层挂在外壳内，遮罩与抽屉的定位基准不变）；`类型.ts` 追加 `职位页面展示属性`（与冻结契约逐字段一致，主按钮/不感兴趣为内联对象类型）；`职位详情.tsx` 两条正常页（Mock / Backend）改消费 `职位页面展示`，原两份正常页 JSX（返回栏 / 浮动条 / 抽屉）删除（约 −180 行），新增连接侧共用 helper `收拢简历` + `职位正文属性`（无 hook，兼容入口 `职位正文` 与两分支共用；`职位正文` 参数与调用方式不变，直聊会话零改动）。CSS 零改动（所需 class 全部已存在）。`职位详情.test.tsx` 零改动。
- 回调语义：抽屉「不感兴趣」走 `不感兴趣.菜单按下`（收层 / toast「已不再推荐，代理会记住这类岗位」/ 返回 的原顺序在连接层回调里承载，Mock 逐字保留）；「举报这个职位」走 `举报`（收层 + 挂举报层都在连接层）；「取消」/ 遮罩走受控 `改更多打开(false)`。展示不拼路由、不读 Context、不区分数据来源；`主按钮/不感兴趣` 的禁用（无推荐坐标 / 写中 / 已委托）与 文案/已委托样式 全部由连接层计算，类名生成规则与原两份 JSX 等价（`已委托 ? 已委托态 : 动作可用 ? '可点' : ''` 收敛为 `已委托样式 ? 已委托态 : 禁用 ? '' : '可点'`）。
- TDD：先写 `职位页面展示.test.tsx`（8 用例：返回 / ⋯ 打开 / 抽屉三态与遮罩收层 / 菜单不感兴趣与举报各调指定回调一次且不改更多打开 / 浮动主按钮与圆钮各一次 / 禁用零触发 / 菜单可见=false 只剩举报与取消 / 正文与已委托态类透传）+ 抛「未实现」桩，`npm test -- src/屏幕/职位详情展示/职位页面展示.test.tsx` RED = 8 failed / 8（全部指向 `职位页面展示 未实现（Task 3 RED 桩）`）；实现后 GREEN = 8 passed。首轮 GREEN 前仅修一处测试 API 笔误（`getByRole` 无 `exact` 选项，改为字符串全等名），实现未改。
- 全量：`npm test -- src/屏幕/职位详情展示/ src/屏幕/职位详情.test.tsx src/屏幕/直聊会话.test.tsx` exit 0，122 passed（114 既有 + 8 新增；既有业务断言——404 不回 Mock、推荐坐标只取当前意向、空库/单文件/多文件、每次披露确认、取消/迟到结果、失败原地、成功禁用零 P5 导航、举报 target-not-found 重读——全部复用未复制）；`npm run typecheck` exit 0；`npx oxlint`（新目录 + `职位详情.tsx`）无输出；`npm test` 全量两次连续 184 文件 / 3806 用例全过（另有一次运行出现 1 条未复现失败，两次复跑与定向套件均稳定通过，未定位到与本改动相关的用例）。
- 视觉回归：`P1_CAPTURE_DIR=ui-regression-output/p1/task3 npm run test:e2e:data-source -- e2e/P1展示统一.spec.ts --project=mock-stg --grep 'P1 Mock视觉' --workers=1` 退出码 0，28 passed（35.6s）；`tsx e2e/视觉回归/比较命令.ts --candidate ui-regression-output/p1/task3 --reference ui-regression-output/p1/reference --output /tmp/p1-task3-report` → pass=28 / warning=0 / blocked=0 / new=0 / removed=0 / 退出码 0 —— 职位正常页（320/390 ×2）、更多层（320/390）、直聊覆盖层（320/390 ×2）逐像素/几何与 Task 1 基准一致，消息基线零变化。
- 禁改核对：`git status` 仅 `职位详情.tsx`、`类型.ts` 修改 + 新目录 2 文件；`e2e/数据源模式.spec.ts`、`e2e/视觉回归/场景.ts`、`e2e/P1展示统一.spec.ts`（只运行）未触碰；`git diff --check` 干净。未运行 `scripts/task_intents.py`（不在本仓库，登记属主控会话）。

### Task 4（2026-09-10）：双端消息列表与会话行统一

- 变更：新建 `src/屏幕/消息列表展示/`（`类型.ts` 按冻结「消息契约」逐字段 verbatim：`消息页签`/`未读展示`/`会话行数据`/`列表提示`/`消息列表展示属性`；`消息行映射.ts` 导出 `从Mock消息行`/`从Backend消息行`，角色标题反转、context 不可用降级、lastMessage=null「已建立真人会话」、MM-DD 短时间自 `P7/Backend会话列表.tsx` 原样移动，Mock undefined/0/正数 → 无/红点/数字 而 Backend 0/正数 → 无/数字；`消息列表展示.tsx` 导出 `消息列表展示`/`会话行`，标题行+放大镜聚焦、页签行、搜索条、前置/行/后置/加载更多一份 JSX，原 class 与声明值逐字沿用 `消息列表.module.css`，代理标/字标都在原 46px 容器，行上无业务按钮）；`消息列表.tsx`、`企业消息.tsx`、`P7/Backend会话列表.tsx` 三入口改消费同一展示，原两份重复列表/行 JSX 删除，页签/搜索 useState、过滤顺序、S3 门控、reducer 未读快照、Mock 点击派发读消息后导航、Backend 登记注销/force 首读/分页/重试/0 未读不派发本地已读全部留在连接层。`rg -n '企业消息.module.css' src` 零消费者后已删除该文件（唯一消费者是企业消息.tsx 的 import）。
- TDD：先写 `消息行映射.test.ts`（表驱动 7 用例：未读三态、字段透传与按下同引用、首字/底色缺省、Backend 未读 0/正数、两角色反转、context 不可用、lastMessage=null+时间+测试标识+点击透传）与 `消息列表展示.test.tsx`（12 用例：放大镜聚焦输入、输入/页签受控与选中类、搜索提示透传、文字行+数字徽标+testid、红点/无标记、代理与字标头像容器与整行按下透传、超长标题/副标题/摘要 class 保留、空副标题留节点不造假姓名、rerender 有值→空切换无残留且时间仍可见、前置错误+缓存行+后置提示共存+重试透传、加载更多存在才显示、多行提示），+ 抛「未实现」桩，`npm test -- src/屏幕/消息列表展示/` RED = 19 failed / 19（全部指向 RED 桩，非模块加载错误）；实现后 GREEN = 19 passed。GREEN 前修正两处：`../组件/*` 导入深度（新目录多一层）与两条「文案<br/>文案」多行空态断言改按共享 `空态` class 的 textContent（原 DOM 结构如此，实现未改）。
- 集成断言：`消息列表.test.tsx` 新增「Mock 行真实消费共享展示」（代理/字标头像容器、未读 undefined/0/正数三态标记、页签三态过滤、搜索过滤与无命中后置提示、点行派发 读消息 + `/chat/human` 既有断言保留）；`企业消息.test.tsx` 新增 S3 门控后真人行共享 class + 点击派发 企业读消息 + `/hr/chat` + 招聘端搜索提示与过滤（既有 S2/S3/终止三分支门控断言零改动）；`P7/Backend会话列表.test.tsx` 新增真实渲染共享展示断言（`会话行`/`页签选中` class、中性「会」头像、unreadCount 正数徽标、错误与缓存行共存+重试 force 刷新），既有 10 用例（角色映射、参数路由且绝不派发读消息、未读 0 无标记、context 不可用、last_message=null、本地搜索服务端顺序、通知空态、加载/空/失败重试、加载更多、可见范围登记注销）零改动通过。
