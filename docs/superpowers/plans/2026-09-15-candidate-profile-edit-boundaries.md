...
- 工作区 `.`，仓库 `myaier/agxp-a2a-recruiting-web`；目标 `origin/main`。规划代码基线 `2312cba5a68f8c2156872dadd0633230330cd4b0`。仅文档规划，无产品验证 PASS。
...
- 两个先行分支的相关实现已在代码基线中核对：基本信息出生年月使用年月滚轮层；实习／毕业／薪资使用共用数字及薪资抽屉；教育学校／专业、经历行业使用全屏目录。无需等待、合入或 cherry-pick 旧分支。开工核对 `git merge-base --is-ancestor 2312cba5a68f8c2156872dadd0633230330cd4b0 HEAD`，缺基线停止相应实施并说明，不悄悄回退旧版组件。
- 已有城市修复：`docs/superpowers/specs/2026-09-15-editor-catalog-fullscreen-design.md` §5.6、对应 Plan Task 6 已有生产实现及测试；保留 `规范精选城市名称`、`精选城市显示名` 和四处消费者。城市不再是未完成外部依赖；本任务只回归受改动影响的消费者。
- 样式保护：`src/组件/公司选择层.tsx` 以 `抽屉样式` 导入工作经历 CSS，清理标签样式前核对全仓库消费者；不得删除其遮罩/选择层/抓手/标题和动画。已合入修复 `6b7f991b` 明确恢复了这些共享规则，不重复引入回归。
...
|---|---|---|---|---|
|1|简历编辑来源与保存出口；基于已合入资料页|Top 5–10（Claude Code: sonnet）|Top 5–10（Claude Code: sonnet）|Top 5–10（Claude Code: sonnet）|
|2|意向私有筛选文本；保留已合入抽屉，串行避免共享文件竞争|Top 5–10（Claude Code: sonnet）|Top 5–10（Claude Code: sonnet）|Top 5–10（Claude Code: sonnet）|
...
**零上下文漂移风险：中。** 恢复边界与多个共用编辑场景仍需细致区分；并行依赖已合入，产品输出与数据契约已冻结。
...
- [ ] 核对资料／日期控件与测试已在实施基线；读取四屏、预填边界及应用调用。核对当前出生年月为点击行加年月抽屉、经历行业为全屏选择，不恢复旧内嵌轮或底部行业层。
...
- [ ] 浏览器现有“核心编辑 简历行业”用例从我的简历点击进入，保存并重入验证；保留公司/行业目录 ID 与原负例，不用裸 `/experience` 冒充日常入口。当前行业已是全屏 dialog，沿用返回／点选关闭行为；现有 `@catalog-fullscreen` 的“选择学校/选择专业/所属行业”用例只校准日常入口与整页保存落点，不删子视图取消零写与ID断言。新增小用例标签 `候选资料编辑边界 @backend`：已完成候选经基本信息保存回简历，首次意向写入零次；通过真实 UI 流程进入，fixture 使用现有安装函数，不导出新的模拟框架。
...
npm run test:e2e:data-source -- e2e/数据源模式.spec.ts --project=backend-stg --grep '核心编辑 简历行业|候选资料编辑边界|选择学校.*选择专业.*所属行业' --retries=0
```
...
- [ ] 读取添加意向草稿、保存与映射，核对当前毕业年月／实习抽屉调用，文本区替换不得删除这些既有调用。
...
- [ ] 实现受控 UI，两处调用保留各自数据处理函数；删除被迁移且经全仓库确认无消费者的旧证书行/技能卡 CSS，保留公司选择层跨文件消费的抽屉骨架及相邻区块间距。新组件不查询后端、不创建证书 ID。
...
3. 浏览器最小联合选集：`npm run test:e2e:data-source -- e2e/数据源模式.spec.ts --project=backend-stg --grep '候选资料编辑边界|候选私有筛选要求|候选个人优势编辑|核心编辑 简历行业|选择学校.*选择专业.*所属行业' --retries=0`；先同命令 `--list` 确認非空。首次建档 consumer 为 `npm run test:e2e:data-source -- e2e/J-PILOT-02接线.spec.ts --project=backend-stg --retries=0` 与 `npm run test:e2e -- e2e/onboarding.spec.ts --retries=0`，前者覆盖恢复/写入，后者覆盖Mock正常出口。上游改变控件时先更新本任务实际消费者，不放宽业务断言。浏览器检查390与1280宽下标签长名称、证书年份、个人优势入口、文本框及代理卡，不做无目的全量视觉基线更新。
4. 城市回归限定在本任务实际修改的 `引导问答.tsx`：用其既有城市测试验证 ID/名称兼容及个人优势编辑分支不会写城市草稿。Task 4 已跑 `src/屏幕/引导问答.test.tsx`，输入有效时复用，不另跑两张未改城市页测试。若实际 diff 扩大到共用城市配置/选择正文，才增加 `src/数据/城市精选.test.ts`、`src/屏幕/选工作城市.test.tsx`、`src/屏幕/选择城市.test.tsx` 中受影响选集并记录原因。不重复实施22项名称校准，不将本次源码核对声称为真实后端验证。
