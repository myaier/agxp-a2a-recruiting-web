...
- [ ] 更新/新增失败断言：以 Spec §10.5 完整原文与两岗位样本覆盖 Markdown；按三类单独、混排、重复类型/空列表检查标题、数量与顺序；查询时间不显示，Agent 时间只在整条气泡之后出现一次。
- [ ] 用 `npm test -- src/组件/问AI代理/查询结果展示.test.tsx src/屏幕/问AI代理.test.tsx` 定向确认旧纯文本/查询时间/卡外理由失败。保留导航、禁用委托、空态、不可用项和解读动作原断言，替换已被用户明确取消的脚注断言。
