...
|数据源模式.spec.ts:7899,8374,8412,8479,8550,9029,9152,9199,9252,9291,9430,9658,9772,10102,10189,10720,11653,12911,12966,13065,13142,13433|22|共享档案阻断的角色切换／发现／Case／会话等|2；后续相应 Task|Task 2 已实施 2026-09-15：21/22 完整通过（P6 7899；P4 8374/8412/8479/8550；P5 9029/9152/9199/9252/9291/9430；在谈 9658/9772；P7 10102/10189；P8 10720；卡片 12911/12966/13065/13142；S0 13433）；11653 水合已恢复但卡名片旧「公司」自由输入（`getByLabel('公司')` 已不存在，公司改选择控件），归 Task 3（其选集含「核心编辑 岗位」）。Task 3 已实施 2026-09-15：11653 入口改应用内名片（注册流入口被已完成账号守卫弹回主壳），公司经 公司选择抽屉 选中后保存，再进发岗向导（档案 organization_ref 按 ID 读回的默认行）；POST body 断言补 direct 双 ref=组织甲编号；其余 21 条不受本次共享输入改动影响，attempt-5/6 仍完整通过。Task 6 已实施 2026-09-15：Task 3–5 对共享文件再变更（experiences handler 对齐合同 C、educations PATCH/GET intentions 新路由、`断言经历写入` 键集收紧）后，本组除 11653（Task 3 run-5 已验）外的 21 条按 Task 2 记录原选集补验：consumer-repair-1 21/21；同任务把本文件 11 处静默失效的 `test.use({timeout})` 死行改为 describe 级 `beforeEach(() => test.setTimeout(原值))`（前提更正 2026-09-15：两配置均未设 test timeout——仅 webServer 有 120s——生效缺省为 Playwright 30s；11 处均由 30s 恢复为原声明预算 60/90/120/150/180s，全部为抬升，无「120s 同值不变」行）后重算：consumer-repair-2 21/21、其余 18 条（Backend 数据源 fixture 9、P6 3、在谈 Mock 双端 2、标注 2、J-PILOT-01 场景一/二）consumer-repair-3 三项目 18/18（原「受预算变化影响的其余 18 条」按 120s 同值不变推差量，前提不成立——抬升覆盖全部 11 处；18 条补验实际执行且全绿，此处仅保留补验事实）。另记录：P1C 招聘组织 fixture 与 P3 Backend 恢复分派两 describe 的 setTimeout 恢复不完整（5/15、3/7），其余用例维持 30s 缺省（与分支前一致，无回归）。round-1 review 处置 2026-09-15：批量抬升违反 Plan「不用批量抬超时」——上述 11 处 describe 级 hook 与逐测试 setTimeout（P1C 组 5 处 60s + 名片 3 处 60s、P3 主链路 150s、P3 恢复分派 3 处 90s、招聘方 onboarding 与 JD 导入各 120s，均无单条用例超时复现证据）已全部移除，恢复生效的 Playwright 30s 缺省；grep 确认相对 8e333ce3 零新增 setTimeout，受影响 describe 全量用例以 30s 缺省重放验证（review-r1-timeout）|
|数据源模式.spec.ts:118,7636,7786,7841,7950,8018,8043|7|规则生命周期、恢复与隔离|4|Task 4 已实施 2026-09-15，run-6 全绿（15/15，含 3 条保留举报 consumer）。基线实况：8 失败（118、7636→全链路、7841→accept 响应丢失、7950→失败提案卡、8018→首次水合、8043→accept 409、10522、10691）；7786→版本冲突一坐标基线已自行通过（现选择器本就匹配当前 UI，未改动）。按标题距推断的旧坐标映射与现存测试序一致（版本冲突从未失败，重复 cursor 不在这 7 个坐标里；后者的空转断言仍顺手校正：旧「手动添加规则」count-0 换成现行「添加规则」并删掉恒真的「2 条」count-0 死断言）。118：删「4 条/3 条生效」总计与旧「Mock 招聘端零开关」假设（产品已改 Mock 本地可维护），改分区标题（你教它的规则／哪些情况直接排除）+ 精确开关标记（`规则：<正文>` aria-checked）验证；Mock 意向级种子「双休是底线…」改断言本页不渲染且无 编辑／删除／开关 三入口；招聘端改断言开关 checked 与「添加规则」在场。7636 全链路：水合段改精确正文按钮（exact 避开 `显示删除：⋯` 同文键 strict 冲突）；删意向 create 段（`规则范围` 选择器已退役，`意向新建草稿` 标记随之删除）与全部「N 条」总计，改 composer 无范围选择器 + 意向规则三写入口 count 0；「手动添加规则」→「添加规则」；编辑=正文按钮→`编辑规则：<正文>` 预填→「完成」（旧「提交修改」键退役）；归档=键盘揭起 `显示删除：<正文>`（触屏布局 ⋯ 是 1×1 可达键，click 会被正文键拦截，走 P7 同款 focus+Enter）→`删除规则：<正文>`→确认前断言零 DELETE→确认层「删除这条规则？」→「删除」→权威回读（行+开关消失、DELETE 恰 1 次带 If-Match "1"）；「两次创建两把 key」断言随意向 create 删除移除（由 失败提案卡 的两次创建断言承载，无覆盖损失），接受恰 2 次（新建+替换）空 body+各自 key 保留；招聘端 create(no scope)→accept→pause/resume If-Match "1"→"2" 版本链全保留。7841：两处规则行断言改 exact 修复与 `显示删除：⋯` 的 strict 冲突。7950：「手动添加规则」→「添加规则」，删「规则范围」值断言（选择器退役）；创建失败文案按现行合同改「后端服务暂时不可用，请稍后重试」+ fixture 原始 message 不上屏 count 0（该段在基线被旧入口超时掩盖从未跑到，属本次新暴露的过时断言），两次创建新 key 保留。8018：挂起段旧入口 count-0 改「添加规则」；放行段删意向可见与「2 条」，改全局行+开关落地、意向规则不渲染零写入口。8043：「2 条」改 候选全局规则行在场 + 不可执行提案未物化（规则计数不变的意图由清单内容承载）。请求协议保持：POST 提案（候选恒 scope global／招聘恒无 scope）、replacement If-Match、accept 空 body+key、DELETE If-Match、PATCH pause/resume 版本链、503 同键受控重试、409 not_actionable 权威恢复零重放、首次水合无写入口、切角色迟到响应隔离、Mock 零 API 全部原样存证。Task 6 已实施 2026-09-15：P6 describe 失效的 `test.use({timeout:60s})` 死行改 beforeEach 恢复原 60s 预算（本组内无自身 setTimeout 的两条——7786 版本冲突现位、8043 accept 409 现位——基线耗时 15.5s/6.6s，预算充足），随 consumer-repair-3 重验通过|
|数据源模式.spec.ts:6782,12037,12102,12244,12409|5|公司选择代替旧输入；保存／重入|3|Task 3 已实施 2026-09-15，5/5 完整通过（run-5 attempt-5/6）：6782 见上行（公司输入断言替换为选择入口按钮 + 抽屉取消零写入 + 空值保存零请求）；12037 公司名称改按钮选择（Mock 走本地 模拟企业目录 全程零 API，选 云衢科技），`placeholder='必填'` 断言 2→1（只剩职位名称，标题保留），行业层根集按当前字典更新（互联网平台、智能硬件 / 制造，根可访问名带「⌄」去 exact），根仅展开→点金融科技→选细分叶「支付与清结算」，经历卡回读断言随所选叶更新；12102 公司名称抽屉选中（磐石信息→org-fixture-p3-manual-a），经历 POST 断言补 organization_id（industry_id 原有），共享 `断言经历写入` 守卫对齐合同 C（organization_id 进闭合键集、company 键退役），handler 存储 organization_id；12244 公司抽屉选中组织甲，首写 PATCH If-Match "0" body 补 organization_ref，发岗走 `走完后端发岗向导`（helper 增补当前产品必需的「用人企业」显式选择步）断言 direct 双 ref，刷新恢复保留；12409 名片抽屉选中，consent 前零 POST、202+串行轮询、建议不自动发布全保留，POST body 断言补双 ref（默认读取行），旧「全远程禁用办公地」断言按当前合同替换（输入仍可用、值为空、非必填——src/屏幕/发布岗位.tsx 办公地点注释为现行事实）。round-1 review 处置 2026-09-15：恢复保存／重入覆盖——experiences POST fixture 按 organization_id 从既有组织目录（组织 fixture + 隐私搜索池）反查 display_name 冻结进 company（对齐 src/数据/后端映射.ts「company 是服务端冻结展示快照」语义），无 organization_id 维持空串；fixture 补 PATCH /me/resume/experiences/{id}（教育 PATCH 同款，重冻结 company）；「三层目录…经历按 ID 保存」@backend 用例扩展 保存→刷新重进→经历卡与编辑页公司行显示所选企业（Fixture 磐石信息）→完成可用→改职位再保存 PATCH 仍提交同一 organization_id；候选 onboarding 用例「空串快照」断言按新行为改为 company=所选企业名（Mock 零 API 用例不动）|
...
|数据源模式.spec.ts:10253,10353|2|账号时间与导出完整链路|1,5|Task 1 部分已实施 2026-09-15：两配置 `use.timezoneId:'UTC'` 缺省兜底，suite 自带 `Asia/Shanghai` 未覆盖。Task 5 已实施 2026-09-15（纯验证，零代码改动）：run-8（宿主 `TZ=Asia/Singapore`，backend-stg，2/2 通过）证实 `P8 账号安全首屏`（`创建 2026-09-01 08:00 · 失效 2026-09-08 08:00`）与 `P8 数据导出：创建无 body`（创建无 body → 轮询 ready → 关闭重开恢复 GET → 同源流式下载，`2026-09-08 00:00 前可下载`）在宿主 UTC+8 下仍按配置内 UTC 上屏，产品代码未写任何时间强制 UTC，原 UTC 文案与完整导出链路断言原样保留|
...
|数据源模式.spec.ts:10896,11391|2|候选完成与城市正确分组|5|Task 5 已实施 2026-09-15，run-7 attempt-5 全绿（3/3：候选完整 + 城市 @mock + 城市 @backend；attempt-1/2/3/4 为修复过程留存）。10896 候选 onboarding 完整：原位修复完整建档，未删旅程。① Backend 初始偏好为空（空求职初筛偏好）：删「默认三档全亮」注释与「点掉混合」倒置点击；「已毕业」断言不替选求职类型（社招全职 aria-pressed false→显式点击→true），办公方式显式点「现场」（false→true），并把显式选择钉进意向（recruitment_type social_full_time、workplace_modes ['onsite']、graduation_month null）。② 经历写入按合同 C 原位迁移：删旧「公司」自由文本输入（`placeholder='必填'` 2→1 只剩职位名称），公司走 公司选择抽屉（抽屉搜企业并选中 磐石→Fixture 磐石信息，搜索池给 P3默认组织库），POST/权威快照断言补 organization_id=org-fixture-p3-manual-a（共享 断言经历写入 守卫已按闭合键集拒绝旧形状）；经历卡断言按当前合同替换——company 是服务端冻结展示快照、stub 不反查组织名落空串，卡上验证职位名 + 行业标签（不伪造公司名展示）。③ 就读时间段：删「滚轮默认 2021—2025」旧默认假设，按年份轮可空合同滚 2020/2024（滚轮交互即确认）再 下一步；共享滚轮 helper 滚薪资轮 更名 滚滚轮（薪资/年份内嵌滚轮共用，行为零变化，picker 就读年份 两用例同步改调用名）。④ fixture 补齐当前合同缺口（旧用例首红从未跑到的段）：Onboarding 域新增 `PATCH /me/resume/educations/{id}`（同 id CAS、断言教育写入、整册权威快照回读）与 `GET /me/intentions/{id}`（完成核对 exact ID 权威回读、404 未知识意），均与 J-PILOT-02 fixture 同款（src/数据/招聘数据源/简历.ts 消费合同为据）。⑤ 首次意向 POST 恰一次、reload 权威重读 poll、完成事实回读（last_used_role candidate、complete POST 推进）全部保留；工资滚轮后退恢复（goBack 30/40 → goForward）为旧长旅程独有责任，保留本用例。J-PILOT-02 对照结论：零经历、URL 三态、教育恢复、头像 unknown 四项责任由其自身用例承载（run-9 5/5 通过，含社招 637/学生 712/教育恢复 765/头像 unknown 805），两文件无重复长旅程，无删除。11391 核心编辑 城市 @backend：Location 桩本地 helper 省份城 增显式 `admin1_code` 参数（浙江 33、江苏 32），杭州/苏州各归其省、宁波/绍兴归浙江；按 admin1_code 聚合的 浙江/江苏 双组标题断言保留（未放宽为「出现任一省名」），默认/翻页/搜索/取消/保存回读与 loc-* ID 断言原样|
|展示字段接线.spec.ts:566（390）|1|主体切换后 claim-only 名称，复测过但原因未明|6|Task 6 已实施 2026-09-15：run-10（brief 原命令 repeat-each=3×两宽度、retries=0）首次采样即复现 1 败/5 过——`getByText('展接FIX 星河科技')` 5s 超时（本次落于 320 宽度），trace 链：URL 已到 job_…02、详情标题与 0 分上屏、organizations 计数与切换前持平（claim-only 零补读合同未破），详情公司块/发布人渲染的是 B 卡 claim 名。根因为组合缺陷、非产品缺陷：① fixture `候选岗位()` 把 `hiring_organization_claim` 写死 A 卡声明（覆盖类型无 claim 键，B/C/D 全错挂），而 claim-only 详情公司名按合同取 claim（`src/数据/发现推荐映射.ts` 建候选岗位视图），`星河科技`（organization.display_name）只上市场列表卡——断言历来靠「URL 已变、列表 DOM 未提交」窗口被列表卡误满足，详情先提交即失败；② 修复仅测试/fixture 侧：覆盖类型增 `hiring_organization_claim`，B/C 构造点随卡显式声明（企业B/企业C；D 无断言消费者不动），B/C 导航段在 URL 断言后先等详情独有 JD 卡标题（岗位信息与职位详情）落定再断言公司名，A→B 切换旅程保持。修复后 run-10-attempt-2 6/6、run-11（展接线 backend 全文件＝fixture 全部消费者 5 用例×两宽度）10/10；src/ 零改动|
