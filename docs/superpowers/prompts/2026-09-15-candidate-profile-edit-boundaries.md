...
文档审查完成；选择对应宿主章节复制到新实施会话。

## Claude Code

```text
你在 Claude Code 的全新实施 session 中，为 repository myaier/agxp-a2a-recruiting-web 执行已批准并完成文档 review 的独立交付 Plan。宿主任务工作区 . 固定为当前仓库根 `.`，复用不新建：不创建第二个用户工作区，不自动 stash/reset/clean 已有内容。target origin/main；实施范围 简历编辑保存边界、意向私有筛选文本、技能证书共用标签、个人优势独立编辑、代理卡固定在线展示与真实数字；城市仅外部分支整合验收，不重复实施；输入输出用仓库相对路径，不写绝对目录、file URI 或 home 路径。
...
按顺序读取当前仓库实际存在的规则、批准 Spec：docs/superpowers/specs/2026-09-15-candidate-profile-edit-boundaries-design.md（revision 3c48e7d5f3f12056a71f9f6d4b5c5cb1a56b9913，blob e06bc1beffb185b39b7978a0f885dbaa375a0af2）、执行 Plan：docs/superpowers/plans/2026-09-15-candidate-profile-edit-boundaries.md（版本 049b73086603a31d2a27d4e7dd176e3dbf7e5517，blob 09cfc41a426d4a958db309847c2d514dd170ddc1）的 Global Constraints、Task index、角色表与当前 Task 完整正文（含预期新增/修改/删除文件清单），及 逻辑 development-workflow skill 根目录的 assets/execution-contract.md；不一次读取其它 Task 正文，批准版本用 Git 对象核对。
...
开工核对并行依赖：investigate-drawer-background-shift 与 audit-option-components-editors 的最终相关实现及测试须在所用基线中存在；仅有文档或中途快照不算完成。按 Plan 的 Global Constraints 处理缺口，不擅自合入未完成分支。Task 5 可独立先行；城市不重复修复。
```

## Codex

```text
你在 Codex 的全新实施 session 中，为 repository myaier/agxp-a2a-recruiting-web 执行已批准并完成文档 review 的独立交付 Plan。宿主任务工作区 . 固定为当前仓库根 `.`，复用不新建：不创建第二个用户工作区，不自动 stash/reset/clean 已有内容。target origin/main；实施范围 简历编辑保存边界、意向私有筛选文本、技能证书共用标签、个人优势独立编辑、代理卡固定在线展示与真实数字；城市仅外部分支整合验收，不重复实施；输入输出用仓库相对路径，不写绝对目录、file URI 或 home 路径。
