...
仅在新实施 session 使用。Spec v1.0 已批准；Plan v1.1 完成两轮 Claude 文档 review；第二轮确认两项修订并返回 NO FINDINGS。当前规划 session 没有实施产品。两个宿主引用同一精确文档版本。
...
严格执行 execution contract 与 Plan 当前 Task brief，不从本提示词补充产品设计。用户明确要求：接受review finding并修改实质内容后，必须交回原reviewer复审，不能仅由主控自行认定已修好后结束。
