...
**前端设计基线：** `origin/main@280f83ef0670d07465e87fd56a2a4b0b843be04e`（已包含 FE-MC-01）
...
`后端状态.候选实名` 在共享类型上定义为可选字段，Provider 则始终用 `创建空候选实名快照()` 显式播种；operation 和页面统一通过 `后端状态.候选实名 ?? 创建空候选实名快照()` 读取。可选只用于兼容仓库内大量聚焦其它域的显式测试桩，不表示运行时存在第二种状态形状，也不得让页面自行构造不同默认值。这沿用既有 `候选预填状态?` 的迁移方式，可避免为了 FE-IV-01 机械修改 FE-MC-01 等无关测试。
...
设计校准时 FE-MC-01 已合入基线。它与本任务唯一共享的产品文件是 `状态/后端/类型.ts`：现有 `P5MatchCase状态.P5摘要`、`P5摘要快照` 和 `MatchCase操作.加载摘要` 必须原样保留。本任务不触碰 MatchCase data source、operation 或页面；`HTTP招聘数据源.ts` 与 `应用状态.tsx` 均以已合入版本为直接基线新增实名组合，不再保留“并行分支后续合并”的假设。
