...
院校查询结果不能套用只含 `id/display_name` 的通用目录项后丢弃字段；查询层须保留 BFF `InstitutionItem.location`，至少把其中的城市显示名、国家代码和国家显示名交给院校候选行。`location` 只服务于搜索结果消歧，不进入 Education 写入；用户点选后仍以 `{id, display_name}` 作为合法目录引用随教育草稿保存。
...
- 学校默认进行全球搜索；每条候选复用现有副行显示“城市 · 国家”以区分同名机构。搜索结果必须保留 `InstitutionItem.location`，不能在通用目录适配时压平成只有校名。
- 首版不增加“先选城市再选学校”的步骤，也不增加国家选择页；城市只作为候选的辅助显示和同名消歧。将来确有收窄需求时，现有查询能力可传 `country_code/location_id`，本轮不暴露新的筛选 UI。
- 后端配套 spec 保证位于 CN Location 的 active 学校以中文 `display_name` 返回；其他国家沿用 ROR 来源展示名。
- 用户继续输入后，清除旧引用；未选择候选时阻止服务端保存并复用现有轻提示。
- 编辑既有资源时，owner DTO 自带引用可直接作为当前合法选择，不必为保存重新搜索；现有简历列表和预览仍只显示学校名，不新增城市文案。用户重新搜索学校时，候选再由 Institution API 提供城市与国家。
...
- 中国城市列表只包含后端判定可选的地级行政区及以上地点，并使用后端中文 `display_name`；中国学校及其地点也为中文。院校查询适配保留 `location`，学校候选携带城市和国家，英文/原名搜索仍选择同一 ID；Education 写入只携带所选 `institution_id`。
...
- 院校候选主行显示学校名、副行显示“城市 · 国家”，不新增城市或国家选择步骤；简历列表和预览保持现有学校名展示。
...
- 院校按城市/国家先筛后选的独立 UI：只有全局搜索结果噪声或同名误选形成真实问题后再增加；当前候选副行已经能完成消歧。
