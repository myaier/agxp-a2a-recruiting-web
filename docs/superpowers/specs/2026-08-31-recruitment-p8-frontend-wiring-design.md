...
本设计的首要产品约束是保留前端视觉：不重排现有页面、不重做 CSS、不改动效和操作层级。除“导出我的数据”缺少任何产品入口、必须增加一行之外，接线只替换数据、提交行为、必要状态和不真实文案。视觉回归把这一约束作为正式验收门。
...
- `举报层.tsx` 由职位详情、Mock 直聊和真人会话共用；提交只弹提示，勾选屏蔽时向本地 reducer 派发名称级拉黑。
...
| Backend 职位详情 | 传 `job` target | 不改举报入口视觉 |
| Backend P7 真人会话 | 传 `conversation` target | 不改举报入口视觉 |
| Backend 直聊 | 隐藏无权威 target 的举报入口 | Mock 保持原样 |
