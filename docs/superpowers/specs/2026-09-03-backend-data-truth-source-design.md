...
新增一个只供 Backend 会话边界使用的根状态清理动作，原子清空这四个数组。它接入统一登出/当前轮 401 清理，以及主体基串（`subject_id + role`）变化的现有 P5 清理点。本批目标页即使处于过渡渲染也不会读取这些数组；双端代理详情的展示型在谈数改读共享 P5 selector，看市场与同 Tab 的在谈首页共用当前范围的 P5 横幅投影，不能把清空后的数组长度 `0` 冒充权威零。岗位管理、岗位详情和发布岗位只按岗位编号匹配 legacy 列表作停止/删除保护谓词；Backend 岗位 ID 与 Mock 候选不同源，结果恒为无匹配，本批不把这种布尔守卫扩成统计接线。已有明确 Backend/Mock 分支的历史页和消息页在 Backend 分支本就不读取这些数组。Mock 初始化和 reducer 行为不变。
...
同模块再提取候选 P5 横幅的纯投影，供同一“职位”Tab 的“在谈/看市场”两个子视图共用。两个屏按 `在谈范围` 选择同一个当前意向或 unfiltered scope；纯函数对当前 owner、加载阶段、已载 `needsAction` 和 `nextCursor` 返回既有四态文案。看市场只消费已有快照，不注册/请求 P5，因此直达时与在谈页同口径显示“正在读入在谈职位…”，不会自造第五套文案。
...
- P5 open/history 组件都拒绝 owner 不匹配快照；看市场与在谈首页按同一 `在谈范围` 共用既有横幅四态投影，双端代理详情复用 unfiltered open 统计；相邻展示页未载快照时显示中性缺失态且不自行请求；
