...
affiliations 或公开企业失败把聚合状态设为失败。底层 `重新水合招聘方组织()` 从整条组织链开头重跑，避免对半完成
状态做局部猜测；受保护招聘路径上的失败恢复由应用层统一入口依次重跑组织链和 owner jobs。
...
2. 聚合组织水合失败：由应用层 recruiter guard 显示错误、“重试”和“切换身份”；重试依次恢复组织链与 owner jobs，
   公司档案页面及其分区深链不再拥有第二套失败分支。
