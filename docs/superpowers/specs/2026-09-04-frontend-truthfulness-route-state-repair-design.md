...
Backend 直接从已水合权威简历快照的 `profile.status` wire 值投影：
...
`后端状态.简历快照 === null` 或 `profile.status === ''` 时显示 `—` 或加载中，不读取 `状态.基本信息.身份` 中为注册流提供的“在职”默认值。Backend 行不可点击，不创建本地轮转，也不新增 intention PATCH 字段。
...
默认“功能异常”。可见分类、placeholder、可提交判断和 payload 必须从同一闭合表派生。复用表单底部现有说明槽位，持久显示现有指引“举报要从具体的岗位、谈判或真人会话里发起；这里只收集产品反馈。”；Backend DOM 不出现两个举报分类，也不存在无目标举报分支。
...
页面不新增分组标题或布局节点；在现有诊断列表行的文字内分别加“待补全 ·”与“可提升 ·”前缀，并继续复用相同 row/class。标题计数只读待补全；待补全为零但仍有建议时，标题可显示“资料已补全”，摘要必须明确标为“可提升”，现有展开按钮改为“看建议”。Mock 可继续使用原型标题和原型建议口径，但两种模式不得共用会让 Backend 恢复五技能阈值的计数。
