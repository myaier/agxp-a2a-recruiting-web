...
旅程创建的对象带固定验收前缀和本次 run ID，例如：

```text
浏览器验收岗位 · 20260829T123456Z
```

正常路径通过 UI 删除。若旅程中途失败，`cleanup` 只允许查找并删除两个专用账号拥有、名称带精确验收前缀的对象。它不得按模糊前缀跨账号删除，也不得清理没有 fixture ownership 证明的对象。
...
路径：修改简历摘要 -> 新建意向 -> 编辑意向 -> 删除意向 -> 修改并恢复隐私设置 -> 上传附件 -> 替换附件 -> 删除附件。

每个业务块只在完成后和硬刷新后验证结果，不验证中间 HTTP 序列。新建意向和附件使用临时对象；基准简历/隐私在旅程结束时恢复，最终由 `verify` 再确认。
