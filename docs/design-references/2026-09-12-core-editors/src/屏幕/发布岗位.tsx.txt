...
function 岗位编辑表单({ 路由岗位编号 }: { 路由岗位编号?: string }) {
  ...
  // P4 互认 Task 3：结构化要求确认。只有 Backend 有这道闸（Mock 发岗语义冻结）；
  // 编辑态按存量 Owner Job 的确认事实水合，新建一律从未确认起步
  const [结构化要求已确认, 设结构化要求已确认] = useState(
    是后端 ? 编辑目标?.结构化要求已确认 === true : false,
  );
  const [结构化确认需注意, 设结构化确认需注意] = useState(false);

  // 三个结构化输入的写入都走这层包装：Backend 下真实改值就撤掉确认，
  ...
  const 改最低学历 = (next: string) => {
    if (next !== 最低学历 && 是后端) 设结构化要求已确认(false);
    设最低学历(next);
  };
  ...
  /** P4 互认 Task 3：Backend 提交前还缺不缺一次显式确认。
   *  新建必须已确认；编辑只有经验 / 学历 / 职位要求三者真的被改过才需要（重新）确认，
   *  legacy-false 岗位显式勾选后原样保存也放行（由 mapper 发仅含确认的补丁）。 */
  const 结构化确认缺失 = (): boolean => {
    if (!是后端) return false;
    if (!编辑目标) return !结构化要求已确认;
    // 与 转岗位补丁 同口径：校招/实习落库一律「不限」，残留档位不算变化
    const 经验有变化 = (按年限筛(招聘类型) ? 经验要求 : '不限') !== 经验要求初值;
        ...
        {第几步 === 2 ? (
          <职位要求步
            ...
            设职位要求={(值) => {
              ...
              if (值 !== 职位要求 && 是后端) 设结构化要求已确认(false);
              设职位要求(值);
            }}
            筛选要求={筛选要求}
            设筛选要求={设筛选要求}
            // P4 互认 Task 3：结构化要求确认勾选框（仅 Backend 渲染在第三步）
            结构化要求已确认={结构化要求已确认}
            设结构化要求已确认={(值) => {
              设结构化要求已确认(值);
...
function 职位要求步({
  ...
  筛选要求: string;
  设筛选要求: (值: string) => void;
  // P4 互认 Task 3：结构化要求确认勾选框（仅 Backend）
  结构化要求已确认: boolean;
