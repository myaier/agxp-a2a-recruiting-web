...
case "${1:-}" in
  ...
  find)
    ...
    # 发布页标题只能在真实点击之后出现；若预先塞进假页，click_until_screen 会把
    # 前一屏误判为已到达，合同测试反而无法证明「发布新岗位」真的被点过。
    if [ "${2:-}" = 'role' ] && [ "${3:-}" = 'button' ] && [ "${4:-}" = 'click' ] \
      && [ "${6:-}" = '发布新岗位' ]; then
      printf '%s\n' '岗位基础信息' >>"$state_dir/body.txt"
    fi
...
seed_recruiter_page(){
  # click_until_screen 在换屏后以 on_screen 标记裁决导航成败：这里的 body 需要带上
  # 公司资料列表的标题（编辑品牌信息）与岗位管理组标（在招/已归档），模拟「导航已落屏」。
  # 发布页标题由假 CLI 在点击「发布新岗位」之后加入，不能预先出现在前一屏。
  printf '%s\n' '浏览器验收招聘官 招聘负责人 浏览器验收科技 浏览器验收岗位 · 在招基线 浏览器验收岗位 · 归档基线 编辑品牌信息 在招 已归档' \
    >"$FAKE_STATE/body.txt"
}
...
assert_true 'toast 稳定等待排在岗位基线截图之前' \
  "W=\$(grep -n 'wait 2000' '$CALLS' | tail -1 | cut -d: -f1); \
   S=\$(grep -n 'screenshot .*recruiter-jobs-after-create.png' '$CALLS' | tail -1 | cut -d: -f1); \
   [ -n \"\$W\" ] && [ -n \"\$S\" ] && [ \"\$W\" -lt \"\$S\" ]"
