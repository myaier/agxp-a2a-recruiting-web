...
// 叠加一层覆盖路由（Playwright 后注册的路由优先）。覆盖层的路由谓词只拦两类路径，
// 谓词未命中的请求根本不进覆盖层，直接由先注册的 P1 白名单路由应答（应用启动水合、
// 岗位 GET、企业公开档、真人会话……全部沿用既有白名单与场景，白名单外由 P1 受控 503）；
// 进入覆盖层但 fixture 未认识的组合落覆盖层自己的受控 503，绝不放行真实网络。两类路径：
 ...
 * 再叠覆盖层 —— 后注册者优先；覆盖层谓词只拦助手两路路径，谓词未命中的请求不进
 * 覆盖层，由 P1 白名单路由直接应答。
 */
