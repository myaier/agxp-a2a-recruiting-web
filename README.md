...
求职者与招聘方分别告诉自己的 AI 代理条件，先发现机会并显式委托，再经历 S0 匿名初筛、
S1 简历递交与筛选、S2 分歧协调、S3 双方意向确认，随后转真人沟通。
候选人经有效授权成功递交 PDF 原件后，原件内姓名和联系方式在 S1 即向招聘方披露；
S3 不再承担首次身份披露。公开岗位薪资与双方私有条件按各自合同处理，不混为同一种数据。
...
- [旅程文档索引](docs/journeys/README.md) — 全量盘点、首条试点与后续阶段澄清
...
- Backend 已有接线：身份/会话、目录、简历与附件解析/预填、意向、岗位/JD导入、组织资料/认证、隐私、发现/委托、MatchCase四阶段及S0记录、Agent规则/设置、真人文本会话、接触记录和账号生命周期。
- 接线存在不等于真实旅程通过；各域实现、原型差异与验收证据见旅程盘点。AI自由聊天/日报、独立初筛周报、直接聊及部分外部接入仍没有对应真实页面闭环。
- 接口失败不回退 Mock。
- `加分关键词/实习转正` 是按后端环境+岗位 ID 保存的本浏览器附属数据；作品集入口仍有本地字段。P2附件文件及版本已有真实服务端资源，不能再统称为“仅本地文件名”。
...
- `local`：本地 BFF 监听 `127.0.0.1:8097`，配置 `RECRUITMENT_BFF_ENV=test` +
  `RECRUITMENT_BFF_PUBLIC_ORIGIN=http://localhost:5173`。
- 后端须部署与当前前端严格契约匹配的全部目标旅程端点，不能只验证 auth / resume / jobs。实施和验收时记录前端、BFF、Recruitment及相关Hub运行commit。
- Backend接口失败不回退演示种子；尚存原型数字和模拟接入的具体页面已列入旅程盘点缺口，不应作为真实能力或验收结果。

### Mock / Backend 本地缓存隔离

- Mock 只有一个 `demo` 账号；简历、引导答案和资料快照在 `localStorage` 中按
  `mock + 后端环境 + demo` 分仓。旧全局键会在成功迁移后删除。
- Backend已接业务事实以服务端回读为权威；头像、公司资料现已有真实接口，不能再视为纯本地字段。
...
> `docs/superpowers/specs/2026-08-29-agent-browser-real-backend-e2e-design.md`（仅存档）。

### 原型与真实能力边界

Mock和Backend仍有页面与交互分支，不能理解成只替换同一套数据。市场、在谈、规则、会话和历史已有Backend实现；AI自由对话、日报、独立完整往来记录等仍没有真实闭环。初始化固定扫描数字与代理详情模拟接入是已记录的真实性缺口，详见旅程报告G04。

`加分关键词` / `实习转正`仍是非权威本地附属字段，作品集存在本地入口；实际PDF附件库已通过P2写入服务端。不要把本地附属字段与真实附件版本混用。
